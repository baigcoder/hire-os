...
import { rankApplicantsForJob, getBulkRecommendations } from "../utils/applicantRanker.js";
    ...
    try {
        ...
        try {
            ...
            if (io && job.company?._id) {
                const { broadcastNewApplication } = await import('../utils/dashboardSocket.js');
            ...
            console.log('Dashboard socket broadcast failed (non-critical):', socketError.message);
        }
...
export const updateStatus = async (req, res) => {
    try {
        ...
        if (status === "interview" && interviewDate) {
            ...
            console.log(`Interview scheduled for ${application.applicant.fullname} for job ${application.job.title} at ${application.job.company.name}`);
        }
...
export const passToCEO = async (req, res) => {
    try {
        ...
        const company = await Company.findById(recruiter.companyId || application.job.company);
        if (!company) {
            return res.status(404).json({
        ...
        try {
            const io = req.app.get('io');
            if (io && company._id) {
                const { broadcastNewApproval } = await import('../utils/dashboardSocket.js');
...
export const getRecruiterNotifications = async (req, res) => {
    try {
        ...
        const companyJobs = await Job.find({ company: recruiter.companyId }).select('_id');
