    ...
    try {
        ...
        if (adminUser) {
            ...
            if (adminUser.role !== 'company_admin') {
                return res.status(400).json({
                    message: "This email is registered with a different role. Please use a company admin account.",
                    success: false
                });
            }
        ...
        const company = await Company.create({
            ...
            payments: paymentToken ? [{
                ...
                amount: selectedPlan.prices?.[billingCycle] || selectedPlan.prices?.monthly || 5000,
                currency: 'PKR',
                plan: planId || 'basic',
        ...
        if (recruitersToInvite && Array.isArray(recruitersToInvite) && recruitersToInvite.length > 0) {
            for (const recruiter of recruitersToInvite) {
                if (!recruiter.email || !recruiter.name) continue;

                // Check if email already exists
                const existingRecruiter = await User.findOne({ email: recruiter.email.toLowerCase() });
                ...
                console.log('📝 Generated password for recruiter:', recruiter.email, '=', recruiterPassword);

                const hashedRecruiterPassword = await bcrypt.hash(recruiterPassword, 12);
                console.log('🔐 Hashed password length:', hashedRecruiterPassword.length);
                ...
                try {
                    ...
                    console.error(`❌ Failed to send invitation to ${recruiter.email}:`, emailError);
                }
        ...
        if (paymentToken) {
            try {
                ...
                console.error(`❌ Failed to send invoice to ${adminEmail}:`, emailError);
            }
        }
...
export const getCompany = async (req, res) => {
    try {
        ...
        // Log for debugging
        console.log(`[getCompany] User ${userId} (${user.role}) - Found ${companies.length} companies`);
...
export const updateCompany = async (req, res) => {
    try {
        const { name, description, website, location, phone, industry, companySize } = req.body;
        const companyId = req.params.id;
...
export const inviteRecruiter = async (req, res) => {
    try {
        ...
        const existingRecruiter = company.recruiters.find(
            r => r.email.toLowerCase() === email.toLowerCase() && r.status !== 'removed'
        );
        ...
        company.recruiters.push({
            ...
            invitedAt: new Date(),
            permissions: permissions || ['post_jobs', 'view_applications', 'schedule_interviews']
        });
    ...
    try {
        ...
        if (user) {
            ...
            user = await User.create({
                fullname: fullname || recruiterInvite.name || recruiterInvite.email.split('@')[0],
                ...
                recruiterProfile: {
                    ...
                    canManageApplications: recruiterInvite.permissions.includes('view_applications'),
                    canScheduleInterviews: recruiterInvite.permissions.includes('schedule_interviews')
                },
                invitation: {
                    status: 'accepted',
...
export const getRecruiters = async (req, res) => {
    try {
        ...
        const company = await Company.findOne({ adminUser: adminId })
            .populate('recruiters.userId', 'fullname email profile.profilePhoto lastLogin');
...
export const updateRecruiter = async (req, res) => {
    try {
        ...
        const recruiterIndex = company.recruiters.findIndex(
            r => r._id.toString() === recruiterId || r.userId?.toString() === recruiterId
        );
        ...
        if (recruiterId2 && permissions) {
            await User.findByIdAndUpdate(recruiterId2, {
                ...
                'recruiterProfile.canManageApplications': permissions.includes('view_applications'),
                'recruiterProfile.canScheduleInterviews': permissions.includes('schedule_interviews'),
...
export const getCompanyDashboard = async (req, res) => {
    try {
        ...
        const planDetails = SUBSCRIPTION_PLANS[company.subscription.plan] || SUBSCRIPTION_PLANS.basic;
        ...
        return res.status(200).json({
            ...
            company: {
                ...
                usage: {
                    ...company.usage.toObject ? company.usage.toObject() : company.usage,
                    totalApplications,
                    interviewsConducted,
                    hiresThisMonth,
                    activeJobs,
                ...
                recruitersCount: company.recruiters.filter(r => r.status === 'active').length
            },
            userRole: user.role,
...
export const resendInvitation = async (req, res) => {
    try {
        ...
        company.recruiters[recruiterIndex].invitationExpiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
...
export const getPendingApprovals = async (req, res) => {
    try {
        ...
        const pendingApprovals = await Application.find({
            ...
            .populate('applicant', 'fullname email profile.profilePhoto profile.skills')
...
export const updateCompanyProfile = async (req, res) => {
    try {
        ...
        const updatedCompany = await Company.findByIdAndUpdate(company._id, updateData, { new: true });
