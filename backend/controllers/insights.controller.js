...
export const getHiringInsights = async (req, res) => {
  try {
    ...
    const pendingApprovals = applications.filter(
      (a) => a.status === "pending_ceo_approval",
    ).length;
    ...
    const insightsData = {
      ...
      avgTimeToHire,
      recruitersCount:
        company.recruiters?.filter((r) => r.status === "active").length || 0,
    };
    ...
    [
      {
        role: "system",
        content:
          "You are a hiring analytics expert. Be concise and actionable.",
      },
...
function generateFallbackInsights(data) {
  ...
  if (data.pendingApprovals > 5) {
    ...
    keyHighlights.push(
      `${data.pendingApprovals} candidates ready for final review`,
    );
  }
  ...
  if (data.inInterview > 0) {
    keyHighlights.push(
      `${data.inInterview} candidates in active interview stage`,
    );
  }
  ...
  return {
    ...
    keyHighlights:
      keyHighlights.length > 0
        ? keyHighlights
        : ["Hiring operations running smoothly"],
    actionItems,
    riskAreas,
    prediction:
      data.inInterview > 0
