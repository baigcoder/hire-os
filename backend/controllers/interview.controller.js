...
export const createInterview = async (req, res) => {
    try {
        ...
        if (personalMessage && personalMessage.trim()) {
            ...
            console.log(`📨 Personal message sent to ${application.applicant.fullname} for interview`);
        }
...
export const getMCQQuestions = async (req, res) => {
    try {
        ...
        if (!interview.mcqTest.questions || interview.mcqTest.questions.length === 0) {
            interview.mcqTest.questions = await generateMCQQuestions(interview.jobId, interview.mcqTest.totalQuestions);
            await interview.save();
        }
...
export const startMCQTest = async (req, res) => {
    try {
        ...
        if (!interview.canStartMCQ()) {
            return res.status(400).json({
                success: false,
                message: 'MCQ test is not yet available. Please wait until the scheduled time.'
            });
        }
        ...
        return res.status(200).json({
            ...
            expiresAt: new Date(interview.mcqTest.startedAt.getTime() + interview.mcqTest.timeLimit * 60 * 1000)
        });
    } catch (error) {
...
export const submitMCQAnswers = async (req, res) => {
    try {
        ...
        // Check time limit
        const elapsedMinutes = (Date.now() - interview.mcqTest.startedAt.getTime()) / (1000 * 60);
        if (elapsedMinutes > interview.mcqTest.timeLimit + 1) { // 1 min grace period
        ...
        answers.forEach(({ questionIndex, selectedAnswer }) => {
            if (interview.mcqTest.questions[questionIndex]) {
                interview.mcqTest.questions[questionIndex].selectedAnswer = selectedAnswer;
            }
        });
        ...
        return res.status(200).json({
            success: true,
            message: passed ? 'Congratulations! You passed the MCQ test!' : 'Unfortunately, you did not meet the passing score.',
            result: {
                score,
...
export const startVideoInterview = async (req, res) => {
    try {
        ...
        if (!interview.canStartVideoInterview()) {
            return res.status(400).json({
                success: false,
                message: 'Video interview cannot be started. Check if MCQ is required and passed.'
            });
        }
...
export const joinVideoInterview = async (req, res) => {
    try {
        ...
        if (interview.videoInterview.studentJoinedAt && interview.videoInterview.recruiterJoinedAt) {
    ...
    try {
        ...
        if (interview.videoInterview.startedAt) {
            interview.videoInterview.duration = Math.round(
                (interview.videoInterview.endedAt - interview.videoInterview.startedAt) / 1000
            );
        }
...
import { generateInterviewReport, generateQuickSummary } from '../utils/reportGenerator.js';
...
export const generateReport = async (req, res) => {
    try {
        ...
        const reportData = {
            ...
            fraudRiskScore: interview.totalFraudAlerts > 5 ? 'high' :
                interview.totalFraudAlerts > 2 ? 'medium' : 'low',
...
export const ceoDecision = async (req, res) => {
    try {
        ...
        interview.finalReport.status = decision === 'approved' ? 'approved' :
        ...
        await Notification.createNotification({
            userId: interview.recruiterId,
            type: decision === 'approved' ? 'candidate_approved' : 'candidate_rejected',
...
export const submitReport = async (req, res) => {
    try {
        ...
        interview.finalReport = {
            ...
            problemSolvingScore,
            cultureFitScore,
            overallScore: Math.round((technicalScore + communicationScore + problemSolvingScore + cultureFitScore) / 4),
            recommendation,
            strengths,
            weaknesses,
            notes
...
export const createOfferLetter = async (req, res) => {
    try {
        const { id } = req.params;
        const { position, department, salary, joiningDate, benefits, additionalTerms } = req.body;
        const recruiterId = req.id;
...
export const sendOfferLetter = async (req, res) => {
    try {
        ...
        try {
            ...
            console.log('Socket broadcast failed (non-critical):', socketError.message);
        }
...
export const respondToOffer = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            success: true,
            message: accepted ? 'Congratulations on your new position!' : 'Response recorded'
        });
    } catch (error) {
