  ...
  try {
    ...
    if (user.role !== "company_admin") {
      return res.status(403).json({
        message:
          "Only Company Admins (CEOs) can post jobs. Recruiters can only manage applications.",
        success: false,
      });
    }
  ...
  try {
    ...
    if (req.id) {
      hasApplied = job.applications.some(
        (app) => app.applicant && app.applicant.toString() === req.id,
      );
    }
  ...
  try {
    ...
    const jobsWithStats = jobs.map((job) => ({
      ...
      stats: {
        ...
        accepted: job.applications.filter((a) => a.status === "accepted")
          .length,
        rejected: job.applications.filter((a) => a.status === "rejected")
          .length,
        interview: job.applications.filter((a) => a.status === "interview")
          .length,
      },
    }));
  ...
  try {
    ...
    const jobsWithStats = jobs.map((job) => ({
      ...
      stats: {
        totalApplications: job.applications.length,
        pending: job.applications.filter((a) => a.status === "pending").length,
        reviewed: job.applications.filter((a) => a.status === "reviewed")
          .length,
        accepted: job.applications.filter((a) => a.status === "accepted")
          .length,
        rejected: job.applications.filter((a) => a.status === "rejected")
...
export const getRecommendedJobs = async (req, res) => {
  try {
    ...
    if (user.jobPreferences?.locations?.length > 0) {
      orConditions.push({
        location: {
          $in: user.jobPreferences.locations.map((l) => new RegExp(l, "i")),
        },
      });
    }
...
export const getJobReportStats = async (req, res) => {
  try {
    ...
    applications.forEach((app) => {
      ...
      if (
        ["offer_pending", "offer_sent", "offer_accepted"].includes(app.status)
      ) {
        stats.offers++;
      }
...
export const getAIMatchedJobs = async (req, res) => {
  try {
    ...
    const strong = matchedJobs.filter(
      (j) => j.match.score >= 70 && j.match.score < 85,
    );
    const good = matchedJobs.filter(
      (j) => j.match.score >= 55 && j.match.score < 70,
    );
  ...
  try {
    ...
    const scoredJobs = similarJobs.map((sj) => {
      ...
      return {
        ...
        matchReason:
          overlap > 0
            ? `${overlap} matching skills`
            : sj.company?._id?.toString() === job.company?.toString()
              ? "Same company"
              : "Similar role",
      };
    });
...
export const getPublicJobsByCompanyId = async (req, res) => {
  try {
    ...
    const jobs = await Job.find({
      ...
      .select(
        "title description location jobType salary experienceLevel position createdAt isRemote skills",
      )
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(Number(limit))
