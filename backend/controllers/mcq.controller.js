...
import { callAI, parseAIJson, generateMCQ as generateMCQFromService } from '../utils/aiServiceV3.js';
...
const generateMCQWithAI = async (jobTitle, skills, experienceLevel, questionCount = 15) => {
    try {
        console.log('🚀 [AI DYNAMIC] Generating INDUSTRIAL-STANDARD MCQ with gpt-4o-mini...');
...
- DO NOT include any text outside the JSON object`;
        ...
        console.log(`✅ [AI DYNAMIC] Industrial-standard MCQ generated using ${result.model} (${result.latency}ms)`);
        ...
        const enhancedQuestions = (parsed.questions || []).map((q, idx) => ({
            ...q,
            index: idx,
            timeEstimate: q.timeEstimate || (q.difficulty === 'hard' ? 120 : q.difficulty === 'medium' ? 90 : 60)
        }));
        ...
        console.log('⚠️ [AI] Primary model failed, trying Gemini fallback:', error.message);
        // Fallback to Gemini
        if (GEMINI_API_KEY) {
            return await generateWithGemini(jobTitle, skills, experienceLevel, questionCount);
        }
...
const generateWithGemini = async (jobTitle, skills, experienceLevel, questionCount) => {
...
export const generateMCQTest = async (req, res) => {
    try {
        ...
        try {
            ...
            const result = await generateMCQWithAI(job.title, skills, experienceLevel, 15);
            ...
            console.log('⚠️ [AI RETRY] First attempt failed, retrying with Gemini...');

            // Retry with Gemini only - NO static fallback
            try {
                const retryResult = await generateWithGemini(job.title, skills, experienceLevel, 15);
                questions = retryResult.questions;
                generatedBy = 'gemini-retry';
                console.log(`✅ [AI DYNAMIC] MCQ generated on retry using Gemini`);
            } catch (retryError) {
                console.error('❌ [AI ERROR] All AI attempts failed:', retryError.message);
...
export const submitMCQTest = async (req, res) => {
    try {
        ...
        console.log(`✅ MCQ test submitted: ${test._id}, Score: ${test.score}%, Status: ${test.status}`);
...
export const reportSuspiciousActivity = async (req, res) => {
    try {
        ...
        if (test.tabSwitchCount >= test.maxTabSwitches) {
            return res.status(200).json({
                message: 'Warning: Multiple tab switches detected. Your test may be flagged.',
                success: true,
                warning: true,
