...
import { generateOTP, saveOTP, verifyOTP, sendOTPEmail, sendWelcomeEmail } from "../utils/email.js";
...
export const sendOTP = async (req, res) => {
    try {
        console.log(`📨 Received OTP request for email: ${req.body.email}, purpose: ${req.body.purpose}`);
        ...
        if (purpose === 'signup' && existingUser) {
            ...
            const roleMessages = {
                'company_admin': 'This email is registered as a Company Owner/CEO account. Please login with your CEO credentials or use a different email.',
                'recruiter': 'This email is registered as a Recruiter account. Please login with your Recruiter credentials or use a different email.',
                'student': 'This email is already registered as a Student account. Please login with your existing credentials or use a different email.'
            };

            return res.status(400).json({
                message: roleMessages[existingUser.role] || "An account with this email already exists. Please login instead.",
...
export const verifySignupOTP = async (req, res) => {
    try {
        const { email, otp, fullname, password, phone, role = 'student' } = req.body;
        ...
        if (role === 'student') {
            ...
            import('../utils/email.js').then(emailModule => {
                emailModule.sendTrialWelcomeEmail(email, fullname, trialEndDate).catch(console.error);
            }).catch(console.error);
        } else {
