...
const PADDLE_CONFIG = {
    ...
    apiUrl: process.env.PADDLE_ENVIRONMENT === 'production'
...
export const verifyPayment = async (req, res) => {
    try {
        ...
        console.log('✅ Payment verified:', { transactionRef, paddleTransactionId });
...
export const handlePaddleWebhook = async (req, res) => {
    try {
        ...
        if (PADDLE_CONFIG.webhookSecret) {
            const isValid = verifyWebhookSignature(signature, rawBody, PADDLE_CONFIG.webhookSecret);
            if (!isValid) {
    ...
    try {
        ...
        if (transactionRef) {
            ...
            if (pendingData?.companyData?.email) {
                company = await Company.findOne({ email: pendingData.companyData.email.toLowerCase() });
            }
        }
        ...
        if (!company && paddleCustomerId) {
            company = await Company.findOne({ 'subscription.paddleCustomerId': paddleCustomerId });
        }
...
const handlePaymentFailed = async (data) => {
    ...
    try {
        ...
        if (companyEmail) {
            ...
            company = await Company.findOne({ 'subscription.paddleCustomerId': data.customer_id });
        }
...
const handleSubscriptionCreated = async (data) => {
    ...
    try {
        ...
        if (company) {
            ...
            company.subscription.status = data.status === 'active' ? 'active' : 'pending';
...
const handleSubscriptionUpdated = async (data) => {
    ...
    try {
        const company = await Company.findOne({ 'subscription.paddleSubscriptionId': data.id });

        if (company) {
            company.subscription.status = data.status === 'active' ? 'active' : data.status;
            ...
            if (data.current_billing_period?.ends_at) {
                company.subscription.endDate = new Date(data.current_billing_period.ends_at);
            }
            await company.save();
