...
const SAFEPAY_CONFIG = {
    ...
    publicKey: process.env.SAFEPAY_PUBLIC_KEY || 'sec_c11137cf-47a8-4d2b-b0e3-1545b3d38b75',
    secretKey: process.env.SAFEPAY_SECRET_KEY || 'ce4bc1209378d9b7d179e8f18d5b74549dca056f8ccabc6fd7af176ddcb8b100',
...
const createSafepaySession = async (amount, currency, metadata, successUrl, cancelUrl) => {
    const transactionId = generateTransactionId();

    try {
        // Create a Tracker token first - This is required for Payments 2.0
        const authHeader = Buffer.from(`${SAFEPAY_CONFIG.publicKey}:${SAFEPAY_CONFIG.secretKey}`).toString('base64');
        ...
        if (response.data && response.data.data && response.data.data.token) {
            ...
            const checkoutUrl = buildCheckoutUrl(tracker, transactionId, successUrl, cancelUrl);
        ...
        console.error('SafePay session error:', error.response?.data || error.message);
...
export const verifyPayment = async (req, res) => {
    try {
        ...
        if (!pending) {
            ...
            const payment = company.payments.find(p => p.transactionId === transactionId);
        ...
        if (tracker && token) {
            try {
                ...
                console.error('SafePay verify error:', verifyError.response?.data || verifyError.message);
            }
        }
...
const handlePaymentSuccess = async (data) => {
    const transactionId = data.order_id || data.transactionId || data.metadata?.transactionId;

    // Update pending transaction
    if (global.pendingTransactions?.[transactionId]) {
        global.pendingTransactions[transactionId].status = 'completed';
        global.pendingTransactions[transactionId].safepayReference = data.tracker || data.reference;
    }
