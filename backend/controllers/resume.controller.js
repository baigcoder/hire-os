  ...
  try {
    ...
    if (userId) {
      try {
        ...
        console.warn(
          "Could not load candidate name for resume analysis:",
          e.message,
        );
      }
    }
    ...
    console.log(
      "🔥 [AI DYNAMIC] Starting resume analysis with unified AI engines...",
    );
    ...
    console.log(
      `   🎯 Engines used: ${result.meta.enginesUsed?.join(", ") || "unified"}`,
    );
    console.log(
      `   📈 Final score: ${result.consensus?.score}/100 (${result.consensus?.confidence}% confidence)`,
    );
    ...
    const quickResult = await analyzeResumeQuick(
      req.body.resumeText,
...
export const getQuickResumeScore = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId)
      .select("profile resume skills bio")
      .lean();
      ...
      [
        {
          role: "system",
          content:
            "You are a professional resume reviewer. Rate profiles on a scale of 0-100 based on completeness, skills relevance, and employability. Be strict but fair.",
        },
        {
          role: "user",
...
Return ONLY valid JSON:
{
    ...
    console.log(
      `✅ [AI DYNAMIC] Profile scored: ${parsed.score}/100 in ${latency}ms`,
    );
