    ...
    try {
        const plans = Object.entries(SUBSCRIPTION_PLANS).map(([key, plan]) => ({
            ...
            features: {
                ...
                maxJobPostings: plan.features.maxJobPostings === -1 ? 'Unlimited' : plan.features.maxJobPostings,
                maxRecruiters: plan.features.maxRecruiters === -1 ? 'Unlimited' : plan.features.maxRecruiters
            }
        }));
...
export const upgradeSubscription = async (req, res) => {
    try {
        ...
        const currentPlanIndex = ['basic', 'professional', 'enterprise'].indexOf(company.subscription.plan);
        const newPlanIndex = ['basic', 'professional', 'enterprise'].indexOf(newPlan);

        if (newPlanIndex <= currentPlanIndex) {
            return res.status(400).json({
                success: false,
                message: 'Can only upgrade to a higher plan. Use downgrade for lower plans.'
            });
        }
...
export const getSubscriptionStatus = async (req, res) => {
    try {
        ...
        if (!endDate) {
            ...
            const durationDays = company.subscription.type === 'yearly' ? 365
                : company.subscription.type === 'quarterly' ? 90
                    : 30;
...
export const getCurrentSubscription = async (req, res) => {
    try {
        ...
        // Get plan details
        const planData = SUBSCRIPTION_PLANS[company.subscription.plan] || SUBSCRIPTION_PLANS.basic;
