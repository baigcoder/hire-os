...
export const invalidateUsersCache = (userId) =>
  ...
  try {
    ...
    if (cache.isConnected) {
      ...
      stats.redis = {
        ...
        hitRate:
          parsedStats.keyspace_hits && parsedStats.keyspace_misses
            ? (
                (parseInt(parsedStats.keyspace_hits) /
                  (parseInt(parsedStats.keyspace_hits) +
                    parseInt(parsedStats.keyspace_misses))) *
                100
              ).toFixed(2) + "%"
            : "N/A",
