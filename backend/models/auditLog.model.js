...
const auditLogSchema = new mongoose.Schema({
    ...
    action: {
        ...
        enum: [
            ...
            // Job actions
            'JOB_CREATE', 'JOB_UPDATE', 'JOB_DELETE', 'JOB_ARCHIVE',

            // Application actions
            'APPLICATION_SUBMIT', 'APPLICATION_WITHDRAW',
            'APPLICATION_APPROVE', 'APPLICATION_REJECT',

            // Interview actions
            'INTERVIEW_SCHEDULE', 'INTERVIEW_COMPLETE', 'INTERVIEW_CANCEL',
            'OFFER_SEND', 'OFFER_ACCEPT', 'OFFER_REJECT',

            // Admin actions
            'SETTINGS_UPDATE', 'SUBSCRIPTION_CHANGE', 'PLAN_CHANGE',
            'EXPORT_DATA', 'BULK_DELETE', 'SYSTEM_CONFIG_CHANGE',

            // AI actions
            'AI_ANALYSIS', 'AI_REPORT_GENERATE',
    ...
    targetType: {
        type: String,
        enum: ['User', 'Job', 'Application', 'Interview', 'Company', 'Subscription', 'System', 'Other'],
        required: true
    },
    targetId: {
        type: mongoose.Schema.Types.ObjectId,
...
auditLogSchema.statics.getForTarget = async function (targetType, targetId, limit = 50) {
...
auditLogSchema.statics.getSecurityAlerts = async function (limit = 100) {
    return this.find({
        action: { $in: ['LOGIN_FAILED', 'PASSWORD_RESET', '2FA_DISABLED', 'BULK_DELETE'] }
    })
        .sort({ timestamp: -1 })
        .limit(limit)
