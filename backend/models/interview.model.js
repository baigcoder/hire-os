...
const interviewSchema = new mongoose.Schema({
    ...
    videoInterview: {
        ...
        status: {
            type: String,
            enum: ['not_started', 'waiting', 'in_progress', 'completed', 'cancelled'],
            default: 'not_started'
        },
        roomId: {
            type: String,
    ...
    offerLetter: {
        status: {
            type: String,
            enum: ['not_applicable', 'pending', 'drafted', 'sent', 'accepted', 'rejected', 'negotiating'],
...
interviewSchema.virtual('criticalAlertsCount').get(function () {
    const mcqCritical = this.mcqTest?.fraudAlerts?.filter(a => a.severity === 'critical' || a.severity === 'high').length || 0;
    const videoCritical = this.videoInterview?.fraudAlerts?.filter(a => a.severity === 'critical' || a.severity === 'high').length || 0;
...
interviewSchema.methods.canStartVideoInterview = function () {
    ...
    if (this.videoInterview.status === 'completed' || this.videoInterview.status === 'cancelled') return false;
    return true;
};
