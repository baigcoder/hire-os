...
const mcqTestSchema = new mongoose.Schema({
    ...
    suspiciousActivity: [{
        type: {
            type: String,
            enum: ['tab_switch', 'copy_paste', 'window_blur', 'rapid_answers', 'time_anomaly']
        },
        timestamp: Date,
        details: String
    }],
...
mcqTestSchema.methods.isExpired = function () {
    ...
    if (this.startedAt) {
        const endTime = new Date(this.startedAt.getTime() + this.duration * 60 * 1000);
