...
router.get('/skill-gap', isAuthenticated, async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).select('profile.skills profile.experience fullname');
        ...
        const allRequiredSkills = recentJobs.flatMap(job => job.requirements || []);
        ...
        const aiResult = await callAI([
            {
                role: 'system',
                content: 'You are a career advisor analyzing skill gaps. Provide actionable insights.'
            },
            {
                role: 'user',
...
${trendingSkills.slice(0, 10).map(s => `- ${s.skill} (${s.demand} jobs)`).join('\n')}
...
router.get('/salary', isAuthenticated, async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).select('profile.skills profile.experience profile.location jobPreferences');

        const userSkills = user?.profile?.skills || [];
        const experienceYears = user?.profile?.experience?.length || 0;
        const userLocation = user?.profile?.location?.city || user?.jobPreferences?.locations?.[0] || null;
        ...
        const salaryData = jobs.map(job => {
            ...
            if (numbers) {
                return {
                    ...
                    min: parseInt(numbers[0]) * (salaryStr.includes('LPA') ? 100000 : 1),
                    max: numbers[1] ? parseInt(numbers[1]) * (salaryStr.includes('LPA') ? 100000 : 1) : parseInt(numbers[0]) * (salaryStr.includes('LPA') ? 100000 : 1)
                };
            }
            return null;
        }).filter(Boolean);
        ...
        const aiResult = await callAI([
            {
                role: 'system',
                content: 'You are a salary analyst. Provide realistic salary expectations based on skills and market data.'
            },
            {
                role: 'user',
...
${salaryData.slice(0, 10).map(s => `- ${s.title} (${s.location || 'Remote'}): ${s.min}-${s.max}`).join('\n')}
...
Return JSON:
{
    ...
    "locationInsight": "<brief insight about ${userLocation || userCountry} job market>"
        ...
        const responseData = {
            ...
            currency: userCountry === 'Pakistan' ? 'PKR' : userCountry === 'India' ? 'INR' : 'USD',
            insights: {
                ...insights,
                userLocation: userLocation || userCountry,
                analyzedAt: new Date().toISOString()
...
router.get('/learning', isAuthenticated, async (req, res) => {
    try {
        ...
        const aiResult = await callAI([
            {
                role: 'system',
                content: 'You are a learning advisor. Recommend specific courses and resources with real platforms like Udemy, Coursera, YouTube, freeCodeCamp.'
            },
            {
                role: 'user',
...
router.get('/timeline', isAuthenticated, async (req, res) => {
    try {
        ...
        const user = await User.findById(userId)
            .select('profile.experience profile.education profile.skills createdAt fullname')
            .lean();
