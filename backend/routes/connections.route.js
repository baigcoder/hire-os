...
router.get('/', isAuthenticated, async (req, res) => {
    try {
        ...
        const connections = await Connection.find({
            ...
            .populate('requester', 'fullname email profile.profilePhoto profile.bio profile.skills role')
            .populate('recipient', 'fullname email profile.profilePhoto profile.bio profile.skills role')
...
router.get('/pending', isAuthenticated, async (req, res) => {
    try {
        ...
        const pending = await Connection.find({
            ...
            .populate('requester', 'fullname email profile.profilePhoto profile.bio profile.skills')
            .sort({ createdAt: -1 });
...
router.get('/suggestions', isAuthenticated, async (req, res) => {
    try {
        const userId = req.id;
        const currentUser = await User.findById(userId).select('profile.skills role');
        ...
        const userSkills = new Set((currentUser.profile?.skills || []).map(s => s.toLowerCase()));
        const scoredSuggestions = suggestions.map(user => {
            const theirSkills = (user.profile?.skills || []).map(s => s.toLowerCase());
            const sharedSkills = theirSkills.filter(s => userSkills.has(s));
            return {
                ...user,
                matchScore: theirSkills.length > 0
                    ? Math.round((sharedSkills.length / theirSkills.length) * 100)
                    : 0,
                sharedSkills,
                reason: user.role === 'recruiter'
