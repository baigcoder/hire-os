...
router.get('/', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        ...
        if (templates.length === 0) {
            const defaultTemplates = await EmailTemplate.createDefaultTemplates(company._id, userId);
...
router.post('/:id/preview', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, message: 'Error previewing template' });
    }
});
