  ...
  try {
    ...
    if (!interviewId || !applicationId || !recommendation) {
      return res.status(400).json({
        success: false,
        message:
          "Interview ID, Application ID, and recommendation are required",
      });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error saving feedback" });
  }
});
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const averageScores =
        await InterviewFeedback.getCandidateAverageScores(applicationId);
...
router.get("/my-drafts", isAuthenticated, isRecruiter, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error fetching drafts" });
  }
});
...
router.get("/pending/list", isAuthenticated, isRecruiter, async (req, res) => {
  try {
    ...
    const completedInterviews = await Interview.find({
      company: company._id,
      status: "completed",
      $or: [{ conductedBy: userId }, { interviewers: userId }],
    })
      .populate("applicationId", "status")
      .populate({
        path: "applicationId",
    ...
    const submittedInterviewIds = feedbackSubmitted.map((f) =>
      f.interviewId.toString(),
    );
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error fetching pending feedback" });
  }
});
