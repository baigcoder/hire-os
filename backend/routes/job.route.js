...
import isAuthenticated, { isRecruiter, isStudent, isCompanyAdmin } from "../middlewares/isAuthenticated.js";
import { validateJobPosting } from "../middlewares/validation.js";
// Performance & Security middleware
import { cacheJobs, cacheJobDetails, invalidateJobsCache } from "../middlewares/cacheMiddleware.js";
import { aiLimiter, jobPostLimiter, searchLimiter } from "../middlewares/rateLimiters.js";
...
router.route("/recommended").get(isAuthenticated, aiLimiter, getRecommendedJobs);
router.route("/ai-matched").get(isAuthenticated, aiLimiter, getAIMatchedJobs);
router.route("/:id/match-score").get(isAuthenticated, aiLimiter, getJobMatchScore);
...
router.route("/post").post(isAuthenticated, isCompanyAdmin, jobPostLimiter, validateJobPosting, invalidateJobsCache, postJob);
router.route("/update/:id").put(isAuthenticated, isCompanyAdmin, invalidateJobsCache, updateJob);
router.route("/delete/:id").delete(isAuthenticated, isCompanyAdmin, invalidateJobsCache, deleteJob);
router.route("/toggle-status/:id").patch(isAuthenticated, isCompanyAdmin, invalidateJobsCache, toggleJobStatus);
