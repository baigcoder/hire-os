...
router.get('/preferences', isAuthenticated, async (req, res) => {
    try {
        const userId = req.id;
        const user = await User.findById(userId).select('notifications jobPreferences');
...
router.put('/preferences', isAuthenticated, async (req, res) => {
    try {
        const userId = req.id;
        const { emailAlerts, locations, jobTypes, salaryMin, salaryMax, industries } = req.body;
...
router.get('/daily', isAuthenticated, async (req, res) => {
    try {
        ...
        if (preferences.locations?.length > 0) {
            query.location = { $in: preferences.locations.map(l => new RegExp(l, 'i')) };
        }
        ...
        const scoredJobs = jobs.map(job => {
            ...
            return {
                ...
                matchScore,
                matchedSkills,
                missingSkills: jobRequirements.filter(r =>
                    !userSkillsLower.some(s => r.includes(s) || s.includes(r))
                ),
                isNew,
                isSaved,
                alertReason: matchScore >= 70
                    ? 'Great skill match!'
                    : isNew
                        ? 'New job posted'
