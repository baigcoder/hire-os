...
router.post('/start', isAuthenticated, async (req, res) => {
    try {
        ...
        console.log(`🎤 [LiveInterview] Starting ${type} interview for ${candidateName}`);
        console.log(`📞 [LiveInterview] Call type: ${callType}, Voice: ${voiceId || 'auto'}`);

        // Check character budget
        if (!hasCharacterBudget(500)) {
            return res.status(429).json({
                success: false,
                message: 'Voice character limit reached for this month. Try again next month.',
        ...
        const persona = INTERVIEWER_PERSONAS[type] || INTERVIEWER_PERSONAS.behavioral;

        // Determine voice to use
        const selectedVoice = voiceId ||
            (type === 'behavioral' ? VOICES.MATILDA :
                type === 'technical' ? VOICES.LIAM : VOICES.RACHEL);
...
Keep it under 150 characters for voice generation.`;
        ...
        let audioData = null;
        try {
            console.log(`🎤 [ElevenLabs] Calling TTS API with voice: ${selectedVoice}`);
            audioData = await textToSpeech(greetingText, {
                voiceId: selectedVoice
            });
            console.log(`✅ [ElevenLabs] Audio generated successfully - ${audioData.charCount} chars used`);
        } catch (audioError) {
            console.error('❌ [ElevenLabs] Audio generation failed:', audioError.message);
        }
...
router.post('/respond', isAuthenticated, async (req, res) => {
    try {
        ...
        console.log(`🎤 [LiveInterview] User: "${userSpeech.substring(0, 100)}..."`);
...
IMPORTANT: Keep response under 200 characters for voice. Be conversational, not formal.`;
        ...
        const shouldMoveToNext = session.responses.filter(r =>
            r.question?.question === currentQuestion?.question
        ...
        if (shouldMoveToNext && shouldContinue) {
            ...
            if (!session.questions[session.currentQuestionIndex]) {
                try {
                    const nextQResult = await callAI(`Generate the next ${session.type} interview question.
Position: ${session.jobTitle}
...
Return JSON: { "question": "...", "category": "...", "tips": "..." }`, {
        ...
        res.json({
            ...
            progress: {
                ...
                totalQuestions: 5,
                shouldContinue,
                nextQuestion: shouldContinue ? session.questions[session.currentQuestionIndex] : null
            },
            characterUsage: getCharacterUsage()
        });
...
router.post('/end', isAuthenticated, async (req, res) => {
    try {
        ...
        const userResponses = session.responses.filter(r => r.answer && r.answer.trim().length > 10);
...
Provide JSON:
{
    ...
    "tips": ["<3 actionable tips for improvement>"]
        ...
        try {
            ...
            console.log(`📊 [LiveInterview] Saved to practice history for user ${req.id}`);
        } catch (historyError) {
            console.error('⚠️ [LiveInterview] Failed to save to history:', historyError.message);
        }
...
router.get('/settings', async (req, res) => {
    ...
    res.json({
        ...
        speedPresets: [
            ...
            { id: 'normal', name: 'Normal', description: 'Natural conversational pace' },
        ...
        stylePresets: [
            ...
            { id: 'expressive', name: 'Expressive', description: 'Animated and varied' }
        ],
        defaults: {
            speed: 'normal',
            style: 'professional'
...
router.post('/stream', isAuthenticated, async (req, res) => {
    try {
        ...
        console.log(`🔊 [LiveInterview] Streaming audio: ${text.length} chars, lang=${language}`);
        ...
        try {
            ...
            console.error('❌ [LiveInterview] Stream setup error:', streamError.message);
            res.status(500).json({
                success: false,
...
function cleanupOldSessions() {
    ...
    for (const [sessionId, session] of interviewSessions.entries()) {
        if (now - session.lastActivity > SESSION_TIMEOUT) {
            console.log(`🧹 [LiveInterview] Cleaning up expired session ${sessionId}`);
            interviewSessions.delete(sessionId);
        }
    }
}
