...
router.post("/generate-questions", isAuthenticated, async (req, res) => {
  try {
    ...
    const result = await callGPT5(
      `You are an experienced, friendly interviewer. ${prompts[type] || prompts.behavioral}
...
Also include an introduction as the first item:
{
    ...
    console.log(
      `✅ [Interview] Generated ${questions.length} questions using ${result.model}`,
    );
...
router.post("/feedback", isAuthenticated, async (req, res) => {
  try {
    ...
    const feedback = await getInterviewResponse(
      question,
      answer,
      jobContext,
      previousQA,
