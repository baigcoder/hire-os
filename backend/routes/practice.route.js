...
async function generateMCQWithAI(prompt, topic) {
  ...
  try {
      ...
      [
        {
          role: "system",
          content:
            "You are an expert interview question designer. Generate realistic MCQ questions based on the user prompt. Always follow the requested JSON schema exactly and do not add any text outside JSON.",
        },
        {
          role: "user",
          content: prompt,
    ...
    if (response && response.content) {
      ...
      console.log(
        `📊 [MCQ AI] Response length: ${response.content.length} chars`,
      );
...
router.post("/mcq/generate", isAuthenticated, async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select(
      "profile.skills fullname profile.experienceYears",
    );
    ...
    const difficultyDescriptions = {
      ...
      medium:
        "intermediate level, testing practical knowledge for 1-3 years of experience",
...
- For ${selectedTopic}, focus on commonly asked interview topics`;
    ...
    try {
        ...
        .map((q, index) => {
          ...
          const safeOptions =
            options.length === 4
              ? options
              : options
                  .concat(Array(Math.max(0, 4 - options.length)).fill("Option"))
                  .slice(0, 4);
          ...
          return {
            ...
            explanation:
              q.explanation ||
              "Review the concepts behind this question to understand why this option is correct.",
      ...
      console.log(
        `✅ [MCQ] Prepared ${questions.length} validated questions from AI`,
      );
    } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to generate test" });
  }
});
...
router.post("/mcq/submit", isAuthenticated, async (req, res) => {
  try {
    ...
    if (!test) {
      return res
        .status(404)
        .json({ success: false, message: "Test not found or expired" });
    }
...
router.get("/mcq/:testId", isAuthenticated, async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: "Test has expired" });
    }
...
function generateFallbackQuestions(topic, difficulty, count) {
  const questions = [
    ...
    },
    {
      ...
      correctAnswer: 0,
      explanation:
        "Following coding standards ensures maintainability and collaboration.",
    },
    {
      ...
      correctAnswer: 0,
      explanation:
        "Scalability ensures the project can grow with user demands.",
    },
    {
    ...
    },
    {
      ...
      correctAnswer: 0,
      explanation:
        "Modular design breaks down complex systems into manageable, reusable components.",
    },
    {
      ...
      correctAnswer: 0,
      explanation:
        "Version control allows teams to track, manage, and collaborate on code changes.",
    },
    {
    ...
    },
    {
      ...
      options: [
        "Gracefully with proper messages",
        "Ignored silently",
    ...
    },
    {
      ...
      correctAnswer: 0,
      explanation:
        "Clear communication ensures team alignment and reduces misunderstandings.",
    },
    {
      ...
      correctAnswer: 0,
      explanation:
        "Regular code reviews catch issues early and share knowledge among team members.",
    },
  ];
