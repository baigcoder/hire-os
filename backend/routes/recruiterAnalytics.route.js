...
router.get('/overview', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        ...
        const hireGrowth = hiresLastMonth > 0
        ...
        return res.status(200).json({
            ...
            overview: {
                ...
                hireGrowth,
                responseRate,
                teamSize: company.recruiters.filter(r => r.status === 'active').length
            }
        });
    } catch (error) {
...
router.get('/funnel', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        ...
        // Funnel stages
        const stages = [
            { name: 'Applied', status: ['pending', 'reviewing', 'shortlisted', 'interview', 'offered', 'hired', 'rejected'] },
            { name: 'Reviewed', status: ['reviewing', 'shortlisted', 'interview', 'offered', 'hired', 'rejected'] },
            { name: 'Shortlisted', status: ['shortlisted', 'interview', 'offered', 'hired'] },
        ...
        return res.status(500).json({ success: false, message: 'Error fetching funnel data' });
    }
});
...
router.get('/time-to-hire', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        ...
        const hiredApplications = await Application.find({
            job: { $in: jobIds },
            status: 'hired'
        }).select('createdAt updatedAt job').populate('job', 'title');
        ...
        const daysToHire = hiredApplications.map(app => {
            const days = Math.ceil((new Date(app.updatedAt) - new Date(app.createdAt)) / (1000 * 60 * 60 * 24));
            return { days, jobTitle: app.job?.title || 'Unknown' };
        });

        const sortedDays = daysToHire.map(d => d.days).sort((a, b) => a - b);
        const average = Math.round(sortedDays.reduce((a, b) => a + b, 0) / sortedDays.length);
        ...
        // Group by job
        const byJob = Object.values(daysToHire.reduce((acc, item) => {
            ...
            job: job.job,
            averageDays: Math.round(job.days.reduce((a, b) => a + b, 0) / job.days.length),
            hires: job.count
        }));
        ...
        return res.status(500).json({ success: false, message: 'Error calculating time to hire' });
    }
});
...
router.get('/source', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, message: 'Error fetching source data' });
    }
});
 ...
 */
router.get('/recruiter-performance', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        const userId = req.id;
        const user = await User.findById(userId);

        if (user.role !== 'company_admin') {
            return res.status(403).json({ success: false, message: 'Only admins can view team performance' });
        }
        ...
        const activeRecruiters = company.recruiters.filter(r => r.status === 'active' && r.userId);

        const performance = await Promise.all(activeRecruiters.map(async (recruiter) => {
            ...
            return {
                ...
                stats: {
                    ...
                    successfulHires,
                    hireRate: applicationsReviewed > 0
        ...
        return res.status(500).json({ success: false, message: 'Error fetching performance data' });
    }
});
 ...
 */
router.get('/job-performance/:jobId', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        ...
        return res.status(200).json({
            ...
            jobPerformance: {
                ...
                dailyApplications,
                daysActive: Math.ceil((new Date() - new Date(job.createdAt)) / (1000 * 60 * 60 * 24)),
        ...
        return res.status(500).json({ success: false, message: 'Error fetching job performance' });
    }
});
...
router.get('/activity', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, message: 'Error fetching activity' });
    }
});
...
router.get('/source-stats', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, message: 'Error fetching source stats' });
    }
});
...
router.get('/trends', isAuthenticated, isRecruiter, async (req, res) => {
    try {
        ...
        const applicantsTrend = applicantsLastMonth > 0
        ...
        const jobsTrend = jobsLastMonth > 0
        ...
        const hireRateThis = applicantsThisMonth > 0 ? (hiresThisMonth / applicantsThisMonth) * 100 : 0;
        const hireRateLast = applicantsLastMonth > 0 ? (hiresLastMonth / applicantsLastMonth) * 100 : 0;
        const hireRateTrend = hireRateLast > 0
            ? Math.round(((hireRateThis - hireRateLast) / hireRateLast) * 100)
            : hireRateThis > 0 ? 100 : 0;
        ...
        return res.status(500).json({ success: false, message: 'Error fetching trends' });
    }
});
