...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Server error during resume upload and analysis" });
    }
  },
);
      ...
      try {
        ...
        if (idToLookup) {
          const user = await User.findById(idToLookup)
            .select("fullname name profile")
            .lean();
          if (user) {
            candidateName =
              user.fullname || user.name || user.profile?.fullName || null;
          }
        }
      } catch (e) {
        console.warn(
          "Could not load candidate name for unified analyze:",
          e.message,
        );
      }

      // Run unified analysis with all 3 engines
      const result = await analyzeResumeUnified(
        resumeText,
        jobDescription || "",
        {
          onProgress: (update) => {
        ...
        try {
          await ResumeAnalysis.findOneAndUpdate(
            { userId: candidateId },
            {
              ...
              engineResults: Object.entries(result.engines).map(
                ([name, data]) => ({
                  engine: name,
                  score: data.score,
...
router.get("/quick-score", isAuthenticated, async (req, res) => {
  try {
    ...
    const analysis = await ResumeAnalysis.findOne({ userId }).sort({
      createdAt: -1,
    });
...
function analyzeResumeText(resumeText, jobText) {
  ...
  // Extract experience
  const expMatch = lowerResume.match(
  ...
  const overallScore = Math.round(
    skillMatch * 0.4 + expScore * 0.3 + education.score * 0.3,
  );
...
router.post("/enhanced-analyze", isAuthenticated, async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn(
        "Could not load candidate name for enhanced analyze:",
        e.message,
      );
    }
    ...
    console.log(
      `✅ [Resume] Unified analysis complete | Score: ${analysis.score} | Confidence: ${analysis.confidence}%`,
    );
    ...
    await ResumeAnalysis.findOneAndUpdate(
      { userId, type: "practice" },
      {
        ...
        experienceMatch: analysis.experienceYears
          ? analysis.experienceYears * 15
          : 50,
        educationMatch:
          analysis.educationLevel === "PhD"
            ? 100
            : analysis.educationLevel === "Master"
              ? 85
              : 70,
    ...
    try {
      ...
      if (normalizedText.length < 100) {
        return res.status(400).json({
          success: false,
          message:
            "Resume appears to be empty or contains too little text. Please upload a complete resume.",
          code: "EMPTY_RESUME",
        });
      }
      ...
      if (pageCount > 2) {
        warnings.push(
          `Your resume has ${pageCount} pages. Consider condensing to 1-2 pages for better ATS performance.`,
        );
      }
...
function deriveSuggestedRoles(analysis) {
  const roles = new Set();
  const skillsFromAnalysis =
    analysis.skills?.matched || analysis.keySkillsMatch || [];
  const skills = skillsFromAnalysis.map((s) => String(s).toLowerCase());
  const text = (analysis.onePageText || "").toLowerCase();
  const experienceYears =
    analysis.experience?.yearsEstimate || analysis.experienceYears || null;
...
function buildOnePageSummary(analysis, onePageText) {
  ...
  const matchedSkills =
    analysis.skills?.matched || analysis.keySkillsMatch || [];
  const missingSkills =
    analysis.skills?.missing || analysis.missingSkills || [];
  const years =
    analysis.experience?.yearsEstimate || analysis.experienceYears || null;
  ...
  if (analysis.experience?.hasQuantifiedResults === false) {
    improvementAreas.push(
      'Add quantified results to experience (e.g. "Improved load time by 30%").',
    );
  }
  const lowerText = (onePageText || "").toLowerCase();
  if (!/linkedin\.com/.test(lowerText)) {
    improvementAreas.push(
      "Include a LinkedIn profile link in the contact section.",
    );
  }
  if (!/github\.com/.test(lowerText) && !/gitlab\.com/.test(lowerText)) {
    improvementAreas.push(
      "Add links to GitHub or live projects where relevant.",
    );
  }
...
function fallbackResumeAnalysis(resumeText) {
  ...
  skills.missing = skillKeywords.filter(
    (skill) => !skills.matched.includes(skill),
  );
  ...
  if (skills.matched.length === 0) {
    suggestions.push(
      "Add a dedicated skills section with your main technologies.",
    );
  } else if (skills.matched.length < 3) {
    suggestions.push(
      "Add more technical skills that reflect your experience and tools.",
    );
  }

  if (skills.missing.length > 0) {
    suggestions.push(
      `Consider learning or highlighting skills such as ${skills.missing.slice(0, 3).join(", ")} where relevant.`,
    );
  }

  if (!/experience|work|employed/i.test(text)) {
    suggestions.push(
      "Include a work experience section with roles, companies, and dates.",
    );
  }

  if (!/project|built|developed/i.test(text)) {
    suggestions.push(
      "Add a projects section describing what you built, tech stack, and impact.",
    );
  }

  if (!/education|degree|university|college/i.test(text)) {
    suggestions.push(
      "Include an education section with degree, institution, and graduation year.",
    );
  }

  if (!/github\.com|gitlab\.com|bitbucket\.org|linkedin\.com/i.test(text)) {
    suggestions.push(
      "Add links to your GitHub or LinkedIn profile where appropriate.",
    );
  }
  ...
  return {
    ...
    overallFit:
      score >= 70 ? "Good" : score >= 50 ? "Fair" : "Needs Improvement",
    atsScore: 60,
    skills,
    ...
    suggestions:
      uniqueSuggestions.length > 0
        ? uniqueSuggestions
        : ["Add more detail so we can generate specific suggestions."],
