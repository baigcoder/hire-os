...
router.get("/recruiter", isAuthenticated, async (req, res) => {
  try {
    ...
    const changePercent =
      prevApplications > 0
        ? Math.round(
            ((totalApplications - prevApplications) / prevApplications) * 100,
          )
        : 0;
    ...
    const funnel = {
      ...
      screened: applications.filter((a) =>
        [
          "reviewing",
          "screened",
          "shortlisted",
      ...
      offered: applications.filter((a) =>
        ["offer_pending", "offer_sent", "offer_accepted", "hired"].includes(
          a.status,
        ),
      ).length,
    ...
    const byStatus = {
      ...
      reviewing: applications.filter(
        (a) => a.status === "reviewing" || a.status === "screened",
      ).length,
      interviewing: applications.filter((a) =>
        ["interview", "video_scheduled", "video_completed"].includes(a.status),
      ).length,
      offered: applications.filter((a) =>
        ["offer_pending", "offer_sent"].includes(a.status),
      ).length,
      rejected: applications.filter((a) => a.status === "rejected").length,
      hired: applications.filter(
        (a) => a.status === "hired" || a.status === "offer_accepted",
      ).length,
    };
    ...
    const offeredApps = applications.filter((a) =>
      ["offer_sent", "offer_accepted", "hired"].includes(a.status),
    );
    const offerAcceptRate =
      offeredApps.length > 0
        ? Math.round(
            (applications.filter((a) =>
              ["offer_accepted", "hired"].includes(a.status),
            ).length /
              offeredApps.length) *
              100,
          )
