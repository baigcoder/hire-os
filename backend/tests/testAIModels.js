...
const MODELS_TO_TEST = [
    ...
    { name: 'claude-3.5-sonnet', description: 'Claude 3.5 Sonnet (if available)' }
];
...
async function testModel(model) {
    ...
    if (result.success) {
        ...
        console.log(`${colors.gray}  Response: ${result.content.substring(0, 100)}...${colors.reset}`);
        ...
        if (result.usage) {
            console.log(`${colors.gray}  Tokens: ${result.usage.total_tokens}${colors.reset}`);
        }
        results.models[model.name] = { status: 'WORKING', latency: result.latency, actualModel: result.model };
...
async function testFeature(featureKey, modelName) {
    ...
    if (result.success) {
        ...
        console.log(`${colors.gray}  Response Preview: ${result.content.substring(0, 200)}...${colors.reset}`);
        console.log(`${colors.gray}  Latency: ${result.latency}ms${colors.reset}`);
        ...
        if (result.reasoning) {
            console.log(`${colors.cyan}  📝 Reasoning included (${result.reasoning.length} chars)${colors.reset}`);
        }

        results.features[featureKey] = { status: 'WORKING', latency: result.latency };
    } else {
        console.log(`${colors.red}✗ Feature Failed: ${result.error}${colors.reset}`);
...
function printSummary() {
    ...
    console.log(`${colors.cyan}║              AI MODEL TEST SUMMARY REPORT                    ║${colors.reset}`);
    ...
    Object.entries(results.models).forEach(([model, data]) => {
        ...
        const info = data.status === 'WORKING'
            ? `${data.latency}ms | ${data.actualModel}`
            : data.error;
        console.log(`  ${model.padEnd(25)} ${status.padEnd(20)} ${colors.gray}${info}${colors.reset}`);
    });
    ...
    if (workingModels.length > 0) {
        console.log(`  Fastest Model: ${workingModels[0][0]} (${workingModels[0][1].latency}ms)`);
        console.log(`  Recommended for Real-time: ${workingModels[0][0]}`);
        if (workingModels.length > 1) {
            console.log(`  Recommended for Accuracy: ${workingModels[workingModels.length - 1][0]}`);
        }
    }

    const failedModels = Object.entries(results.models).filter(([_, d]) => d.status === 'FAILED');
...
async function runAllTests() {
    console.log(`${colors.cyan}╔══════════════════════════════════════════════════════════════╗${colors.reset}`);
    console.log(`${colors.cyan}║        HIRE.OS AI MODEL VERIFICATION TEST SUITE              ║${colors.reset}`);
    console.log(`${colors.cyan}║        ChatAnywhere API - Full Model Audit                   ║${colors.reset}`);
    console.log(`${colors.cyan}╚══════════════════════════════════════════════════════════════╝${colors.reset}`);

    console.log(`\n${colors.gray}API URL: ${API_URL}${colors.reset}`);
    console.log(`${colors.gray}API Key: ${API_KEY ? '***' + API_KEY.slice(-4) : 'NOT SET'}${colors.reset}`);
    ...
    const workingModel = Object.entries(results.models)
        .find(([_, d]) => d.status === 'WORKING')?.[0] || 'gpt-5-ca';
