...
async function runTests() {
    ...
    console.log(`${colors.cyan}║           AI SERVICE V2 - INDUSTRIAL GRADE TEST              ║${colors.reset}`);
    console.log(`${colors.cyan}╚══════════════════════════════════════════════════════════════╝${colors.reset}`);

    console.log(`\nAPI Keys Status:`);
    console.log(`  ChatAnywhere: ${process.env.GPT5_API_KEY ? '✓ SET' : '✗ NOT SET'}`);
...
`;
        return await analyzeResume(resumeText, 'Senior Software Engineer - React/Node.js', 'test_candidate_1');
    });

    // Test 4: MCQ Generation
    await test('MCQ Generation', async () => {
        return await generateMCQ('React Developer', ['React', 'JavaScript', 'TypeScript'], 'Mid', 3);
    });
    ...
    await test('CEO Report (deepseek-r1 reasoning)', async () => {
        return await generateCEOReport({
            ...
            interviewRating: 8,
            fraudRiskScore: 5,
            recruiterNotes: 'Excellent communication, strong technical skills, team player'
        });
    });
