...
async function runTests() {
  ...
  console.log(
    `${colors.cyan}║              RAG SERVICES TEST SUITE                          ║${colors.reset}`,
  );
  console.log(
    `${colors.cyan}╚══════════════════════════════════════════════════════════════╝${colors.reset}`,
  );
  ...
  const embedding1 = await test("Generate Embedding", async () => {
    const text =
      "Senior React developer with 5 years experience in TypeScript and Node.js";
  ...
  await test("Find Similar Resumes", async () => {
    const results = await findSimilarResumes(
    ...
    if (results.length > 0) {
      console.log(
        `  Top match: ${results[0].metadata.name} (${(results[0].score * 100).toFixed(1)}%)`,
      );
    }
  ...
  await test("Find Matching Jobs", async () => {
    ...
    if (results.length > 0) {
      console.log(
        `  Top match: ${results[0].metadata.title} (${results[0].matchPercentage}%)`,
      );
    }
    return results;
  });
  ...
  await test("RAG Resume Analysis", async () => {
    ...
    console.log(
      `  RAG context: ${analysis.ragContext?.similarCandidates || 0} similar candidates`,
    );
