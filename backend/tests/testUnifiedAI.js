...
async function runTests() {
  ...
  console.log(
    `${colors.cyan}║         UNIFIED AI ENGINES TEST SUITE                        ║${colors.reset}`,
  );
  console.log(
    `${colors.cyan}╚══════════════════════════════════════════════════════════════╝${colors.reset}`,
  );
  ...
  const unifiedResult =
    await test("Unified Analysis (All Engines)", async () => {
      ...
      for (const [engine, data] of Object.entries(result.engines)) {
        ...
        console.log(
          `  ${status} ${engine}: ${data.score || "N/A"} (${data.latency}ms)${colors.reset}`,
        );
      }
  ...
  await test("Job Matching (Multi-Engine)", async () => {
    ...
    result.matches.forEach((m, i) => {
      console.log(
        `  ${i + 1}. ${m.job.title}: ${m.match.score}% (${m.match.fitLevel})`,
      );
    });
  ...
  await test("Applicant Ranking", async () => {
    ...
    result.ranked.forEach((r) => {
      console.log(
        `  #${r.rank} ${r.applicant.name}: ${r.scores.final}% (${r.tier})`,
      );
    });
