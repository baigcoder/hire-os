...
const getAPIKey = (provider) => {
    switch (provider) {
        case 'GPT': return process.env.GPT5_API_KEY || process.env.OPENAI_API_KEY;
...
const PROVIDERS = {
    ...
    GROQ: {
        ...
        models: [AI_MODELS.GROQ_LLAMA_70B, AI_MODELS.GROQ_MIXTRAL, AI_MODELS.GROQ_LLAMA_8B],
        format: 'openai',
        supportsJSON: true
    },
    GEMINI: {
...
export const callAI = async (messages, options = {}) => {
    ...
    if (preferredProvider && PROVIDERS[preferredProvider]) {
        providerChain = [preferredProvider, ...FALLBACK_CHAIN.filter(p => p !== preferredProvider)];
    } else if (modelConfig) {
        ...
        providerChain = [primaryProvider, fallbackProvider, finalProvider].filter(Boolean);
    } else {
    ...
    for (const providerKey of providerChain) {
        ...
        if (modelConfig) {
            ...
            modelToUse = useReasoning && provider.models.includes(AI_MODELS.DEEPSEEK_R1)
        ...
        try {
            ...
            console.log(`✅ [AI] ${provider.name} response: ${result.tokens || 0} tokens, ${latency}ms`);
...
function getProviderForModel(model) {
    if ([AI_MODELS.GPT4O_MINI, AI_MODELS.DEEPSEEK_R1].includes(model)) return 'GPT';
    if ([AI_MODELS.GROQ_LLAMA_70B, AI_MODELS.GROQ_MIXTRAL, AI_MODELS.GROQ_LLAMA_8B].includes(model)) return 'GROQ';
    if ([AI_MODELS.GEMINI_FLASH, AI_MODELS.GEMINI_PRO].includes(model)) return 'GEMINI';
    return 'GPT'; // Default
}

// Check if error is rate limit related
function isRateLimitError(message) {
    const rateLimitPatterns = ['rate', 'limit', '限制', '429', 'too many', 'quota'];
    return rateLimitPatterns.some(pattern => message.toLowerCase().includes(pattern));
}
 ...
 */
async function callOpenAIFormat(url, apiKey, model, messages, temperature, maxTokens, responseFormat, supportsJSON) {
    const body = {
        model,
        messages,
        temperature,
