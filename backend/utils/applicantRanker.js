...
export async function calculateApplicantScore(application, job) {
  try {
    ...
    return {
      ...
      autoAction:
        totalScore < 30 ? "reject" : totalScore >= 80 ? "interview" : "review",
    };
  } catch (error) {
...
function calculateSkillsMatch(applicant, job) {
  const applicantSkills = (applicant.profile?.skills || []).map((s) =>
    s.toLowerCase().trim(),
  );
  const jobSkills = (job.skills || job.requirements || []).map((s) =>
    s.toLowerCase().trim(),
  );

  if (jobSkills.length === 0) {
    return {
      points: SCORE_WEIGHTS.skillsMatch * 0.7,
      matched: [],
      missing: [],
      ratio: 0.7,
    };
  ...
  jobSkills.forEach((skill) => {
    if (
      applicantSkills.some((as) => as.includes(skill) || skill.includes(as))
    ) {
      matched.push(skill);
    } else {
      missing.push(skill);
...
function calculateEducationMatch(applicant, job) {
  ...
  if (
    ...
    educationText.includes("bs") ||
    educationText.includes("ba")
  ) {
    points = SCORE_WEIGHTS.educationMatch * 0.8;
  }
  if (
    educationText.includes("master") ||
    educationText.includes("ms") ||
    educationText.includes("ma")
  ) {
...
export async function rankApplicantsForJob(jobId) {
  try {
    ...
    return {
      ...
      topCandidates: rankedApplicants.filter((a) => a.score.totalScore >= 70)
        .length,
...
export function getBulkRecommendations(rankedApplicants) {
  return {
    ...
    interview: rankedApplicants.filter(
      (a) => a.score.totalScore >= 70 && a.score.totalScore < 85,
    ),
    review: rankedApplicants.filter(
      (a) => a.score.totalScore >= 50 && a.score.totalScore < 70,
    ),
