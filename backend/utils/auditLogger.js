...
export const logAction = async ({
  ...
  try {
    ...
    console.log(
      `📝 Audit: ${action} by ${userEmail} on ${targetType}${targetId ? ":" + targetId : ""}`,
    );
...
export const auditActions = {
  ...
  jobUpdate: (user, job, previousValue, req) =>
    logAction({
      userId: user._id || user.id,
      userEmail: user.email,
      userRole: user.role,
  ...
  jobDelete: (user, jobId, jobTitle, req) =>
    logAction({
      userId: user._id || user.id,
      userEmail: user.email,
      userRole: user.role,
  ...
  // Application actions
  applicationApprove: (user, application, req) =>
    logAction({
      userId: user._id || user.id,
      userEmail: user.email,
      userRole: user.role,
  ...
  applicationReject: (user, application, reason, req) =>
    logAction({
      userId: user._id || user.id,
      userEmail: user.email,
      userRole: user.role,
  ...
  // Interview actions
  interviewSchedule: (user, interview, req) =>
    logAction({
      userId: user._id || user.id,
      userEmail: user.email,
      userRole: user.role,
  ...
  offerSend: (user, applicant, jobTitle, req) =>
    logAction({
      userId: user._id || user.id,
      userEmail: user.email,
      userRole: user.role,
  ...
  // Admin actions
  settingsUpdate: (user, settingName, previousValue, newValue, req) =>
    logAction({
      userId: user._id || user.id,
      userEmail: user.email,
      userRole: user.role,
  ...
  subscriptionChange: (user, company, previousPlan, newPlan, req) =>
    logAction({
      userId: user._id || user.id,
      userEmail: user.email,
      userRole: user.role,
  ...
  // AI actions
  aiReportGenerate: (user, reportType, targetId, req) =>
    logAction({
      userId: user._id || user.id,
      userEmail: user.email,
      userRole: user.role,
...
export const getAuditLogs = async (filters = {}, options = {}) => {
  const { userId, action, targetType, startDate, endDate, status } = filters;
