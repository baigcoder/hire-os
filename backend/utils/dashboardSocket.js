...
export const initializeDashboardSockets = (io) => {
  io.on("connection", (socket) => {
    ...
    socket.on("join-ceo-dashboard", async (data) => {
      try {
        const { companyId } =
          typeof data === "string" ? { companyId: data } : data;
        ...
        if (
          socket.user?.role !== "company_admin" &&
          socket.user?.role !== "recruiter"
        ) {
          socket.emit("error", {
            message: "Unauthorized - CEO/Recruiter access only",
          });
          return;
        }
        ...
        console.log(
          `📊 [Dashboard] ${socket.user?.fullname || "User"} joined company dashboard: ${companyId}`,
        );
      } catch (error) {
    ...
    socket.on("subscribe-dashboard", async (data) => {
      try {
        ...
        console.log(
          `📊 [Student Dashboard] User ${userId} subscribed to real-time updates`,
        );
      } catch (error) {
    ...
    socket.on("request-stats-refresh", (data) => {
      ...
      console.log(
        `📊 [Student Dashboard] Stats refresh requested for ${userId}`,
      );
    });
  });
...
const handleLeaveStudentDashboard = (socket) => {
  ...
  console.log(
    `📊 [Student Dashboard] User ${userId} unsubscribed from updates`,
  );
};
