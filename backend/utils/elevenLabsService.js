...
export const textToSpeech = async (text, options = {}) => {
  ...
  console.log(
    `🎤 [ElevenLabs] Generating speech: ${charCount} chars, lang=${language}, model=${model}`,
  );
  console.log(
    `   Settings: stability=${stability.toFixed(2)}, similarity=${similarityBoost.toFixed(2)}, style=${styleValue.toFixed(2)}`,
  );
  ...
  try {
    ...
    console.log(
      `✅ [ElevenLabs] Generated ${(audioBuffer.byteLength / 1024).toFixed(1)}KB audio`,
    );
...
export const textToSpeechStream = async (text, options = {}) => {
  ...
  console.log(
    `🎤 [ElevenLabs] Streaming speech: ${text.length} chars, lang=${language}`,
  );
...
export const getSubscriptionInfo = async () => {
  ...
  try {
    ...
    const data = await response.json();
    console.log(
      `📊 [ElevenLabs] Character limit: ${data.character_limit}, Used: ${data.character_count}`,
    );
