...
export const prepareResumeText = (resumeData) => {
    ...
    if (resumeData.rawText) parts.push(`Full Resume: ${resumeData.rawText.substring(0, 8000)}`);
...
export const prepareJobText = (jobData) => {
    ...
    if (jobData.requirements?.length) parts.push(`Requirements: ${jobData.requirements.join(', ')}`);
...
export const prepareInterviewText = (interviewData) => {
    ...
    if (interviewData.candidateName) parts.push(`Candidate: ${interviewData.candidateName}`);
