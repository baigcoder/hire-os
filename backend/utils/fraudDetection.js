...
export const generateFraudReport = async (interviewData, behaviorData) => {
    ...
    if (riskAnalysis.riskLevel === 'critical') {
        summary = 'High-risk behavior detected during interview. Multiple indicators suggest potential integrity issues.';
        recommendation = 'REVIEW REQUIRED: Manual review strongly recommended before proceeding.';
    } else if (riskAnalysis.riskLevel === 'high') {
        summary = 'Several concerning behaviors detected. The candidate showed signs of potential external assistance.';
        recommendation = 'CAUTION: Consider follow-up verification or technical interview.';
    } else if (riskAnalysis.riskLevel === 'medium') {
        summary = 'Some minor anomalies detected, but within acceptable limits for online testing.';
        recommendation = 'PROCEED: Minor concerns noted, but no significant red flags.';
    } else {
        summary = 'No significant integrity concerns. Candidate behavior appears genuine throughout.';
...
export const quickRiskCheck = (behaviorData) => {
    const { tabSwitches = 0, copyPasteAttempts = 0, rapidAnswers = 0, totalQuestions = 0 } = behaviorData;
    ...
    if (totalQuestions > 0 && rapidAnswers / totalQuestions > 0.5) return 'medium';
