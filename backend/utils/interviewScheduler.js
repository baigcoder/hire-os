...
export const checkAndSendReminders = async () => {
  try {
    ...
    console.log(
      `📅 Checking ${upcomingInterviews.length} upcoming interviews for reminders...`,
    );
    ...
    for (const interview of upcomingInterviews) {
      ...
      if (
        timeUntilInterview <= REMINDER_15MIN &&
        !sentTypes.includes("15min")
      ) {
        reminderType = "15min";
      } else if (
        timeUntilInterview <= REMINDER_1H &&
        timeUntilInterview > REMINDER_15MIN &&
        !sentTypes.includes("1h")
      ) {
        reminderType = "1h";
      } else if (
        timeUntilInterview <= REMINDER_24H &&
        timeUntilInterview > REMINDER_1H &&
  ...
  try {
    ...
    const interviewDetails = {
      ...
      meetingLink:
        ...
        `${process.env.FRONTEND_URL}/interview/live/${interview._id}`,
      ...
      title:
        reminderType === "15min"
          ...
          : reminderType === "1h"
            ? "📅 Interview in 1 Hour"
            : "📅 Interview Tomorrow",
      message: `Your interview for ${job?.title} is ${
        reminderType === "15min"
    ...
    console.log(
      `✅ Sent ${reminderType} reminder for interview ${interview._id}`,
    );
  } catch (error) {
    console.error(
      `❌ Failed to send reminder for interview ${interview._id}:`,
      error,
    );
  }
};
