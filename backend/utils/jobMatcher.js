...
export async function calculateJobMatch(student, job, options = {}) {
    try {
        ...
        const jobRequirements = {
            ...
            isRemote: !!job.isRemote || (job.jobType || '').toLowerCase() === 'remote'
        };
        ...
        if (useAI && quickScore.total >= 30) {
            console.log(`   📊 [RAG] Quick score: ${quickScore.total}/100 - Running RAG analysis...`);
            ...
            const aiAnalysis = await getAIMatchAnalysisWithRAG(studentProfile, jobRequirements, ragContext);

            const latency = Date.now() - startTime;
            const aiScore = typeof aiAnalysis.matchScore === 'number' ? aiAnalysis.matchScore : null;
            const finalScore = aiScore === null
                ? quickScore.total
                : Math.round((quickScore.total * 0.35) + (aiScore * 0.65));
            console.log(`   ✅ [RAG] Match complete: ${finalScore}/100 (${latency}ms)`);
        ...
        console.log(`   ⚡ [QUICK] Low match: ${quickScore.total}/100 - Skipping RAG`);
        return {
            ...quickScore,
            jobId: job._id,
            studentId: student._id,
...
function calculateQuickMatch(student, job) {
    ...
    const skillsRatio = jobSkills.length > 0 ? matchedSkills.length / jobSkills.length : 0.45;
    ...
    if (jobYears === null) {
        ...
        breakdown.experience = Math.round(Math.min(20, (studentYears / jobYears) * 20));
    }
    ...
    const jobRemote = job.isRemote || jobLocation.includes('remote') || jobLocation.includes('work from home');
    if (jobRemote) {
        breakdown.location = 10;
    } else if (studentLocation && jobLocation && (jobLocation.includes(studentLocation) || studentLocation.includes(jobLocation))) {
        breakdown.location = 10;
    } else {
    ...
    const profileScore = (
        ...
        (student.bio ? 2 : 0) +
        (Array.isArray(student.experience) && student.experience.length > 0 ? 1 : 0)
    );
    score += profileScore;
...
async function getAIMatchAnalysisWithRAG(student, job, ragContext) {
    try {
        ...
        const ragInsights = ragContext.similarCandidates.length > 0
            ? `\nSIMILAR CANDIDATES HIRED FOR THIS ROLE:\n${ragContext.similarCandidates.slice(0, 3).map((c, i) =>
...
Return ONLY valid JSON:
{
    ...
    "confidenceLevel": "<High|Medium|Low>"
        ...
        console.log(`   ✅ [AI] Response from ${result.model} (${result.latency}ms)`);
        ...
        console.log('   ⚠️ [AI] Analysis failed, using Gemini fallback:', error.message);
...
export async function getMatchedJobsForStudent(studentId, options = {}) {
    try {
        ...
        if (!options.location) {
            const locations = Array.isArray(student.jobPreferences?.locations) ? student.jobPreferences.locations : [];
        ...
        if (!options.jobType) {
            const jobTypes = Array.isArray(student.jobPreferences?.jobTypes) ? student.jobPreferences.jobTypes : [];
            if (jobTypes.length > 0) {
                preferredQuery.jobType = { $in: jobTypes };
            }
        }

        const industries = Array.isArray(student.jobPreferences?.industries) ? student.jobPreferences.industries : [];
        ...
        if (options.location) preferredQuery.location = new RegExp(options.location, 'i');
...
function buildCandidateText(student) {
    ...
    const exp = Array.isArray(student.profile?.experience) ? student.profile.experience : [];
    const edu = Array.isArray(student.profile?.education) ? student.profile.education : [];

    const expLines = exp.map((e) => {
        const parts = [e.title, e.company, e.location, e.description].filter(Boolean);
        return parts.join(' | ');
    }).filter(Boolean);

    const eduLines = edu.map((e) => {
        const parts = [e.school, e.degree, e.fieldOfStudy, e.description].filter(Boolean);
        return parts.join(' | ');
    }).filter(Boolean);
...
function getStudentEducationRank(education) {
    ...
    for (const e of education) {
        const degree = (e?.degree || e?.fieldOfStudy || '').toString().toLowerCase();
        const r =
            degree.includes('phd') || degree.includes('doctor') ? 4
                : degree.includes('master') || degree.includes('msc') || degree.includes('mba') ? 3
                    : degree.includes('bachelor') || degree.includes('bsc') || degree.includes('bs') || degree.includes('be') || degree.includes('b.e') ? 2
                        : degree.includes('associate') || degree.includes('diploma') ? 1
                            : degree.includes('high school') || degree.includes('secondary') ? 0
                                : null;
        if (r === null) continue;
...
function calculateIndustryScore(student, job) {
    ...
    const prefs = Array.isArray(student.preferences?.industries) ? student.preferences.industries : [];
    const prefHit = prefs.some((p) => normalizeIndustry(p) && jobIndustry.includes(normalizeIndustry(p)));
    if (prefHit) return 5;
...
function extractSkillsFromText(text) {
    ...
    for (const skill of known) {
        ...
        const rx = new RegExp(`\\b${normalizedNeedle.replace(/\s+/g, '\\s+')}\\b`, 'i');
...
function estimateExperienceYears(experience) {
    ...
    for (const item of experience) {
        ...
        if (from instanceof Date && !Number.isNaN(from.valueOf()) && end instanceof Date && !Number.isNaN(end.valueOf()) && end >= from) {
            months += Math.max(1, Math.round((end - from) / (1000 * 60 * 60 * 24 * 30)));
        } else {
            months += 6;
        }
    }
