 ...
 */
export const analyzeResumeWithRAG = async (
  resumeText,
  jobDescription = "",
  candidateId = null,
) => {
 ...
 */
export const findJobsWithRAG = async (
  resumeText,
...
export const generateCEOReportWithRAG = async (interviewData) => {
  ...
  const ragContext = await getCEOReportContext(
...
Past similar hires:
${ragContext.pastHires
  .slice(0, 3)
  .map(
    (h, i) => `${i + 1}. ${h.candidateName} for ${h.jobTitle} - ${h.outcome}`,
  )
  .join("\n")}
`
 ...
 */
export const getInterviewResponseWithMemory = async (
  question,
  candidateAnswer,
  interviewId,
  jobContext = "",
  ...
  const previousQA = memory.messages.filter(
