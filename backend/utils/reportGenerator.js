...
export const generateInterviewReport = async (interviewData) => {
  ...
  const mcqPercentage =
    mcqTotal > 0 ? Math.round((mcqScore / mcqTotal) * 100) : null;
  ...
  if (GPT5_API_KEY) {
    try {
      ...
      console.log(
        "GPT-5 report generation failed, trying Gemini:",
        error.message,
      );
    }
  }
...
const generateFallbackReport = (interviewData) => {
  ...
  const mcqPercentage =
    mcqTotal > 0 ? Math.round((mcqScore / mcqTotal) * 100) : 0;
  ...
  if (mcqPercentage) {
    overallScore += mcqPercentage * 0.4;
    factors++;
  }
  if (resumeScore) {
    overallScore += resumeScore * 0.3;
    factors++;
  }
  if (videoScores?.overall) {
    overallScore += videoScores.overall * 0.3;
    factors++;
  }
  ...
  return {
    summary: `Candidate ${candidateName} completed the interview process for ${jobTitle}. ${
      overallScore >= 70
    ...
    nextSteps:
      recommendation === "STRONGLY_RECOMMEND" || recommendation === "RECOMMEND"
 ...
 */
export const processCEODecision = async (
  reportId,
  decision,
  ceoNotes,
  decidedBy,
