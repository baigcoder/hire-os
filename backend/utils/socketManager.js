...
export const initializeSocketManager = async (httpServer, allowedOrigins) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-interview", async (data) => {
      try {
        const { interviewId, role = "candidate" } =
          typeof data === "string" ? { interviewId: data } : data;
        ...
        if (room.state === ROOM_STATES.WAITING) {
          const hasRecruiter = Array.from(room.participants.values()).some(
            (p) => p.role === ROLES.RECRUITER,
          );
          const hasCandidate = Array.from(room.participants.values()).some(
            (p) => p.role === ROLES.CANDIDATE,
          );
        ...
        console.log(
          `👤 [${role}] ${participant.fullname} joined interview: ${interviewId}`,
        );
      } catch (error) {
    ...
    socket.on("send-message", ({ interviewId, message, type = "text" }) => {
      ...
      console.log(
        `💬 [${interviewId}] ${chatMessage.senderName}: ${message.slice(0, 50)}...`,
      );
    });
    ...
    socket.on(
      "media-state-change",
      ({ interviewId, isAudioOn, isVideoOn, isScreenSharing }) => {
        const room = activeRooms.get(interviewId);
    ...
    socket.on("fraud-alert", ({ interviewId, type, details }) => {
      ...
      const recruiterParticipants = Array.from(
  ...
  console.log(
    "🚀 Socket Manager initialized" + (redisEnabled ? " with Redis" : ""),
  );
  return io;
};
