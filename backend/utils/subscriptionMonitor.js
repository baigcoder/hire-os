...
import { sendTrialExpiryEmail, sendSubscriptionExpiryEmail, sendTrialExpiredEmail } from './email.js';
...
class SubscriptionMonitor {
            ...
            for (const student of expiredStudents) {
                try {
                    ...
                    try {
                        ...
                        console.error(`Failed to send trial expired email to ${student.email}:`, emailError);
                    }
    ...
    async checkCompanySubscriptions() {
        try {
            ...
            console.log(`📋 Found ${expiredCompanies.length} expired company subscriptions`);
                ...
                try {
                    ...
                    if (company.adminUser?.email) {
                        try {
                            ...
                            console.error(`Failed to send expiry email to ${company.adminUser.email}:`, emailError);
                        }
                    }
    ...
    async sendExpiryReminders() {
        try {
            ...
            for (const student of studentsNearing7Days) {
                const daysLeft = Math.ceil((student.trialEndDate - now) / (1000 * 60 * 60 * 24));
                try {
            ...
            for (const company of companiesNearing7Days) {
                if (company.adminUser?.email) {
                    const daysLeft = Math.ceil((company.subscription.endDate - now) / (1000 * 60 * 60 * 24));
                    try {
                        ...
                        console.error(`Failed to send reminder to ${company.adminUser.email}:`, error);
                    }
                }
            }

            console.log(`📧 Sent expiry reminders: ${studentsNearing7Days.length} students, ${companiesNearing7Days.length} companies`);
        } catch (error) {
