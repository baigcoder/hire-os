  ...
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      const pythonScript = path.resolve(
        __dirname,
        "../../Models/AI-powered-Resume-Screening-and-Ranking-System/app.py",
      );
...
const runAIAnalysis = async (
  ...
  const startTime = Date.now();
  console.log(
    "🤖 [AI Engine] Starting industrial-standard GPT-4o-mini analysis...",
  );

  try {
    const { candidateName, candidateRole, candidateEmail } =
      candidateContext || {};
...

{
    ...
    console.log(
      `✅ [AI Engine] Analysis complete | Score: ${analysis.score} | Model: ${result.model}`,
    );
 ...
 */
export const analyzeResumeUnified = async (
  resumeText,
  jobDescription,
  options = {},
) => {
  console.log(
    "🚀 [Resume UNIFIED] Starting industrial-standard 3-engine analysis...",
  );
  console.log(
    `📄 [Resume UNIFIED] Resume: ${resumeText.length} chars | Job: ${jobDescription.length} chars`,
  );
  ...
  if (includeEngines.includes("javascript-regex")) {
    ...
    enginePromises.push(
      runJavaScriptAnalysis(resumeText, jobDescription).then((r) => {
        notifyProgress("javascript-regex", "completed", r);
        return r;
      }),
    );
  }
  ...
  if (includeEngines.includes("python-ml")) {
    ...
    enginePromises.push(
      runPythonML(resumeText, jobDescription).then((r) => {
        notifyProgress("python-ml", "completed", r);
        return r;
      }),
    );
  }
  ...
  if (includeEngines.includes("gpt-4o-mini")) {
    ...
    enginePromises.push(
      runAIAnalysis(resumeText, jobDescription, {
        ...
        notifyProgress("gpt-4o-mini", "completed", r);
        return r;
      }),
    );
  }
  ...
  if (scores.length > 0) {
    ...
    const combinedMatchedSkills = [
      ...(Array.isArray(mergedData.keySkillsMatch)
        ? mergedData.keySkillsMatch
        : []),
      ...aiMatchedSkills,
    ...
    const combinedMissingSkills = [
      ...(Array.isArray(mergedData.missingSkills)
        ? mergedData.missingSkills
        : []),
      ...aiMissingSkills,
    ].filter((value, index, self) => value && self.indexOf(value) === index);
    ...
    if (
      !mergedData.experienceYears &&
      aiData.experienceAnalysis?.totalYears != null
    ) {
    ...
    if (
      Array.isArray(aiData.detailedSuggestions) &&
      aiData.detailedSuggestions.length > 0
    ) {
    ...
    if (
      Array.isArray(aiData.atsOptimizationTips) &&
      aiData.atsOptimizationTips.length > 0
    ) {
    ...
    if (
      Array.isArray(aiData.learningResources) &&
      aiData.learningResources.length > 0
    ) {
    ...
    if (
      Array.isArray(aiData.interviewQuestions) &&
      aiData.interviewQuestions.length > 0
    ) {
    ...
    const scoreVariance =
      scores.length > 1
        ? Math.sqrt(
            scores.reduce(
              (sum, s) => sum + Math.pow(s - consensusScore, 2),
              0,
            ) / scores.length,
          )
        : 0;
    ...
    results.consensus = {
      ...
      engineScores: successfulEngines.map((e) => ({
        engine: e.engine,
        score: e.score,
      })),
    };
 ...
 */
export const analyzeResumeStreaming = async (
  resumeText,
  jobDescription,
  socket,
  eventName = "analysis_progress",
) => {
