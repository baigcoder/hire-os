...
export const rankApplicantsUnified = async (applicants, job, options = {}) => {
  ...
  for (
    let i = 0;
    i < applicants.length && rankedApplicants.length < limit;
    i++
  ) {
    ...
    if (jobEmbedding && applicant.resumeText) {
      try {
        ...
        scores.semantic = Math.round(
          cosineSimilarity(jobEmbedding, resumeEmbedding) * 100,
        );
      } catch (e) {
        scores.semantic = 50;
      }
    ...
    rankedApplicants.push({
      ...
      enginesUsed: Object.keys(scores).filter(
  ...
  return {
    ...
    summary: {
      ...
      averageScore: Math.round(
        rankedApplicants.reduce((sum, a) => sum + a.scores.final, 0) /
...
const getRecommendation = (score, scores) => {
  ...
  if (score >= 50) {
    ...
    if (scores.experience < 50)
      return "Less experienced - consider for junior role";
