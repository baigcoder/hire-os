...
const keywordMatch = (resumeSkills, jobRequirements) => {
  ...
  const matched = resumeLower.filter((s) =>
    jobLower.some((j) => j.includes(s) || s.includes(j)),
  );
  const matchRate =
    jobLower.length > 0 ? (matched.length / jobLower.length) * 100 : 50;
    ...
    const batchResults = await Promise.all(
      batch.map(async (job) => {
        ...
        const preliminaryScore =
          (keywordResult.matchRate + semanticResult.similarity) / 2;
        ...
        const weights = { keyword: 0.25, semantic: 0.35, ai: 0.4 };
        let weightedSum = 0;
        let totalWeight = 0;
