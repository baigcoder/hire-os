...
export const getResumeAnalysisContext = async (resumeText, jobDescription) => {
    ...
    if (context.similarCandidates.length > 0) {
        const avgScore = context.similarCandidates.reduce((s, c) => s + c.score, 0) / context.similarCandidates.length;
        context.insights.push(`This candidate is ${Math.round(avgScore * 100)}% similar to ${context.similarCandidates.length} past candidates`);
    }
...
export const getCEOReportContext = async (jobTitle, candidateName) => {
    ...
    const queryEmbedding = await generateEmbedding(`${jobTitle} interview hiring`);
    ...
    if (hired.length > 0) {
        context.benchmarks.hireRate = Math.round((hired.length / context.pastHires.length) * 100);
        context.benchmarks.avgScore = Math.round(hired.reduce((s, h) => s + (h.score || 0), 0) / hired.length * 100);
    }
...
export const getVectorStoreStats = () => ({
    ...
    totalVectors: vectorStores.resumes.size + vectorStores.jobs.size + vectorStores.interviews.size,
 ...
 */
export const pruneOldVectors = (ttlHours = 168) => { // Default 7 days
