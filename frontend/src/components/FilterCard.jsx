...
import { MapPin, Briefcase, DollarSign, Clock, X, RotateCcw, ChevronDown, Filter } from 'lucide-react'
...
const filterData = [
    {
        filterType: "Location",
        icon: MapPin,
        options: ["Remote", "Delhi NCR", "Bangalore", "Hyderabad", "Pune", "Mumbai", "Chennai"]
    },
    {
        filterType: "Job Type",
        icon: Clock,
    ...
    },
    {
        filterType: "Industry",
        icon: Briefcase,
        options: ["Technology", "Finance", "Healthcare", "Marketing", "Design", "Sales"]
    },
    {
                ...
                {filterData.map((data, index) => {
                    ...
                    return (
                        <div key={index} className="p-4">
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <Icon size={14} className="text-gray-500 group-hover:text-[#FFD700] transition-colors" />
                                ...
                                {isExpanded && (
                                    ...
                                    >
                                        {data.options.map((option, idx) => {
                                            const isSelected = (selectedFilters[data.filterType] || []).includes(option);
                                            return (
                                                <label
                                                    key={idx}
                                                ...
                                                >
                                                    <div
                                                        className={`w-4 h-4 rounded-sm border flex items-center justify-center transition-all ${isSelected
                                                    ...
                                                    <input
                                                        type="checkbox"
                                                        checked={isSelected}
                                                        onChange={() => handleFilterChange(data.filterType, option)}
                                                        className="sr-only"
                                                    />
                                                    <span className={`text-xs font-mono transition-colors ${isSelected
                                                            ? 'text-white'
