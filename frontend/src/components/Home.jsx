...
import {
    Briefcase, Building, Users, Check, X, ChevronRight, Play,
    Zap, Sparkles, Target, BarChart3, Star, Crown, ArrowRight,
    Rocket, CheckCircle2, Shield, Video, Brain, FileText,
    Bell, CreditCard, Globe, Award, TrendingUp, Clock, Eye,
    UserPlus, Lock, Unlock, Gift, Menu, Search, MapPin, Terminal,
    Activity, Cpu, Radio, User, MessageSquare, Send, Loader2
} from 'lucide-react';
...
const DataCounter = ({ target, suffix = '', prefix = '', label }) => {
    ...
    return (
        <div className="text-center">
            ...
            <div className="text-xs text-gray-500 uppercase tracking-widest mt-2 font-medium">{label}</div>
        </div>
    );
};

// Industrial Feature Card with HUD aesthetic
const IndustrialFeatureCard = ({ icon: Icon, title, description, isPro = false, index = 0 }) => (
    <motion.div
        ...
        className={`relative p-6 rounded-md border transition-all duration-300 group cursor-default overflow-hidden ${isPro
            ? 'bg-[#FFD700]/5 border-[#FFD700]/20 hover:border-[#FFD700]/50'
            : 'bg-[#111111] border-white/10 hover:border-white/20'
            }`}
    >
        {/* Corner accent */}
        <div className={`absolute top-0 right-0 w-8 h-8 border-t border-r ${isPro ? 'border-[#FFD700]/40' : 'border-white/10'}`} />
        <div className={`absolute bottom-0 left-0 w-8 h-8 border-b border-l ${isPro ? 'border-[#FFD700]/40' : 'border-white/10'}`} />
        ...
        <div className={`w-10 h-10 rounded-sm flex items-center justify-center mb-4 transition-all duration-300 ${isPro
            ? 'bg-[#FFD700] text-black'
            : 'bg-white/5 text-[#FFD700] group-hover:bg-[#FFD700]/10'
            }`}>
            <Icon className="w-5 h-5" strokeWidth={2} />
        </div>
        <h3 className="text-lg font-bold text-white mb-2 font-['Space_Grotesk',sans-serif] group-hover:text-[#FFD700] transition-colors">{title}</h3>
...
const LiveIndicator = () => (
    <div className="flex items-center gap-2 px-3 py-1.5 bg-[#00FF94]/10 border border-[#00FF94]/20 rounded-sm">
        ...
        <span className="text-[#00FF94] text-xs font-mono uppercase tracking-wider">System Online</span>
    </div>
);
...
const Home = () => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                ...
                if (response.data.success && response.data.raw) {
                    const { activeJobs, companies, candidates, successRate } = response.data.raw;
                    ...
                    setStats({
                        ...
                        successRate: Math.min(BASE_STATS.successRate + Math.floor(successRate / 10), 99) // Cap at 99%
                    });
                    setIsLive(true);
                }
            } catch (error) {
        ...
        const fetchReviews = async () => {
            try {
                const response = await axios.get(`${STATS_API_END_POINT}/reviews?limit=6`);
    ...
    const fetchFeaturedCompanies = async () => {
        try {
            const response = await axios.get(`${COMPANY_API_END_POINT}/featured?limit=6`);
            if (response.data.success) {
    ...
    const handleSubmitComment = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                // Refresh reviews
                const reviewsRes = await axios.get(`${STATS_API_END_POINT}/reviews?limit=6`);
        ...
        <div className="min-h-screen bg-[#0A0A0A] text-white overflow-x-hidden font-['Space_Grotesk',sans-serif]">
                        ...
                        >
                            ...
                            <div className="flex items-center justify-center gap-3 mb-4">
                                <div className="flex items-center gap-2 px-4 py-2 bg-[#00FF94]/10 border border-[#00FF94]/30 rounded-sm">
                                    ...
                                    <span className="text-[#00FF94] text-xs font-mono uppercase tracking-wider">Live Monitoring</span>
                                </div>
                                <span className="text-gray-600 text-[10px] font-mono">Auto-updating every 30s</span>
                            </div>

                            {/* Stats Grid */}
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-6 bg-[#111111]/80 border border-white/5 rounded-md">
                                <DataCounter target={stats.activeJobs} suffix="+" label="Active Jobs" />
                                <DataCounter target={stats.companies} suffix="+" label="Companies" />
                                <DataCounter target={stats.candidates} suffix="+" label="Candidates" />
                                <DataCounter target={stats.successRate} suffix="%" label="Success Rate" />
                            </div>
                        </motion.div>
                    </div>
                </div>
            ...
            <section className="py-20 bg-gradient-to-b from-[#0A0A0A] to-[#111111] relative overflow-hidden">
                ...
                <div className="container mx-auto px-4 relative z-10">
                    ...
                    >
                        <Link to="/features">
                            <Button variant="outline" className="px-6 py-4 text-sm font-bold border-white/20 text-white hover:bg-white/5 rounded-sm uppercase tracking-wider">
            ...
            <section className="py-24 bg-[#0A0A0A] relative">
                ...
                <div className="container mx-auto px-4 relative z-10">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-6xl mx-auto">
                        <IndustrialFeatureCard icon={FileText} title="AI Resume Analysis" description="Instant feedback on ATS compatibility and optimization suggestions." index={0} />
                        <IndustrialFeatureCard icon={Target} title="Precision Matching" description="ML algorithms connect your profile with relevant opportunities." index={1} />
                        <IndustrialFeatureCard icon={Bell} title="Real-time Alerts" description="Instant notifications when matching positions are posted." index={2} />
                        <IndustrialFeatureCard icon={Eye} title="Application Tracker" description="Monitor application status and employer engagement." index={3} />
                        <IndustrialFeatureCard icon={Briefcase} title="One-Click Apply" description="Streamlined application process across multiple listings." index={4} />
                        <IndustrialFeatureCard icon={TrendingUp} title="Market Intelligence" description="Salary benchmarks and industry trend analysis." index={5} />
                    </div>
                </div>
            </section>
            ...
            <section className="py-24 bg-gradient-to-b from-[#0A0A0A] via-[#111111] to-[#0A0A0A] relative overflow-hidden">
                ...
                <div className="container mx-auto px-4 relative z-10">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-6xl mx-auto">
                        <IndustrialFeatureCard icon={Video} title="Live Video Interviews" description="HD video conferencing with recording and AI transcription." isPro index={0} />
                        <IndustrialFeatureCard icon={Brain} title="MCQ Assessment Engine" description="Custom test creation with auto-grading and analytics." isPro index={1} />
                        <IndustrialFeatureCard icon={Shield} title="Fraud Detection System" description="Behavioral analysis and proctoring for test integrity." isPro index={2} />
                        <IndustrialFeatureCard icon={BarChart3} title="Analytics Dashboard" description="Real-time pipeline metrics and conversion tracking." isPro index={3} />
                        <IndustrialFeatureCard icon={Users} title="Team Collaboration" description="Multi-recruiter access with role-based permissions." isPro index={4} />
                        <IndustrialFeatureCard icon={Gift} title="Offer Management" description="Digital offer letters with e-signature integration." isPro index={5} />
                    </div>
            ...
            {featuredCompanies.length > 0 && (
                <section className="py-24 bg-[#0A0A0A] relative overflow-hidden">
                    ...
                    <div className="container mx-auto px-4 relative z-10">
                        ...
                        >
                            <Link to="/browse">
                                <Button variant="outline" className="px-6 py-4 text-xs font-bold border-white/20 text-gray-400 hover:border-[#00FF94]/50 hover:text-[#00FF94] rounded-sm uppercase tracking-wider">
                                    View All Companies
                    ...
                    <div className="grid lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
                        ...
                        >
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <h3 className="text-sm font-bold text-[#00FF94] uppercase tracking-wider">Candidate Protocol</h3>
                            </div>
                            <div className="space-y-4">
                                {[
                                    { step: "01", title: "Create Profile", desc: "Sign up free & complete your profile" },
                                    { step: "02", title: "Upload Resume", desc: "Get AI-powered resume analysis" },
                                    { step: "03", title: "Apply to Jobs", desc: "One-click apply to matching jobs" },
                                    { step: "04", title: "Take Assessments", desc: "Complete MCQ tests & interviews" },
                                    { step: "05", title: "Get Hired", desc: "Receive offer & accept position" }
                                ].map((item, idx) => (
                                    <div key={idx} className="flex items-start gap-3 p-3 bg-[#0A0A0A] border border-white/5 rounded-sm group hover:border-[#00FF94]/30 transition-colors">
                                        <span className="text-[10px] font-mono text-[#00FF94] bg-[#00FF94]/10 px-1.5 py-0.5 rounded-sm">{item.step}</span>
                                        <div>
                                            <h4 className="text-sm font-bold text-white group-hover:text-[#00FF94] transition-colors">{item.title}</h4>
                        ...
                        >
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <h3 className="text-sm font-bold text-[#FFD700] uppercase tracking-wider">Recruiter Protocol</h3>
                            </div>
                            <div className="space-y-4">
                                {[
                                    { step: "01", title: "Join Company", desc: "Get invited or request access" },
                                    { step: "02", title: "Post Jobs", desc: "Create detailed job listings" },
                                    { step: "03", title: "Screen Candidates", desc: "Review applications & resumes" },
                                    { step: "04", title: "Conduct Interviews", desc: "Video calls & MCQ assessments" },
                                    { step: "05", title: "Send Offers", desc: "Extend offers to top candidates" }
                                ].map((item, idx) => (
                                    <div key={idx} className="flex items-start gap-3 p-3 bg-[#0A0A0A] border border-white/5 rounded-sm group hover:border-[#FFD700]/30 transition-colors">
                                        <span className="text-[10px] font-mono text-[#FFD700] bg-[#FFD700]/10 px-1.5 py-0.5 rounded-sm">{item.step}</span>
                                        <div>
                                            <h4 className="text-sm font-bold text-white group-hover:text-[#FFD700] transition-colors">{item.title}</h4>
                                            <p className="text-xs text-gray-500">{item.desc}</p>
                                        </div>
                                    </div>
                                ))}
                        ...
                        >
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <h3 className="text-sm font-bold text-[#00BFFF] uppercase tracking-wider">Admin Protocol</h3>
                            </div>
                            <div className="space-y-4">
                                {[
                                    { step: "01", title: "Register Company", desc: "Setup organization & profile" },
                                    { step: "02", title: "Choose Plan", desc: "Select subscription & features" },
                                    { step: "03", title: "Invite Team", desc: "Add recruiters & set permissions" },
                                    { step: "04", title: "Configure Settings", desc: "Brand, workflows & integrations" },
                                    { step: "05", title: "Monitor & Grow", desc: "Analytics, billing & scaling" }
                                ].map((item, idx) => (
                                    <div key={idx} className="flex items-start gap-3 p-3 bg-[#0A0A0A] border border-white/5 rounded-sm group hover:border-[#00BFFF]/30 transition-colors">
                                        <span className="text-[10px] font-mono text-[#00BFFF] bg-[#00BFFF]/10 px-1.5 py-0.5 rounded-sm">{item.step}</span>
                                        <div>
                                            <h4 className="text-sm font-bold text-white group-hover:text-[#00BFFF] transition-colors">{item.title}</h4>
                                            <p className="text-xs text-gray-500">{item.desc}</p>
                                        </div>
                                    </div>
                                ))}
                ...
                <div className="container mx-auto px-4 relative z-10">
                    ...
                    {reviews.length > 0 ? (
                                ...
                                >
                                    ...
                                    <div className="flex gap-1 mb-4">
                                        {[...Array(5)].map((_, i) => (
                                            <Star
                                                key={i}
                                                className={`w-4 h-4 ${i < review.rating ? 'text-[#FFD700] fill-[#FFD700]' : 'text-gray-700'}`}
                                            />
                                        ))}
                                    </div>
                                    ...
                                    <p className="text-gray-400 text-sm mb-4 line-clamp-3">"{review.comment}"</p>

                                    {/* User Info */}
                                    <div className="flex items-center gap-3 pt-4 border-t border-white/5">
                                        <div className="w-10 h-10 rounded-sm bg-[#FFD700]/10 flex items-center justify-center overflow-hidden">
                                            {review.user?.photo ? (
                                        ...
                                        <div>
                                            ...
                                            <Badge className={`text-[10px] px-1.5 py-0 ${review.user?.role === 'company_admin'
                                                ? 'bg-[#FFD700]/20 text-[#FFD700]'
                                                : 'bg-[#00FF94]/20 text-[#00FF94]'
                                                }`}>
                                                {review.user?.role === 'company_admin' ? 'Company Owner' :
                                                    review.user?.role === 'recruiter' ? 'Recruiter' : 'Job Seeker'}
                                            </Badge>
                                        </div>
                                    </div>
                                </motion.div>
                        ...
                        <div className="text-center py-16 max-w-2xl mx-auto">
                            ...
                            <p className="text-gray-500 mb-6">Be the first to share your experience with HIRE.OS</p>
                        </div>
                    )}
                        ...
                        {showCommentForm && (
                            ...
                            >
                                ...
                                >
                                    ...
                                    {/* Star Rating */}
                                    <div className="mb-6">
                                        <label className="text-xs text-gray-500 uppercase tracking-wider mb-2 block font-mono">Rating</label>
                                    ...
                                    {/* Title (optional) */}
                                    <div className="mb-4">
                                        <label className="text-xs text-gray-500 uppercase tracking-wider mb-2 block font-mono">Title (optional)</label>
                                        <Input
                                            value={commentTitle}
                                    ...
                                    {/* Comment */}
                                    <div className="mb-6">
                                        <label className="text-xs text-gray-500 uppercase tracking-wider mb-2 block font-mono">Your Feedback *</label>
                                        <textarea
                                            value={commentText}
                                    ...
                                    <div className="flex items-center gap-3 mb-6 p-3 bg-white/5 rounded-sm border border-white/5">
                                        <div className="w-10 h-10 rounded-full bg-[#FFD700]/10 flex items-center justify-center">
                                            {user?.profile?.profilePhoto ? (
                                                <img src={user.profile.profilePhoto} alt={user.fullname} className="w-full h-full rounded-full object-cover" />
                                            ) : (
                                                <User className="w-5 h-5 text-[#FFD700]" />
                                            )}
                                        </div>
                ...
                <div className="container mx-auto px-4 relative z-10">
                    ...
                    >
                        ...
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#00FF94]/10 border border-[#00FF94]/30 rounded-sm mb-8">
                            ...
                            <span className="text-[#00FF94] text-sm font-bold">Students get 30 days FREE access to all features!</span>
                        </div>
