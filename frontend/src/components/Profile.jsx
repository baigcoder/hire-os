...
const Profile = () => {
    ...
    const stats = {
        ...
        reviewing: allAppliedJobs?.filter(j => ['under_review', 'shortlisted'].includes(j.status))?.length || 0,
    ...
    const StatCard = ({ icon: Icon, label, value, color }) => (
        <div className="p-4 rounded-md bg-[#111111] border border-white/10 hover:border-[#FFD700]/30 transition-all">
            <div className="flex items-center gap-3">
                <div className={`w-9 h-9 rounded-sm bg-white/5 flex items-center justify-center ${color}`}>
                    <Icon size={16} />
                </div>
                <div>
    ...
    return (
        <div className="min-h-screen bg-[#0A0A0A] text-white font-['Space_Grotesk',sans-serif]">
            ...
            <main className="max-w-6xl mx-auto px-4 py-8 relative z-10">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6 pt-6 border-t border-white/5">
                            ...
                            <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-sm font-medium text-gray-900 dark:text-white">{user?.phoneNumber || 'Not added'}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-sm font-medium text-gray-900 dark:text-white">{typeof user?.profile?.location === 'object' ? `${user?.profile?.location?.city || ''}${user?.profile?.location?.city && user?.profile?.location?.country ? ', ' : ''}${user?.profile?.location?.country || ''}` || 'Not specified' : user?.profile?.location || 'Not specified'}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                ...
                >
                    <StatCard icon={Briefcase} label="Total Applied" value={stats.total} color="bg-gray-600" bgColor="bg-white dark:bg-gray-800" />
                    <StatCard icon={Clock} label="Pending" value={stats.pending} color="bg-yellow-500" bgColor="bg-yellow-50 dark:bg-yellow-900/20" />
                    <StatCard icon={Target} label="In Review" value={stats.reviewing} color="bg-blue-500" bgColor="bg-blue-50 dark:bg-blue-900/20" />
                    <StatCard icon={TrendingUp} label="Interviews" value={stats.interviews} color="bg-purple-500" bgColor="bg-purple-50 dark:bg-purple-900/20" />
                    <StatCard icon={CheckCircle} label="Accepted" value={stats.accepted} color="bg-green-500" bgColor="bg-green-50 dark:bg-green-900/20" />
                    <StatCard icon={XCircle} label="Rejected" value={stats.rejected} color="bg-red-500" bgColor="bg-red-50 dark:bg-red-900/20" />
                </motion.div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {user?.profile?.skills?.length > 0 ? (
                                ...
                                <div className="text-center py-6 w-full">
                                    <Code size={32} className="mx-auto text-gray-300 dark:text-gray-600 mb-2" />
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400 mt-0.5">
                                Complete your profile to increase your chances of getting hired. Add your skills, experience, and education.
                                You'll upload your resume when applying to specific jobs.
                            </p>
                        </div>
                    </div>
                </motion.div>
