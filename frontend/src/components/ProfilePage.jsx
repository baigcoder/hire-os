...
import {
    ...
    ExternalLink, Shield, Calendar, Award, Target, TrendingUp,
    Clock, CheckCircle, XCircle, Pen, Camera, Save, Loader2,
    Code, Globe, Linkedin, Github, Twitter, Building2, Users,
    CreditCard, Settings, ChevronRight, Link2, Upload, X, Sparkles
} from 'lucide-react';
...
const ProfilePage = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        location: typeof user?.profile?.location === 'object' ? user?.profile?.location?.city || '' : user?.profile?.location || '',
        ...
        website: user?.profile?.socialLinks?.website || user?.profile?.socialLinks?.portfolio || ''
    });
    ...
    const stats = {
        ...
        interviews: allAppliedJobs?.filter(j => j.status === 'interview')?.length || 0,
        accepted: allAppliedJobs?.filter(j => ['hired', 'accepted'].includes(j.status))?.length || 0,
        rejected: allAppliedJobs?.filter(j => j.status === 'rejected')?.length || 0
    };
    ...
    const handlePhotoUpload = async (e) => {
        ...
        try {
            setLoading(true);
            const res = await axios.post(`${USER_API_END_POINT}/profile/photo`, form, {
    ...
    const calculateCompletion = () => {
        const fields = [
            ...
            user?.profile?.bio, user?.profile?.skills?.length,
            user?.profile?.resume, user?.profile?.profilePhoto
        ];
    ...
    const trialDaysRemaining = trialEnd ? Math.max(0, Math.ceil((trialEnd - new Date()) / (1000 * 60 * 60 * 24))) : null;
    ...
    return (
        <div className="min-h-screen bg-[#0A0A0A] text-white font-['Space_Grotesk',sans-serif] selection:bg-[#FFD700]/30">
            ...
            <main className="max-w-6xl mx-auto px-4 py-8 pt-20 relative z-10">
                ...
                >
                    {/* Corner Accents */}
                    <div className="absolute top-0 right-0 w-8 h-8 border-t border-r" style={{ borderColor: `${currentRole.color}50` }} />
                    <div className="absolute bottom-0 left-0 w-8 h-8 border-b border-l" style={{ borderColor: `${currentRole.color}50` }} />

                    {/* Cover / Header */}
                    <div className="h-32 relative overflow-hidden"
                        style={{ background: `linear-gradient(135deg, ${currentRole.color}20, transparent)` }}>
                    ...
                    <div className="px-6 pb-6">
                        <div className="flex flex-col sm:flex-row sm:items-end gap-4 -mt-16">
                            ...
                            <div className="relative group">
                                {user?.profile?.profilePhoto ? (
                                    <Avatar className="w-28 h-28 border-2 shadow-lg" style={{ borderColor: `${currentRole.color}80` }}>
                                        <AvatarImage src={user.profile.profilePhoto} alt={user?.fullname} />
                                    </Avatar>
                                ) : (
                                    <div className="w-28 h-28 rounded-full border-2 flex items-center justify-center"
                                        style={{ borderColor: `${currentRole.color}80`, backgroundColor: currentRole.color }}>
                                        <span className="text-4xl font-bold text-black">{getInitials(user?.fullname)}</span>
                                    </div>
                                )}
                                ...
                                <div className="flex items-start justify-between">
                                    <div>
                                        <h1 className="text-2xl font-bold text-white flex items-center gap-3 uppercase tracking-tight">
                                            {user?.fullname || 'User'}
                                            <Badge className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5"
                                                style={{ backgroundColor: `${currentRole.color}20`, color: currentRole.color, borderColor: `${currentRole.color}50` }}>
                                                {currentRole.title}
                                            </Badge>
                                        </h1>
                                    ...
                                    <div className="hidden sm:flex gap-2">
                                        ...
                                        <Button
                                            ...
                                            style={{ backgroundColor: currentRole.color, color: '#000' }}
                                        >
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6 pt-6 border-t border-white/5">
                            {[
                                { icon: Mail, label: 'Email', value: user?.email, color: '#FFD700' },
                                { icon: Phone, label: 'Phone', value: user?.phoneNumber || 'Not added', color: '#00FF94' },
                                { icon: MapPin, label: 'Location', value: typeof user?.profile?.location === 'object' ? `${user?.profile?.location?.city || ''}${user?.profile?.location?.city && user?.profile?.location?.country ? ', ' : ''}${user?.profile?.location?.country || ''}` || 'Not set' : user?.profile?.location || 'Not set', color: '#00BFFF' },
                                { icon: Calendar, label: 'Member Since', value: formatDate(user?.createdAt), color: '#FF6B6B' }
                            ].map((item, idx) => (
                                <div key={idx} className="flex items-center gap-3 p-3 bg-white/5 rounded-sm border border-white/5 hover:border-white/10 transition-colors">
                                    <div className="w-8 h-8 rounded-sm flex items-center justify-center" style={{ backgroundColor: `${item.color}15` }}>
                        ...
                        <div className="mt-6 p-4 bg-white/5 rounded-sm border border-white/5">
                            <div className="flex justify-between mb-2 text-xs">
                                <span className="text-gray-500 uppercase tracking-wider">Profile Completion</span>
                                <span className="font-mono" style={{ color: currentRole.color }}>{calculateCompletion()}%</span>
                            </div>
                            <Progress value={calculateCompletion()} className="h-1.5 bg-white/10" />
                        </div>
                    </div>
                </motion.div>
                    ...
                    {isEditing && (
                        ...
                        >
                            <div className="bg-[#111111] border border-white/10 rounded-md p-6 relative overflow-hidden">
                                <div className="absolute top-0 right-0 w-6 h-6 border-t border-r" style={{ borderColor: `${currentRole.color}30` }} />

                                <h3 className="text-sm font-bold uppercase tracking-wider mb-6 flex items-center gap-2" style={{ color: currentRole.color }}>
                                    <Pen size={14} />
                                    Edit Profile
                                </h3>
                                ...
                                <div className="grid md:grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wider mb-2 block">Bio</label>
                                            <textarea
                                                name="bio"
                                                value={formData.bio}
                                                onChange={handleInputChange}
                                        ...
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wider mb-2 block">Skills (comma separated)</label>
                                            <input
                                                name="skills"
                                ...
                                <div className="mt-6 pt-6 border-t border-white/5">
                                    <label className="text-xs text-gray-500 uppercase tracking-wider mb-4 block">Social Links</label>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                        {[
                                            ...
                                            { name: 'website', icon: Globe, placeholder: 'Website URL' }
                                        ].map((social) => (
                                ...
                                {/* Resume Upload */}
                                <div className="mt-6 pt-6 border-t border-white/5">
                                    <label className="text-xs text-gray-500 uppercase tracking-wider mb-4 block">Resume (PDF)</label>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0">
                                                <p className="text-xs text-gray-300 font-mono truncate max-w-[180px]">
                                                    {resumeName || user?.profile?.resumeOriginalName || 'No resume selected'}
                                                </p>
                ...
                {user?.role === 'student' && (
                    ...
                    >
                        {[
                            { icon: Briefcase, label: 'Total Applied', value: stats.total, color: '#FFD700' },
                            { icon: Clock, label: 'Pending', value: stats.pending, color: '#F59E0B' },
                            { icon: Target, label: 'Interviews', value: stats.interviews, color: '#8B5CF6' },
                            { icon: CheckCircle, label: 'Accepted', value: stats.accepted, color: '#00FF94' },
                            { icon: XCircle, label: 'Rejected', value: stats.rejected, color: '#EF4444' }
                        ].map((stat, idx) => (
                            <div key={idx} className="bg-[#111111] border border-white/10 rounded-sm p-4 hover:border-white/20 transition-all">
                                <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 rounded-sm flex items-center justify-center" style={{ backgroundColor: `${stat.color}15` }}>
                ...
                {user?.role === 'student' && user?.trialEndDate && activeSection === 'subscription' && (
                    <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-3 relative z-10">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-xs text-[#00FF94] font-bold uppercase tracking-wider">Free Trial</p>
                                    <p className="text-[11px] text-gray-400 font-mono">
                                        {trialEnd ? `Ends on ${trialEnd.toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}` : 'Trial active'}
                                    </p>
                                </div>
                            </div>
                            <Badge className="bg-[#00FF94]/20 text-[#00FF94] border-[#00FF94]/40 text-[10px] font-mono uppercase tracking-wider">
                                {user.subscriptionStatus === 'trial' ? 'ACTIVE' : (user.trialExpired ? 'EXPIRED' : 'TRIAL')}
                            </Badge>
                        </div>
                        <div className="flex items-center justify-between relative z-10">
                            ...
                            <div className="text-right">
                                <p className="text-[11px] text-gray-400 font-mono mb-1">Plan</p>
                                <p className="text-sm text-white font-semibold font-mono">HIRE.OS Trial Access</p>
                            </div>
                        </div>
                    </motion.div>
                )}
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {user?.profile?.skills?.length > 0 ? (
                                user.profile.skills.map((skill, idx) => (
                                    <Badge key={idx} className="bg-[#FFD700]/10 text-[#FFD700] border-[#FFD700]/30 text-xs">
                                        {skill}
                                    </Badge>
                                ))
                            ) : (
                    ...
                    >
                        ...
                        {user?.profile?.resume ? (
                            <div className="space-y-3">
                                <p className="text-gray-400 text-xs font-mono truncate">{user.profile.resumeOriginalName || 'resume.pdf'}</p>
                                <Button
                    ...
                    >
                        ...
                        <div className="space-y-2">
                            {[
                                { icon: Linkedin, url: user?.profile?.socialLinks?.linkedin, label: 'LinkedIn' },
                                { icon: Github, url: user?.profile?.socialLinks?.github, label: 'GitHub' },
                                { icon: Twitter, url: user?.profile?.socialLinks?.twitter, label: 'Twitter' },
                                {
                                    icon: Globe,
                                    url: user?.profile?.socialLinks?.website || user?.profile?.socialLinks?.portfolio,
                                    label: 'Website'
                                }
                                ...
                                >
                                    <social.icon size={14} className="text-gray-400 group-hover:text-[#00BFFF]" />
