...
import { CheckCircle, XCircle, Calendar, Clock, ArrowRight } from 'lucide-react';
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Monitor and manage candidate applications</CardDescription>
      </CardHeader>
      <CardContent>
              ...
              .map((app, index) => (
                <div key={index} className="flex items-center justify-between border-b pb-3 last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-medium">{app.user?.fullname || 'Applicant'}</h4>
                      <p className="text-sm text-gray-500">{app.job?.title || 'Job Position'}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-500 mt-1">{formatDate(app.updatedAt || app.createdAt)}</p>
                  </div>
                </div>
              ))
          ) : (
            <p className="text-gray-500 text-center py-4">No recent application activity</p>
          )}
        </div>
