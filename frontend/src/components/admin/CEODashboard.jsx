 ...
 * Now with real-time WebSocket updates and AI-generated insights
 */
...
import {
    Users, Briefcase, TrendingUp, TrendingDown, Clock, CheckCircle,
    XCircle, AlertTriangle, Award, Target, DollarSign, Calendar,
    ChevronRight, Building, Star, Zap, BarChart3, Eye, ThumbsUp,
    ThumbsDown, MessageSquare, FileText, Loader2, RefreshCw, Filter,
    Sparkles, Crown, ArrowUpRight, ArrowDownRight, Timer, UserCheck,
    Brain, Wifi, WifiOff
} from 'lucide-react';
...
const CEODashboard = () => {
    ...
    useEffect(() => {
        ...
        newSocket.on('connect', () => {
            setIsConnected(true);
            // Join CEO dashboard room
            if (user?.companyId) {
                newSocket.emit('join-ceo-dashboard', { companyId: user.companyId });
            }
        });
        ...
        newSocket.on('new-application', (data) => {
            toast.info(`New Application: ${data.data.applicantName} applied for ${data.data.jobTitle}`, {
                duration: 5000,
                icon: '📥'
            });
            // Refresh stats
        ...
        newSocket.on('new-approval', (data) => {
            toast.success(`Pending Approval: ${data.data.applicantName} for ${data.data.jobTitle}`, {
                duration: 5000,
                icon: '👑'
            });
    ...
    const fetchAIInsights = async () => {
        ...
        try {
            ...
            setAiInsights({
                executiveSummary: 'AI insights are currently unavailable. Check back later for hiring analytics.',
    ...
    const fetchExecutiveStats = async () => {
        try {
            ...
            const [jobsRes, statsRes] = await Promise.all([
                axios.get(`${API_BASE}/job/getadminjobs`, { withCredentials: true }).catch(() => ({ data: { jobs: [] } })),
                axios.get(`${API_BASE}/company/dashboard`, { withCredentials: true }).catch(() => ({ data: {} }))
            ]);
            ...
            const openJobs = jobs.filter(j => j.status !== 'closed' && j.isActive !== false).length;
            ...
            jobs.forEach(job => {
                ...
                if (!deptMap[dept]) deptMap[dept] = { name: dept, open: 0, filled: 0, pending: 0 };
    ...
    const fetchPendingApprovals = async () => {
        try {
            ...
            setPendingApprovals([
                {
                    _id: '1',
                    applicant: { fullname: 'John Smith', email: 'john@example.com', profile: { profilePhoto: '' } },
                    job: { title: 'Senior Software Engineer', company: { name: 'TechCorp' } },
                ...
                {
                    _id: '2',
                    applicant: { fullname: 'Sarah Johnson', email: 'sarah@example.com', profile: { profilePhoto: '' } },
    ...
    const fetchRecruiterPerformance = async () => {
        try {
            ...
            const perfRes = await axios.get(`${API_BASE}/company/recruiter-performance`, {
                withCredentials: true
            }).catch(() => null);
    ...
    const fetchHiringTrends = async () => {
        try {
            ...
            const res = await axios.get(`${API_BASE}/stats/applications`, {
                params: { period: '30d' },
                withCredentials: true
            }).catch(() => null);
            ...
            if (res?.data?.success && res.data.byDate) {
                ...
                const weeklyData = res.data.byDate.reduce((acc, day, idx) => {
                    ...
                    if (!acc[weekNum]) acc[weekNum] = { week: `W${weekNum + 1}`, applications: 0, interviews: 0, hires: 0 };
    ...
    if (loading) {
        return <DashboardLoader type="admin" message="LOADING EXECUTIVE DASHBOARD..." />;
    }
            ...
            <div className="max-w-[1800px] mx-auto px-6 py-8 relative z-10 pt-20">
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Badge className={`${isConnected ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' : 'bg-red-500/10 text-red-400 border-red-500/30'} border px-3 py-1.5`}>
                        ...
                        <Badge className="bg-[#FFD700]/10 text-[#FFD700] border border-[#FFD700]/30 px-3 py-1.5">
                    ...
                    {kpiCards.map((kpi, idx) => (
                        ...
                        >
                            <div className="flex items-center justify-between mb-3">
                                <div className={`p-2 rounded-sm ${kpi.bgColor} border ${kpi.borderColor}`}>
                                    <kpi.icon className={`w-4 h-4 ${kpi.color}`} />
                                </div>
                                {kpi.trend && (
                                    <span className={`text-[10px] font-mono flex items-center gap-0.5 ${kpi.trendUp ? 'text-emerald-400' : kpi.urgent ? 'text-[#FFD700]' : 'text-gray-500'
                                        }`}>
                            ...
                            <div className="text-[10px] text-gray-500 uppercase tracking-wider font-mono">{kpi.label}</div>
                        </motion.div>
                    ))}
                </div>
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                    <TabsList className="bg-[#111111] border border-white/10 p-1">
                        <TabsTrigger value="overview" className="data-[state=active]:bg-[#FFD700] data-[state=active]:text-black font-mono text-xs">
                            OVERVIEW
                        </TabsTrigger>
                        <TabsTrigger value="approvals" className="data-[state=active]:bg-[#FFD700] data-[state=active]:text-black font-mono text-xs">
                            APPROVALS ({pendingApprovals.length})
                        </TabsTrigger>
                        <TabsTrigger value="team" className="data-[state=active]:bg-[#FFD700] data-[state=active]:text-black font-mono text-xs">
                            TEAM PERFORMANCE
                        </TabsTrigger>
                    </TabsList>
                                    ...
                                    {departmentStats.map((dept, idx) => (
                                        <div key={dept.name} className="group">
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="text-sm text-gray-300 font-mono">{dept.name}</span>
                                                <div className="flex items-center gap-4 text-xs font-mono">
                                                    <span className="text-blue-400">{dept.open} Open</span>
                                                    <span className="text-emerald-400">{dept.filled} Filled</span>
                                                    <span className="text-amber-400">{dept.pending} Pending</span>
                                                </div>
                                            </div>
                                            <div className="flex gap-1 h-2">
                                                <div
                                                    className="bg-emerald-500/50 rounded-sm transition-all group-hover:bg-emerald-500"
                                                    style={{ width: `${(dept.filled / (dept.open + dept.filled + dept.pending)) * 100}%` }}
                                                />
                                                <div
                                                    className="bg-blue-500/50 rounded-sm transition-all group-hover:bg-blue-500"
                                                    style={{ width: `${(dept.open / (dept.open + dept.filled + dept.pending)) * 100}%` }}
                                                />
                                                <div
                                                    className="bg-amber-500/50 rounded-sm transition-all group-hover:bg-amber-500"
                                                    style={{ width: `${(dept.pending / (dept.open + dept.filled + dept.pending)) * 100}%` }}
                                                />
                                            </div>
                                        </div>
                                    ))}
                            ...
                            <div className="bg-[#111111] border border-white/10 rounded-sm p-6">
                                ...
                                {pendingApprovals.length === 0 ? (
                                    <div className="text-center py-8">
                                        ...
                                        <p className="text-gray-500 text-sm font-mono">All caught up!</p>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                                ...
                                                <div className="flex items-center justify-between mt-3 pt-2 border-t border-white/5">
                                                    ...
                                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <Button
                                                            ...
                                                            onClick={(e) => { e.stopPropagation(); handleApprovalDecision(approval._id, 'approved'); }}
                                                        >
                                                        ...
                                                        <Button
                                                            ...
                                                            onClick={(e) => { e.stopPropagation(); handleApprovalDecision(approval._id, 'rejected'); }}
                                                        >
                                        ...
                                        {pendingApprovals.length > 3 && (
                                            ...
                                            >
                                                View All ({pendingApprovals.length}) <ChevronRight className="w-4 h-4 ml-1" />
                                            </Button>
                                        )}
                                    </div>
                                )}
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-sm p-6">
                            ...
                            <div className="grid grid-cols-4 gap-4">
                                {hiringTrends.map((week, idx) => (
                                    <div key={week.week} className="text-center">
                                        <div className="text-xs text-gray-500 font-mono mb-3">{week.week}</div>
                                        <div className="space-y-2">
                                            <div className="h-24 flex items-end justify-center gap-1">
                                                <div
                                                    ...
                                                    style={{ height: `${(week.applications / 150) * 100}%` }}
                            ...
                            <div className="flex items-center justify-center gap-6 mt-4 pt-4 border-t border-white/10">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-gray-500 font-mono">Interviews</span>
                                </div>
                                <div className="flex items-center gap-2">
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-sm p-6">
                            ...
                            {insightsLoading ? (
                                <div className="flex items-center justify-center py-8">
                                    ...
                                    <span className="ml-3 text-gray-400 font-mono text-sm">Generating insights...</span>
                                </div>
                            ) : aiInsights ? (
                                <div className="space-y-6">
                                    ...
                                    {/* Performance Rating */}
                                    <div className="flex items-center gap-4">
                                        <div className="text-xs text-gray-500 font-mono uppercase">Performance</div>
                                        <Badge className={`
                                        ...
                                        {aiInsights.efficiencyScore !== undefined && (
                                            <div className="flex items-center gap-2 ml-auto">
                                                <span className="text-xs text-gray-500 font-mono">Efficiency</span>
                                    ...
                                    {aiInsights.keyHighlights?.length > 0 && (
                                        <div>
                                            <div className="text-xs text-gray-500 font-mono uppercase mb-2">Key Highlights</div>
                                            <div className="flex flex-wrap gap-2">
                                                {aiInsights.keyHighlights.map((highlight, idx) => (
                                                    <div key={idx} className="flex items-center gap-1.5 bg-emerald-500/10 text-emerald-400 text-xs px-2 py-1 rounded-sm">
                                    ...
                                    {aiInsights.actionItems?.length > 0 && (
                                        <div>
                                            <div className="text-xs text-gray-500 font-mono uppercase mb-2">Recommended Actions</div>
                                            <div className="space-y-2">
                                    ...
                                    {aiInsights.riskAreas?.length > 0 && (
                                        <div>
                                            <div className="text-xs text-gray-500 font-mono uppercase mb-2">Risk Areas</div>
                                            <div className="flex flex-wrap gap-2">
                                                {aiInsights.riskAreas.map((risk, idx) => (
                                                    <div key={idx} className="flex items-center gap-1.5 bg-red-500/10 text-red-400 text-xs px-2 py-1 rounded-sm">
                                    ...
                                    {aiInsights.prediction && (
                                        <div className="bg-purple-500/10 p-3 rounded-sm border border-purple-500/20">
                                            <div className="flex items-center gap-2 text-purple-400 text-sm">
                                                ...
                                                <span className="font-mono text-xs uppercase">Prediction:</span>
                                            </div>
                                            <p className="text-gray-300 text-sm mt-1">{aiInsights.prediction}</p>
                                        </div>
                                    )}
                                </div>
                            ) : (
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-gray-500 text-sm font-mono">No insights available</p>
                                </div>
                            )}
                        </div>
                    </TabsContent>
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-sm">
                            ...
                            {pendingApprovals.length === 0 ? (
                                <div className="p-12 text-center">
                                    ...
                                    <h4 className="text-white text-lg font-bold mb-2">All Caught Up!</h4>
                                    <p className="text-gray-500 text-sm">No pending approvals at this time.</p>
                                </div>
                            ) : (
                                <div className="divide-y divide-white/10">
                                    {pendingApprovals.map((approval) => (
                                        <div key={approval._id} className="p-4 hover:bg-white/5 transition-colors">
                                            <div className="flex items-center gap-4">
                                                <Avatar className="w-14 h-14 border-2 border-white/10">
                                                    <AvatarImage src={approval.applicant?.profile?.profilePhoto} />
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-3 mb-1">
                                                        <h4 className="text-white font-bold">{approval.applicant?.fullname}</h4>
                                                        <Badge className={getUrgencyColor(approval.urgency)}>
                                                            {approval.urgency?.toUpperCase() || 'PENDING'}
                                                        </Badge>
                                                    </div>
                                                    <p className="text-gray-400 text-sm">{approval.job?.title}</p>
                                                    <div className="flex items-center gap-4 mt-2 text-xs font-mono">
                                                        <span className="text-gray-500">
                                                            Recruiter Score: <span className="text-emerald-400">{approval.recruiterScore || 85}%</span>
                                                        </span>
                                                        <span className="text-gray-500">
                                                            Interview: <span className="text-purple-400">{approval.interviewScore || 88}%</span>
                                                        </span>
                                                        <span className="text-gray-500">
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() => handleApprovalDecision(approval._id, 'approved')}
                                                    >
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() => handleApprovalDecision(approval._id, 'rejected')}
                                                    >
                    ...
                    <TabsContent value="team" className="space-y-6">
                        <div className="bg-[#111111] border border-white/10 rounded-sm">
                            ...
                            <div className="divide-y divide-white/10">
                                {recruiterPerformance.sort((a, b) => b.positionsFilled - a.positionsFilled).map((recruiter, idx) => (
                                    <div key={recruiter._id} className="p-4 flex items-center gap-4 hover:bg-white/5 transition-colors">
                                        <div className={`w-8 h-8 rounded-sm flex items-center justify-center font-bold font-mono ${idx === 0 ? 'bg-[#FFD700]/20 text-[#FFD700]' :
                                        ...
                                        <div className="flex-1">
                                            <h4 className="text-white font-medium">{recruiter.fullname}</h4>
                                            <p className="text-gray-500 text-xs font-mono">{recruiter.email}</p>
                                        </div>
                                        <div className="grid grid-cols-4 gap-8 text-center">
                                            <div>
                                                <div className="text-xl font-bold text-emerald-400 font-mono">{recruiter.positionsFilled}</div>
                                                <div className="text-[10px] text-gray-500 uppercase tracking-wider">Filled</div>
                                            </div>
                                            <div>
                                                <div className="text-xl font-bold text-blue-400 font-mono">{recruiter.avgTimeToFill}d</div>
                                                <div className="text-[10px] text-gray-500 uppercase tracking-wider">Avg Time</div>
                                            </div>
                                            <div>
                                                <div className="text-xl font-bold text-purple-400 font-mono">{recruiter.qualityScore}%</div>
                                                <div className="text-[10px] text-gray-500 uppercase tracking-wider">Quality</div>
                                            </div>
                                            <div>
                                                <div className="text-xl font-bold text-[#FFD700] font-mono">{recruiter.activeInterviews}</div>
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="p-6 space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-white/5 p-4 rounded-sm">
                                        <div className="text-xs text-gray-500 font-mono mb-1">POSITION</div>
                                        <div className="text-white font-medium">{selectedApproval.job?.title}</div>
                                    </div>
                                    <div className="bg-white/5 p-4 rounded-sm">
                                        <div className="text-xs text-gray-500 font-mono mb-1">INTERVIEW SCORE</div>
                                ...
                                <div>
                                    <label className="text-xs text-gray-500 font-mono mb-2 block">CEO NOTES</label>
                                    <Textarea
                                        value={ceoNotes}
                            ...
                            <div className="p-6 border-t border-white/10 flex justify-end gap-3">
                                ...
                                <Button
                                    onClick={() => handleApprovalDecision(selectedApproval._id, 'rejected')}
                                    disabled={processingDecision}
                                    className="bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30"
                                >
                                    {processingDecision ? <Loader2 className="w-4 h-4 animate-spin" /> : <XCircle className="w-4 h-4 mr-2" />}
                                    Reject
                                </Button>
                                <Button
                                    onClick={() => handleApprovalDecision(selectedApproval._id, 'approved')}
                                    disabled={processingDecision}
                                    className="bg-[#FFD700] hover:bg-[#FFD700]/90 text-black font-bold"
                                >
                                    {processingDecision ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4 mr-2" />}
                                    Approve Hire
                                </Button>
                            </div>
                        </motion.div>
