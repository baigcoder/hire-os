  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:8000/api/v1/interview/${id}`,
        {
  ...
  const handleDecision = async (status) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8000/api/v1/interview/${id}/ceo/decision`,
        {
          decision: status,
          comments: ceoNotes,
        },
      ...
      if (res.data.success) {
        toast.success(
          `Candidate ${status === "approved" ? "Approved" : status === "rejected" ? "Rejected" : "put on Hold"}`,
        );
        setDecision(status);
        setTimeout(() => {
          navigate("/company/admin/dashboard");
  ...
  if (!report || !report.finalReport)
    return (
      <div className="p-8 text-center">
        Report not found or not generated yet.
      </div>
    );
            ...
            <Card>
              <CardHeader className="text-center pb-2">
                <Avatar className="w-24 h-24 mx-auto mb-4 border-4 border-white shadow-lg">
                  ...
                  <AvatarFallback className="text-2xl">
                    {report.studentId?.fullname?.charAt(0)}
                  </AvatarFallback>
                </Avatar>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <div
                    className={`text-4xl font-extrabold ${finalReport.overallScore >= 70 ? "text-green-600" : finalReport.overallScore >= 50 ? "text-yellow-600" : "text-red-600"}`}
                  >
                  ...
                  <Badge
                    variant="outline"
                    className={`mt-2 ${
                      finalReport.aiRecommendation === "STRONGLY_RECOMMEND"
                        ? "bg-green-100 text-green-800"
                        : finalReport.aiRecommendation === "RECOMMEND"
              ...
              <CardContent className="space-y-8">
                ...
                <div>
                  <h4 className="flex items-center gap-2 font-semibold text-gray-900 mb-2">
                    <TrendingUp className="w-4 h-4 text-blue-500" /> Executive
                    Summary
                  </h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="flex items-center gap-2 font-semibold text-gray-900 mb-3">
                      <CheckCircle className="w-4 h-4 text-green-500" /> Key
                      Strengths
                    </h4>
                  ...
                  <div>
                    <h4 className="flex items-center gap-2 font-semibold text-gray-900 mb-3">
                      <AlertTriangle className="w-4 h-4 text-amber-500" /> Areas
                      of Concern
                    </h4>
                    <ul className="space-y-2">
                ...
                <div>
                  <h4 className="flex items-center gap-2 font-semibold text-gray-900 mb-3">
                    <div
                      className={`w-2 h-2 rounded-full ${finalReport.hiringRisk === "HIGH" ? "bg-red-500" : finalReport.hiringRisk === "MEDIUM" ? "bg-yellow-500" : "bg-green-500"}`}
                    ></div>
                    Risk Assessment
                  </h4>
                  <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-lg">
                    <div>
                      ...
                      <div className="font-semibold">
                        {report.totalFraudAlerts || 0} Events
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div
                        className={`font-semibold ${finalReport.hiringRisk === "HIGH" ? "text-red-600" : finalReport.hiringRisk === "MEDIUM" ? "text-yellow-600" : "text-green-600"}`}
                      >
