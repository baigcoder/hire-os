    ...
    const fetchData = async () => {
      try {
        ...
        const [statsRes, jobRes, applicantsRes] = await Promise.all([
          axios.get(`${JOB_API_END_POINT}/${id}/report/stats`, {
            withCredentials: true,
          }),
          axios.get(`${JOB_API_END_POINT}/get/${id}`, {
            withCredentials: true,
          }),
          axios.get(`${APPLICATION_API_END_POINT}/${id}/applicants`, {
            withCredentials: true,
          }),
        ]);
      ...
      <div className="max-w-[1200px] mx-auto space-y-8 print:p-0 print:bg-white print:text-black">
        ...
        <div className="space-y-8 print:space-y-4">
          ...
          <div className="text-center space-y-2 border-b border-white/10 pb-8">
            ...
            <Badge
              variant="outline"
              className="mt-2 border-white/10 text-gray-500"
            >
              {reportData.period}
            </Badge>
          </div>
            ...
            {[
              {
                ...
                value: reportData.stats.totalApplied,
                icon: Users,
                color: "text-blue-400",
              },
              {
                label: "Interviewed",
                value: reportData.stats.interviewed,
                icon: Users,
                color: "text-purple-400",
              },
              {
                label: "Hired",
                value: reportData.stats.hired,
                icon: Clock,
                color: "text-yellow-400",
              },
              {
                label: "Avg Profile Score",
                value: `${reportData.stats.avgScore}%`,
                icon: TrendingUp,
              ...
              <Card
                key={i}
                className="bg-[#0a0a0a] border-white/10 print:border-gray-200"
              >
                <CardContent className="p-6 text-center">
                  <stat.icon
                    className={`mx-auto mb-2 ${stat.color} print:text-black`}
                    size={24}
                  />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Card className="bg-[#0a0a0a] border-white/10 print:border-gray-200">
              ...
              <CardContent className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={reportData.funnelData} layout="vertical">
            ...
            <Card className="bg-[#0a0a0a] border-white/10 print:border-gray-200">
              <CardHeader>
                <CardTitle className="text-white print:text-black text-lg">
                  Top Selected Talent
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {reportData.topCandidates.length === 0 ? (
                    ...
                    reportData.topCandidates.map((c, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/5 print:border-gray-300 print:bg-gray-50"
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-bold text-gray-200 print:text-black">
                              {c.name}
                            </div>
                        ...
                        <div className="text-right">
                          ...
                          <Badge
                            variant="secondary"
                            className="text-[10px] bg-white/10 text-gray-400 print:bg-gray-200 print:text-black"
                          >
                            {c.status}
                          </Badge>
                        </div>
          ...
          <div className="bg-gradient-to-r from-yellow-900/20 to-black border border-yellow-500/20 rounded-2xl p-6 print:border-black">
            ...
            <p className="text-gray-400 text-sm leading-relaxed print:text-black">
              The candidate pool shows an average profile match of{" "}
              <span className="text-white font-bold print:text-black">
                {reportData.stats.avgScore}%
              </span>
              .
