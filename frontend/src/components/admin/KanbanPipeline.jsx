...
import {
    User, GripVertical, Mail, Clock,
    Star, Loader2, Search,
    UserCheck, UserX, Calendar, MessageSquare, FileText, Sparkles
} from 'lucide-react';
...
const PIPELINE_STAGES = [
    {
        ...
        headerBg: 'bg-gray-500/10', headerBorder: 'border-gray-500/30',
        headerText: 'text-gray-400', badgeBg: 'bg-gray-500/20', badgeText: 'text-gray-400'
    },
    {
        id: 'reviewing', label: 'SCREENING', icon: FileText,
        headerBg: 'bg-blue-500/10', headerBorder: 'border-blue-500/30',
        headerText: 'text-blue-400', badgeBg: 'bg-blue-500/20', badgeText: 'text-blue-400'
    },
    {
        id: 'interview', label: 'INTERVIEW', icon: Calendar,
        headerBg: 'bg-purple-500/10', headerBorder: 'border-purple-500/30',
        headerText: 'text-purple-400', badgeBg: 'bg-purple-500/20', badgeText: 'text-purple-400'
    },
    {
        id: 'pending_ceo_approval', label: 'FINAL REVIEW', icon: UserCheck,
        headerBg: 'bg-amber-500/10', headerBorder: 'border-amber-500/30',
        headerText: 'text-[#FFD700]', badgeBg: 'bg-amber-500/20', badgeText: 'text-[#FFD700]'
    },
    {
        id: 'offer_sent', label: 'OFFER', icon: Mail,
        headerBg: 'bg-emerald-500/10', headerBorder: 'border-emerald-500/30',
        headerText: 'text-emerald-400', badgeBg: 'bg-emerald-500/20', badgeText: 'text-emerald-400'
    },
    {
        id: 'hired', label: 'HIRED', icon: Star,
        headerBg: 'bg-green-500/10', headerBorder: 'border-green-500/30',
        headerText: 'text-green-400', badgeBg: 'bg-green-500/20', badgeText: 'text-green-400'
    },
    {
        id: 'rejected', label: 'REJECTED', icon: UserX,
        headerBg: 'bg-red-500/10', headerBorder: 'border-red-500/30',
        headerText: 'text-red-400', badgeBg: 'bg-red-500/20', badgeText: 'text-red-400'
    }
];
...
const KanbanPipeline = ({ jobId, jobTitle }) => {
    ...
    const fetchApplicants = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${API_BASE}/application/${jobId}/applicants`, {
            ...
            if (res.data.success) {
                ...
                (res.data.job?.applications || []).forEach(app => {
                    ...
                    if (['screened', 'shortlisted'].includes(status)) mappedStatus = 'reviewing';
                    if (['video_scheduled', 'video_completed', 'interviewed'].includes(status)) mappedStatus = 'interview';
                    if (['offer_pending', 'offer_accepted'].includes(status)) mappedStatus = 'offer_sent';
    ...
    const handleDrop = async (e, targetStage) => {
        ...
        newApplicants[sourceStage] = newApplicants[sourceStage].filter(a => a.id !== item.id);
        newApplicants[targetStage] = [...newApplicants[targetStage], { ...item, status: targetStage }];
        ...
        try {
            await axios.post(`${API_BASE}/application/status/${item.id}/update`, {
                status: targetStage
            }, { withCredentials: true });
    ...
    const filteredApplicants = useCallback((stageApplicants) => {
        if (!searchQuery) return stageApplicants;
        const query = searchQuery.toLowerCase();
        return stageApplicants.filter(a =>
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-16">
                ...
                <p className="text-gray-400 mt-4 font-mono text-sm tracking-wider">LOADING PIPELINE...</p>
            </div>
        );
    }
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500 font-mono tracking-wider">{jobTitle || 'ALL CANDIDATES'}</p>
                    </div>
                </div>
            ...
            <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-white/10">
                {PIPELINE_STAGES.map((stage) => {
                    ...
                    const stageApplicants = filteredApplicants(applicants[stage.id] || []);
                    ...
                    return (
                        ...
                        >
                            {/* Column Header */}
                            <div className={`flex items-center justify-between p-3 ${stage.headerBg} border ${stage.headerBorder} rounded-t-sm`}>
                                <div className="flex items-center gap-2">
                                    <Icon className={`w-4 h-4 ${stage.headerText}`} />
                                    <span className={`font-mono text-xs tracking-wider ${stage.headerText}`}>{stage.label}</span>
                                </div>
                                <Badge className={`${stage.badgeBg} ${stage.badgeText} ${stage.headerBorder} border font-mono text-[10px]`}>
                                    {counts[stage.id] || 0}
                                </Badge>
                            </div>
                            ...
                            {/* Column Body */}
                            <div
                                className={`min-h-[400px] p-2 space-y-2 bg-[#0a0a0a] border border-t-0 rounded-b-sm transition-all ${isDragOver
                                    ...
                                    {stageApplicants.length === 0 ? (
                                        ...
                                        stageApplicants.map((applicant, index) => (
                                            <motion.div
                                                ...
                                                draggable
                                                onDragStart={(e) => handleDragStart(e, applicant, stage.id)}
                                                ...
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center justify-between">
                                                            ...
                                                            <span className={`text-xs font-mono ${getScoreColor(applicant.score)}`}>
                                                                {applicant.score}%
                                                            </span>
                                                        </div>
                                                        <p className="text-[10px] text-gray-500 font-mono truncate">{applicant.email}</p>
