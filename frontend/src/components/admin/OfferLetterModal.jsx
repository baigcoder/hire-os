...
import { Loader2, Send, FileText, DollarSign, Calendar, Briefcase, Gift, CheckCircle } from 'lucide-react';
import axios from 'axios';
import { Badge } from '@/components/ui/badge';

const API_BASE = import.meta.env.VITE_API_URL || 'http://localhost:8000/api/v1';

const OfferLetterModal = ({ isOpen, onClose, interviewId, candidateName, jobTitle, onOfferSent }) => {
    ...
    const handleCreateOffer = async () => {
        ...
        setIsLoading(true);
        try {
            const res = await axios.post(`${API_BASE}/interview/${interviewId}/offer/create`, {
                ...offerData,
    ...
    const handleSendOffer = async () => {
        ...
        setStep('sending');
        try {
            const res = await axios.post(`${API_BASE}/interview/${interviewId}/offer/send`, {}, {
                withCredentials: true
            });
            ...
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        ...
                        {step === 'success' && 'The candidate will receive an email notification'}
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="py-4">
                        ...
                        <div className="space-y-6">
                            <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-6">
                                <h3 className="text-lg font-bold text-green-800 mb-4 flex items-center gap-2">
                                    <FileText className="w-5 h-5" /> Offer Summary for {candidateName}
                                </h3>

                                <div className="grid grid-cols-2 gap-4 text-sm">
                                    <div>
                                        <span className="text-gray-500">Position:</span>
                                        <p className="font-semibold text-gray-900">{createdOffer.position}</p>
                                    </div>
                                    {createdOffer.department && (
                                        <div>
                                            <span className="text-gray-500">Department:</span>
                                            <p className="font-semibold text-gray-900">{createdOffer.department}</p>
                                        </div>
                                    )}
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <p className="font-semibold text-gray-900">
                                            {new Date(createdOffer.joiningDate).toLocaleDateString('en-US', {
                                                weekday: 'long',
                                                year: 'numeric',
                                                month: 'long',
                                                day: 'numeric'
                            ...
                            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
                                ...
                                <p className="text-sm text-yellow-800">
                                    Once sent, the candidate will receive an email with these offer details.
                                    The offer will expire in 7 days.
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 'sending' && (
                        <div className="flex flex-col items-center justify-center py-12 space-y-4">
                            ...
                            <div className="text-center">
                                <h3 className="text-lg font-medium text-gray-900">Sending Offer Letter...</h3>
                                <p className="text-gray-500">Preparing email for {candidateName}</p>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 'success' && (
                        <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
                            ...
                            <h3 className="text-2xl font-bold text-gray-900">Offer Sent Successfully! 🎉</h3>
                            <p className="text-gray-500 max-w-sm">
                                {candidateName} will receive an email with the offer details and can respond through their dashboard.
                            </p>
                        </div>
                    )}
                </div>
                ...
                <DialogFooter className="border-t pt-4">
                    ...
                    {step === 'review' && (
                        <>
                            <Button variant="outline" onClick={() => setStep('create')}>Edit</Button>
                            <Button
