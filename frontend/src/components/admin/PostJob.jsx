...
import {
    Loader2, MapPin, Building, Briefcase, Sparkles, Wand2,
    Terminal, ChevronRight, Check, AlertTriangle, Clock,
    DollarSign, Users, GraduationCap, FileText, Zap
} from 'lucide-react'

// Pakistan cities for the location dropdown
const pakistanCities = [
    "Karachi", "Lahore", "Islamabad", "Rawalpindi", "Faisalabad", "Multan", "Peshawar",
    "Quetta", "Sialkot", "Gujranwala", "Hyderabad", "Abbottabad", "Bahawalpur",
    "Sargodha", "Sukkur", "Larkana", "Sheikhupura", "Rahim Yar Khan", "Jhang", "Dera Ghazi Khan", "Remote"
];
...
// Experience levels
const experienceLevels = ["Fresher", "1-2 years", "2-4 years", "4-6 years", "6+ years", "Senior Level"];
        ...
        const fetchCompanies = async () => {
            try {
                ...
                if (res.data.success && res.data.companies?.length > 0) {
                    ...
                    setInput(prev => ({ ...prev, companyId: res.data.companies[0]._id }));
                }
            } catch (error) {
    ...
    const generateAISuggestion = async (field) => {
        ...
        try {
            ...
            if (response.data.success && response.data.content) {
                setAiSuggestions(prev => ({ ...prev, [field]: response.data.content }));
                ...
                const fallbackSuggestions = {
                    ...
                    salary: input.experience?.includes('Fresher') ? '50,000 - 80,000 PKR' : input.experience?.includes('Senior') ? '200,000 - 350,000 PKR' : '100,000 - 180,000 PKR'
                };
                setAiSuggestions(prev => ({ ...prev, [field]: fallbackSuggestions[field] }));
            ...
            <div className="max-w-4xl mx-auto px-4 py-8 pt-24 relative z-10">
                ...
                >
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-white uppercase tracking-tight">Create Job Post</h1>
                            <p className="text-gray-500 text-sm font-mono">AI-powered job creation</p>
                        </div>
                    </div>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-[#00FF94] rounded-sm flex items-center justify-center">
                                {selectedCompany.logo ? (
                                    <img src={selectedCompany.logo} alt={selectedCompany.name} className="w-full h-full object-cover rounded-sm" />
                                ) : (
                                    <span className="text-lg font-bold text-black">{selectedCompany.name?.charAt(0)}</span>
                                )}
                            </div>
                            <div className="flex-1">
                                ...
                                <p className="text-gray-500 text-xs font-mono">Posting as company admin</p>
                            </div>
                            {companies.length > 1 && (
                                <Select onValueChange={selectChangeHandler} value={selectedCompany.name.toLowerCase()}>
                ...
                {companiesLoading && (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-gray-400 font-mono text-sm">Loading company data...</p>
                        </div>
                    </motion.div>
                )}
                ...
                {!companiesLoading && companies.length === 0 && (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-red-400 font-bold">Company Not Found</p>
                                <p className="text-red-400/70 text-sm">Unable to load your company. Please contact support or try logging out and back in.</p>
                            </div>
                            <Button
                                onClick={() => navigate('/company/admin/dashboard')}
                                className="ml-auto bg-red-500 hover:bg-red-600 text-white rounded-sm"
                            >
                                Go to Dashboard
                            </Button>
                        </div>
                ...
                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="md:col-span-2">
                            ...
                            />
                            <AnimatePresence>
                                {aiSuggestions.description && input.description !== aiSuggestions.description && (
                                    <motion.div
                                    ...
                                    >
                                        ...
                                        <p className="text-gray-400 text-xs whitespace-pre-line">{aiSuggestions.description}</p>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            />
                            <AnimatePresence>
                                {aiSuggestions.requirements && input.requirements !== aiSuggestions.requirements && (
                                    <motion.div
                                        initial={{ opacity: 0, height: 0 }}
                                        animate={{ opacity: 1, height: 'auto' }}
                                        exit={{ opacity: 0, height: 0 }}
                                    ...
                                    >
                                        ...
                                        <p className="text-gray-400 text-xs">{aiSuggestions.requirements}</p>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </div>
                        ...
                        <div>
                            ...
                            <Input
                                ...
                                className="bg-[#0A0A0A] border-white/10 text-white rounded-sm focus:border-[#FFD700]/50 placeholder:text-gray-600"
                            />
                            {aiSuggestions.salary && input.salary !== aiSuggestions.salary && (
                                <button
                                    type="button"
                                ...
                                <SelectContent className="bg-[#111111] border-white/10">
                                    <SelectGroup>
                                        {pakistanCities.map((city) => (
                                            <SelectItem key={city} value={city} className="text-white hover:bg-white/5">
                                                {city}
                                            </SelectItem>
                                        ))}
                                    </SelectGroup>
                        ...
                        <div>
                            ...
                            >
                                ...
                                <SelectContent className="bg-[#111111] border-white/10">
                                    <SelectGroup>
                                        {jobTypes.map((type) => (
                                            <SelectItem key={type} value={type} className="text-white hover:bg-white/5">
                                                {type}
                                            </SelectItem>
                                        ))}
                                    </SelectGroup>
                                ...
                                <SelectContent className="bg-[#111111] border-white/10">
                                    <SelectGroup>
                                        {experienceLevels.map((level) => (
                                            <SelectItem key={level} value={level} className="text-white hover:bg-white/5">
                                                {level}
                                            </SelectItem>
                                        ))}
                                    </SelectGroup>
