...
import {
    ...
    Target, BarChart3, Activity, RefreshCw, Loader2, Sparkles
} from 'lucide-react';
...
const RecruiterAnalytics = () => {
    ...
    const fetchAnalytics = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${API_BASE}/stats/recruiter?period=${period}`, {
            ...
            setAnalytics({
                ...
                funnel: { applied: 248, screened: 180, interviewed: 45, offered: 12, hired: 9 },
                byStatus: { pending: 68, reviewing: 45, interviewing: 23, offered: 12, rejected: 91, hired: 9 },
                topJobs: [
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-16">
                ...
                <p className="text-gray-400 mt-4 font-mono text-sm tracking-wider">LOADING ANALYTICS...</p>
            </div>
        );
    }
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-white tracking-tight">RECRUITMENT ANALYTICS</h2>
                        <p className="text-xs text-gray-500 font-mono tracking-wider">REAL-TIME HIRING METRICS</p>
                    </div>
                </div>
                ...
                <div className="flex gap-2">
                    {['7d', '30d', '90d'].map(p => (
                        <button
                            key={p}
                            onClick={() => setPeriod(p)}
                            className={`px-3 py-1.5 text-xs font-mono uppercase tracking-wider rounded-sm transition-all ${period === p
                    ...
                    <Button onClick={fetchAnalytics} size="sm" className="bg-white/5 border border-white/10 text-gray-400 hover:bg-white/10">
            ...
            <div className="grid grid-cols-4 gap-4">
                {[
                    { icon: Users, label: 'TOTAL APPLICATIONS', value: analytics?.overview?.totalApplications || 0, change: analytics?.overview?.changePercent, color: 'blue' },
                    { icon: Briefcase, label: 'ACTIVE JOBS', value: analytics?.overview?.activeJobs || 0, color: '[#FFD700]' },
                    { icon: Clock, label: 'AVG. TIME TO HIRE', value: `${analytics?.overview?.avgTimeToHire || 0} days`, color: 'amber' },
                    { icon: Target, label: 'OFFER ACCEPT RATE', value: `${analytics?.overview?.offerAcceptRate || 0}%`, color: 'emerald' }
                ].map((stat, i) => {
                    ...
                    return (
                        ...
                        >
                            <div className="flex items-center justify-between mb-3">
                                <div className={`w-10 h-10 rounded-sm bg-${stat.color}-500/10 flex items-center justify-center border border-${stat.color}-500/30`}>
                                    <Icon className={`w-5 h-5 text-${stat.color}-400`} />
                                </div>
                                {stat.change !== undefined && (
                                    <Badge className={`${stat.change >= 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' : 'bg-red-500/10 text-red-400 border-red-500/30'} border font-mono text-[10px]`}>
            ...
            <div className="grid grid-cols-2 gap-6">
                ...
                >
                    <div className="flex items-center gap-2 mb-5">
                        ...
                        <h3 className="text-xs text-gray-500 font-mono tracking-wider">HIRING FUNNEL</h3>
                    </div>
                            ...
                            return (
                                <div key={stage.key} className="space-y-1.5">
                                    <div className="flex justify-between text-xs font-mono">
                                        <span className="text-gray-500 tracking-wider">{stage.label}</span>
                ...
                >
                    <div className="flex items-center gap-2 mb-5">
                        ...
                        <h3 className="text-xs text-gray-500 font-mono tracking-wider">STATUS BREAKDOWN</h3>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {[
                            ...
                            return (
                                ...
                                >
                                    <div className={`text-xl font-bold font-mono text-${status.color}-400`}>{value}</div>
                                    <div className="text-[10px] text-gray-500 font-mono tracking-wider">{status.label}</div>
                                </div>
                            );
                        })}
                    </div>
            ...
            >
                <div className="flex items-center gap-2 mb-5">
                    ...
                    <h3 className="text-xs text-gray-500 font-mono tracking-wider">TOP PERFORMING JOBS</h3>
                </div>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-6">
                                <div className="text-center">
                                    <div className="text-sm text-white font-mono">{job.applications}</div>
                                    <div className="text-[10px] text-gray-500 font-mono">APPS</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-sm text-emerald-400 font-mono">{job.interviews}</div>
                                    <div className="text-[10px] text-gray-500 font-mono">INTERVIEWS</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-sm text-[#FFD700] font-mono">
                                        {job.applications > 0 ? Math.round((job.interviews / job.applications) * 100) : 0}%
                                    </div>
                                    <div className="text-[10px] text-gray-500 font-mono">CONV.</div>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <motion.div
                ...
                className="bg-[#0a0a0a] border border-white/10 rounded-sm p-5"
            >
                <h3 className="text-xs text-gray-500 font-mono tracking-wider mb-5">WEEKLY APPLICATION TREND</h3>
                    ...
                    {['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN'].map((day, index) => {
                        ...
                        return (
                            <div key={day} className="flex-1 flex flex-col items-center group">
                                ...
                                <div className="text-[10px] text-gray-500 font-mono mt-2">{day}</div>
                            </div>
                        );
                    })}
                </div>
