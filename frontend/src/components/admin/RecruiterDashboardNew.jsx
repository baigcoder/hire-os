...
import {
    Calendar, Users, Briefcase, Clock, PlusCircle,
    TrendingUp, TrendingDown, CheckCircle, Bell,
    ArrowUp, ArrowDown, Minus, ArrowUpRight, ArrowDownRight,
    Mail, Phone, Download, Linkedin, Globe, UserPlus,
    MapPin, Building, Search, Filter, MonitorPlay, BrainCircuit,
    Send, Key, User, Shield, RefreshCw, Sparkles, Target,
    CheckSquare, Square, Trash2, MoreHorizontal, CalendarDays,
    ChevronLeft, ChevronRight, ExternalLink, Crown
} from 'lucide-react';
...
import { APPLICATION_API_END_POINT, USER_API_END_POINT, RECRUITER_ANALYTICS_API_END_POINT } from '@/utils/constant';
...
const RecruiterDashboard = () => {
    ...
    const [stats, setStats] = useState({
        totalJobs: 0, activeJobs: 0, totalApplicants: 0, pendingInterviews: 0,
        acceptedCandidates: 0, conversionRate: 0, recentApplications: [],
        applicationsByStatus: [], applicationTrends: [], topPerformingJobs: [], upcomingInterviews: [],
    ...
    const handlePassToCEO = async (applicationId) => {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || 'Failed to pass candidate to CEO');
        } finally {
        ...
        const fetchAnalyticsData = async () => {
            try {
                ...
                const [sourceRes, trendsRes] = await Promise.all([
                    axios.get(`${RECRUITER_ANALYTICS_API_END_POINT}/source-stats`, { withCredentials: true }).catch(() => ({ data: { success: false } })),
                    axios.get(`${RECRUITER_ANALYTICS_API_END_POINT}/trends`, { withCredentials: true }).catch(() => ({ data: { success: false } }))
                ]);
    ...
    useEffect(() => {
        ...
        const activeJobs = allAdminJobs.filter(job => job.isActive !== false).length;
        ...
        allAdminJobs.forEach(job => {
            if (job.applications?.length > 0) {
                ...
                jobPerformance[job._id] = { title: job.title, applicants: job.applications.length, accepted: 0 };

                job.applications.forEach(app => {
                    if (statusCounts[app.status] !== undefined) statusCounts[app.status]++;
                    if (app.status === 'accepted') { acceptedCandidates++; jobPerformance[job._id].accepted++; }
                    if (app.status === 'rejected') rejectedCandidates++;

                    const mKey = new Date(app.createdAt).toLocaleDateString('en-US', { month: 'short' });
                    if (monthlyData[mKey] !== undefined) monthlyData[mKey]++;

                    if (app.status === 'interview') pendingInterviews++;
                    // Mock upcoming interviews logic if sensitive
                    if (app.status === 'interview') {
                        upcomingInterviews.push({
                            applicantName: app.applicant?.fullname || 'Candidate', // Fallback
                            jobTitle: job.title,
                            date: new Date(Date.now() + 86400000 * Math.floor(Math.random() * 5)).toISOString(), // Mock date for demo
                            mode: 'Video Call'
                        })
                    }
        ...
        const sortedRecent = recentApplications.sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 10);
        const sortedInterviews = upcomingInterviews.slice(0, 5);

        setStats({
            totalJobs, activeJobs, totalApplicants, pendingInterviews, acceptedCandidates, rejectedCandidates,
            conversionRate: totalApplicants > 0 ? ((acceptedCandidates / totalApplicants) * 100).toFixed(1) : 0,
            ...
            applicationTrends: Object.keys(monthlyData).map(k => ({ name: k, applications: monthlyData[k] })),
            topPerformingJobs: Object.values(jobPerformance).sort((a, b) => b.applicants - a.applicants).slice(0, 5)
        });
            ...
            <div className="max-w-[1600px] mx-auto px-4 py-8 relative z-10 pt-20">
                ...
                >
                    <div className="flex items-center gap-6">
                        <div className="relative">
                            <Avatar className="h-20 w-20 border-2 border-white/10 shadow-2xl">
                                ...
                                <AvatarFallback className="bg-zinc-900 text-white font-bold text-xl">R</AvatarFallback>
                            </Avatar>
                            <div className="absolute -bottom-1 -right-1 bg-black p-1 rounded-full">
                                <div className="bg-emerald-500 rounded-full p-1"><CheckCircle size={10} className="text-black" /></div>
                            </div>
                        </div>
                        <div>
                            ...
                            <p className="text-gray-500 flex items-center gap-2 mt-1 font-mono text-xs">
                                <Building size={12} className="text-[#FFD700]" /> {user?.profile?.company?.name || 'HIRE.OS Operator'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3 w-full xl:w-auto">
                        ...
                        >
                            <Bell size={16} />
                            <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full text-[9px] flex items-center justify-center">3</span>
                        </Button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
                    {[
                        { label: "APPLICANTS", value: stats.totalApplicants, icon: Users, color: "text-[#00FF94]", bgColor: "bg-[#00FF94]/10", trend: stats.trends?.applicants || 12 },
                        { label: "ACTIVE JOBS", value: stats.activeJobs, icon: Briefcase, color: "text-[#FFD700]", bgColor: "bg-[#FFD700]/10", trend: stats.trends?.jobs || -2 },
                        { label: "INTERVIEWS", value: stats.pendingInterviews, icon: Calendar, color: "text-cyan-400", bgColor: "bg-cyan-400/10", trend: stats.trends?.interviews || 5 },
                        { label: "HIRE RATE", value: `${stats.conversionRate}%`, icon: Target, color: "text-[#FFD700]", bgColor: "bg-[#FFD700]/10", trend: stats.trends?.hireRate || 3 }
                    ].map((stat, i) => (
                        <motion.div
                            key={i}
                            initial={{ opacity: 0, y: 20 }}
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-3">
                                <div className={`p-2 rounded-sm ${stat.bgColor} border border-white/10`}>
                                    <stat.icon size={18} className={stat.color} />
                                </div>
                                {/* Trend Indicator */}
                                <div className={`flex items-center gap-1 text-[10px] font-mono ${stat.trend > 0 ? 'text-emerald-400' : stat.trend < 0 ? 'text-red-400' : 'text-gray-500'}`}>
                                ...
                                <div className="h-[250px]">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            ...
                                            <Tooltip
                                                contentStyle={{ backgroundColor: '#111', customBorder: '1px solid #333', borderRadius: '12px', color: '#fff' }}
                                                itemStyle={{ color: '#fff' }}
                                            />
                                        </PieChart>
                                    </ResponsiveContainer>
                                        ...
                                        <PieChart>
                                            ...
                                            >
                                                {sourceData.map((entry, index) => (
                                                    <Cell key={`source-${index}`} fill={entry.color} stroke="rgba(0,0,0,0.5)" />
                                                ))}
                                            </Pie>
                                            <Tooltip
                                                contentStyle={{ backgroundColor: '#111', border: '1px solid #333', borderRadius: '4px', color: '#fff' }}
                                                itemStyle={{ color: '#fff' }}
                                            />
                                        </PieChart>
                                    </ResponsiveContainer>
                            ...
                            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-5">
                                ...
                                <div className="flex items-center gap-3 w-full md:w-auto">
                                    ...
                                    <div className="flex gap-1">
                                        {['all', 'pending', 'interview', 'accepted'].map(status => (
                                            <Button
                                                key={status}
                                                size="sm"
                                    ...
                                    <Button variant="link" className="text-[#FFD700] text-xs uppercase tracking-wider" onClick={() => navigate('/admin/jobs')}>View All</Button>
                                </div>
                            </div>
                                ...
                                {selectedCandidates.length > 0 && (
                                    ...
                                    >
                                        ...
                                        <div className="flex gap-2">
                                            <Button size="sm" onClick={() => handleBulkAction('Advanced')} className="h-7 px-3 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 text-[10px] uppercase border border-emerald-500/30">
                                                <CheckCircle className="w-3 h-3 mr-1" /> Advance
                                            </Button>
                                            <Button size="sm" onClick={() => handleBulkAction('Rejected')} className="h-7 px-3 bg-red-500/20 hover:bg-red-500/30 text-red-400 text-[10px] uppercase border border-red-500/30">
                                                <Trash2 className="w-3 h-3 mr-1" /> Reject
                                            </Button>
                                            <Button size="sm" onClick={() => handleBulkAction('Email sent to')} className="h-7 px-3 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 text-[10px] uppercase border border-blue-500/30">
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b border-white/5 text-left">
                                            ...
                                            <th className="py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Candidate</th>
                                            <th className="py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Role</th>
                                            <th className="py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Applied</th>
                                            <th className="py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Status</th>
                                            <th className="py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right pr-4">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-white/5">
                                        {filteredApplications.length > 0 ? (
                                            filteredApplications.map((app, i) => (
                                                <tr key={app.id || i} className={`group hover:bg-white/5 transition-colors ${selectedCandidates.includes(app.id) ? 'bg-[#FFD700]/5' : ''}`}>
                                                    ...
                                                    <td className="py-4">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div className="font-medium text-gray-200">{app.applicantName}</div>
                                                        </div>
                                                    </td>
                                                    <td className="py-4 text-gray-400 text-sm font-mono">{app.jobTitle}</td>
                                                    <td className="py-4 text-gray-500 text-sm font-mono">{new Date(app.date).toLocaleDateString()}</td>
                                                    <td className="py-4">
                                                        <Badge className={`${getStatusColor(app.status)} px-3 py-1 bg-opacity-10 border-opacity-20`}>{app.status}</Badge>
                                                    </td>
                                                    <td className="py-4 pr-4 text-right">
                                                        {(app.status === 'interview' || app.status === 'video_completed' || app.status === 'accepted') && (
                                                            <Button
                                                                size="sm"
                                                                onClick={() => {
                                            ...
                                            <tr>
                                                <td colSpan="4" className="py-8 text-center text-gray-500">No applicants yet</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-md p-5 relative overflow-hidden">
                            ...
                            <div className="space-y-4 relative z-10">
                                {stats.upcomingInterviews.length > 0 ? (
                                    stats.upcomingInterviews.map((int, i) => (
                                        <div key={i} className="bg-white/5 border border-white/5 rounded-sm p-3 hover:border-cyan-500/30 transition-all">
                                            <div className="flex justify-between items-start mb-2">
                                                <div className="font-bold text-gray-200 text-sm">{int.applicantName}</div>
                                                <Badge variant="secondary" className="bg-cyan-500/10 text-cyan-400 text-[10px] uppercase tracking-wider">{int.mode}</Badge>
                                            </div>
                                            <div className="text-xs text-gray-500 mb-2 font-mono">{int.jobTitle}</div>
                                            <Button size="sm" className="w-full bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 font-medium border border-cyan-500/30 h-7 rounded-sm text-xs uppercase tracking-wider" onClick={() => navigate('/interview/lobby')}>
                                                Start Session
                                            </Button>
                                        </div>
                                    ))
                                ) : (
                                    <div className="text-center py-6 text-gray-500 text-sm">No upcoming interviews</div>
                                )}

                                <Button variant="outline" className="w-full border-dashed border-white/10 text-gray-500 hover:text-[#FFD700] hover:border-[#FFD700]/30 bg-transparent rounded-sm text-xs uppercase tracking-wider" onClick={() => navigate('/interviews/schedule')}>
                                    Schedule New
                                </Button>
                            </div>
                        </div>
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-md p-5">
                            ...
                            <p className="text-xs text-gray-500 mb-4 font-mono">
                                Job postings receiving 24% more qualified candidates than market average.
                            </p>
                            <div className="h-1.5 w-full bg-white/5 rounded-sm overflow-hidden">
                                <div className="h-full bg-[#FFD700] w-[74%]"></div>
                            </div>
                            <div className="mt-2 text-right text-[10px] text-[#FFD700] font-mono uppercase tracking-wider">Top 5% Performance</div>
                        </div>

                        {/* Top Jobs */}
                        <div className="bg-[#111111] border border-white/10 rounded-md p-5">
                            <h3 className="text-sm font-bold text-white mb-5 uppercase tracking-wider">Top Jobs</h3>
                            <div className="space-y-3">
                                {stats.topPerformingJobs.map((job, i) => (
                                    <div key={i} className="flex items-center justify-between p-2 bg-white/5 rounded-sm">
                                        <div className="flex items-center gap-3">
                                            <div className="text-gray-600 font-mono text-xs">0{i + 1}</div>
                                            <div className="text-gray-300 font-medium text-sm truncate w-28">{job.title}</div>
                                        </div>
                                        <div className="text-[#00FF94] font-mono text-xs">{job.applicants}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
