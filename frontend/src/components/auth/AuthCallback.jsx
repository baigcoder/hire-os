        ...
        const handleAuthCallback = async () => {
            try {
                ...
                if (session) {
                    ...
                    try {
                        const response = await axios.post(`${USER_API_END_POINT}/supabase-sync`, {
                        ...
                        if (response.data.success) {
                            ...
                            setTimeout(() => {
                                console.log('🔐 Auth redirect - User:', user.fullname, 'Role:', user.role, 'isNewUser:', response.data.isNewUser);
                        ...
                        if (syncError.response?.data?.code === 'GOOGLE_LOGIN_RESTRICTED') {
                            setError('Recruiters must login with email and password. Please use the credentials sent to your email by your company admin.');
                            setStatus('error');
