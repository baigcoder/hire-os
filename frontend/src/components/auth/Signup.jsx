...
const Signup = () => {
    ...
    useEffect(() => {
        ...
        strength.score = [strength.hasMinLength, strength.hasUppercase, strength.hasLowercase, strength.hasNumber, strength.hasSpecial].filter(Boolean).length;
    ...
    const handleOtpPaste = (e) => {
        ...
        setOtpCode(pastedData.split('').concat(Array(6 - pastedData.length).fill('')));
    };

    const handleResendOTP = async () => {
        setLoading(true);
        try {
            await axios.post(`${OTP_API_END_POINT}/send`, { email: formData.email, purpose: 'signup' });
            setOtpTimer(60);
            ...
            <div className="min-h-screen bg-[#0A0A0A] text-white font-['Space_Grotesk',sans-serif] flex items-center justify-center p-6">
                ...
                >
                    <div className="bg-[#111111] border border-white/10 rounded-sm p-8 relative">
                        ...
                        <button onClick={() => setStep('form')} className="flex items-center gap-2 text-gray-500 hover:text-white text-xs uppercase tracking-wider mb-6">
                        ...
                        <div className="text-center mb-6">
                            ...
                            <h2 className="text-xl font-bold uppercase tracking-wider mb-2">Verify Email</h2>
                            <p className="text-gray-600 text-xs font-mono">Code sent to <span className="text-white">{formData.email}</span></p>
                        </div>

                        {error && (
                            <div className="flex items-center gap-2 p-3 bg-red-900/20 border border-red-900/50 rounded-sm text-red-400 text-xs mb-4">
                                <AlertCircle size={14} /><span className="font-mono">{error}</span>
                            </div>
                        )}
                        ...
                        <div className="text-center mb-4">
                            {otpTimer > 0 ? (
                                ...
                                <button onClick={handleResendOTP} disabled={loading} className="text-xs text-[#FFD700] hover:underline font-mono">Resend</button>
                            )}
                        </div>
                        ...
                        >
                            {loading ? <Loader2 size={16} className="animate-spin" /> : <>Initialize Account <ArrowRight size={14} /></>}
                        </button>
                    </div>
                </motion.div>
            </div>
        ...
        <div className="min-h-screen bg-[#0A0A0A] text-white font-['Space_Grotesk',sans-serif] flex">
            ...
            >
                ...
                <Link to="/" className="flex items-center gap-2 relative z-10">
                    <div className="w-10 h-10 rounded-sm bg-[#FFD700] flex items-center justify-center text-black font-black text-sm shadow-[0_0_20px_rgba(255,215,0,0.3)]">H.</div>
                    <span className="text-xl font-bold tracking-tight">HIRE<span className="text-[#FFD700]">.OS</span></span>
                </Link>

                <div className="relative z-10 space-y-8">
                    <div>
                        <p className="text-[10px] uppercase tracking-widest text-[#FFD700] font-mono mb-3">// New User Registration</p>
                        <h1 className="text-4xl font-bold uppercase tracking-tight mb-4">
                            Initialize<br /><span className="text-[#FFD700]">Profile</span>
                        </h1>
            ...
            <div className="flex-1 flex items-center justify-center p-6 relative z-10 overflow-y-auto">
                ...
                >
                    <div className="bg-[#111111] border border-white/10 rounded-sm p-8 relative">
                        ...
                        <div className="text-center mb-6">
                            <h2 className="text-xl font-bold uppercase tracking-wider mb-2">Create Account</h2>
                            <p className="text-gray-600 text-xs font-mono">
                                Have an account? <Link to="/login" className="text-[#FFD700] hover:underline">Sign In</Link>
                            </p>
                        </div>
                        ...
                        {/* Role Selection */}
                        <div className="mb-6">
                            <label className="text-[10px] font-mono text-gray-500 uppercase tracking-wider mb-3 block">I am a</label>
                            <div className="grid grid-cols-2 gap-3">
                                <button
                                    type="button"
                                    onClick={() => setFormData({ ...formData, role: 'student' })}
                                    className={`p-4 border rounded-sm text-center transition-all ${formData.role === 'student'
                                        ? 'border-[#00FF94] bg-[#00FF94]/10 text-[#00FF94]'
                                        : 'border-white/10 bg-white/5 text-gray-400 hover:border-white/20'
                                        }`}
                                >
                                    <User className="w-6 h-6 mx-auto mb-2" />
                                    <span className="text-xs font-bold uppercase tracking-wider block">Job Seeker</span>
                                    <span className="text-[10px] text-gray-500 font-mono">Find opportunities</span>
                                </button>
                                <button
                                    type="button"
                                    onClick={() => setFormData({ ...formData, role: 'company_admin' })}
                                    className={`p-4 border rounded-sm text-center transition-all ${formData.role === 'company_admin'
                                        ? 'border-[#FFD700] bg-[#FFD700]/10 text-[#FFD700]'
                                        : 'border-white/10 bg-white/5 text-gray-400 hover:border-white/20'
                                        }`}
                                >
                                    <Building className="w-6 h-6 mx-auto mb-2" />
                                    <span className="text-xs font-bold uppercase tracking-wider block">Company Owner</span>
                                    <span className="text-[10px] text-gray-500 font-mono">Hire talent</span>
                                </button>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-10 pr-4 py-3 rounded-sm border border-white/10 bg-white/5 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-[#FFD700]/50 font-mono"
                                    />
                                </div>
                            </div>
                            ...
                            {/* Phone */}
                            <div>
                                <label className="text-[10px] font-mono text-gray-500 uppercase tracking-wider mb-1.5 block">Phone (Optional)</label>
                                <div className="relative">
                            ...
                            {/* Password */}
                            <div>
                                <label className="text-[10px] font-mono text-gray-500 uppercase tracking-wider mb-1.5 block">Password</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        onChange={handleChange}
                                        placeholder="••••••••"
                                        className="w-full pl-10 pr-10 py-3 rounded-sm border border-white/10 bg-white/5 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-[#FFD700]/50 font-mono"
                                    />
                                ...
                                {formData.password && (
                                    <div className="mt-2">
                                        <div className="flex gap-1 mb-1">
                                            {[1, 2, 3, 4, 5].map((i) => (
                                                <div key={i} className={`h-1 flex-1 rounded-sm ${i <= passwordStrength.score ? getStrengthColor() : 'bg-white/10'}`} />
                                            ))}
                                        </div>
                                        <div className="grid grid-cols-2 gap-1 text-[9px] font-mono">
                                            {[
                                                { check: passwordStrength.hasMinLength, text: '8+ chars' },
                                                ...
                                                <div key={i} className={`flex items-center gap-1 ${req.check ? 'text-[#00FF94]' : 'text-gray-600'}`}>
                                                    {req.check ? <CheckCircle2 size={10} /> : <XCircle size={10} />} {req.text}
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                            ...
                            {/* Confirm Password */}
                            <div>
                                <label className="text-[10px] font-mono text-gray-500 uppercase tracking-wider mb-1.5 block">Confirm Password</label>
                                <div className="relative">
                                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" size={16} />
                                    <input
                                        ...
                                        className="w-full pl-10 pr-10 py-3 rounded-sm border border-white/10 bg-white/5 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-[#FFD700]/50 font-mono"
                                    />
                                    <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-white">
                            ...
                            <div className="flex items-start gap-2">
                                ...
                                />
                                <label htmlFor="terms" className="text-[10px] text-gray-500 font-mono">
                                    I accept the <Link to="/terms" className="text-[#FFD700] hover:underline">Terms</Link> and <Link to="/privacy" className="text-[#FFD700] hover:underline">Privacy Policy</Link>
                                </label>
                            </div>
