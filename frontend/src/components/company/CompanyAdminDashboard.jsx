...
import {
    Building2, Users, Briefcase, TrendingUp, Crown, Plus, Settings,
    CreditCard, ChevronRight, Calendar, Clock, Eye, CheckCircle,
    XCircle, AlertTriangle, Mail, Phone, BarChart3, Zap, Video,
    UserPlus, MoreVertical, RefreshCw, Trash2, Edit2, Upload, UserCheck,
    FileText, ThumbsUp, ThumbsDown, Terminal, Activity
} from 'lucide-react';
...
const IndustrialStatCard = ({ label, value, subtext, icon: Icon, delay = 0, progress = 0, color = '#FFD700' }) => (
    <motion.div
    ...
    >
        ...
        <div className="flex items-center justify-between">
            <div>
                <p className="text-xs text-gray-500 uppercase tracking-wider font-mono mb-2">{label}</p>
                <p className="text-3xl font-bold text-white font-mono">{value}</p>
                {subtext && (
                    <p className={`text-xs mt-1 font-mono ${subtext.startsWith('+') ? 'text-[#00FF94]' : subtext.startsWith('-') ? 'text-red-400' : 'text-gray-600'}`}>{subtext}</p>
                )}
            </div>
            <div className="w-12 h-12 rounded-sm flex items-center justify-center border" style={{ backgroundColor: `${color}10`, borderColor: `${color}30` }}>
                <Icon className="w-6 h-6" style={{ color }} />
            </div>
        </div>
        <Progress value={Math.min(progress, 100)} className="mt-4 h-1 bg-white/5" indicatorClassName="bg-[#FFD700]" />
    </motion.div>
);
...
const CompanyAdminDashboard = () => {
    ...
    const fetchPendingApprovals = async () => {
        try {
            const response = await axios.get(`${COMPANY_API_END_POINT}/pending-approvals`, {
                withCredentials: true
            });
            if (response.data.success) {
    ...
    const handleInviteRecruiter = async () => {
        ...
        setInviting(true);
        try {
            const response = await axios.post(`${COMPANY_API_END_POINT}/recruiters/invite`, {
                email: inviteEmail,
                name: inviteName
            }, { withCredentials: true });
    ...
    const handleRemoveRecruiter = async (recruiterId) => {
        try {
            const response = await axios.delete(`${COMPANY_API_END_POINT}/recruiters/${recruiterId}`, {
                withCredentials: true
            });
            if (response.data.success) {
    ...
    const handleResendInvite = async (recruiterId) => {
        try {
            const response = await axios.post(`${COMPANY_API_END_POINT}/recruiters/${recruiterId}/resend-invite`, {}, {
                withCredentials: true
            });
            if (response.data.success) {
    ...
    const usagePercentage = features.maxJobPostings === 'Unlimited' || features.maxJobPostings === -1
        ? 10
        : (usage.activeJobs / features.maxJobPostings) * 100;

    const recruiterPercentage = features.maxRecruiters === 'Unlimited' || features.maxRecruiters === -1
        ? 10
            ...
            <div className="container mx-auto px-4 py-8 pt-24 relative z-10">
                ...
                >
                    <div className="flex items-center gap-4">
                        <div className="w-16 h-16 bg-[#FFD700] rounded-sm flex items-center justify-center">
                            {company?.logo ? (
                                <img src={company.logo} alt={company.name} className="w-full h-full object-cover rounded-sm" />
                            ) : (
                                <Building2 className="w-8 h-8 text-black" />
                            )}
                        </div>
                        <div>
                            <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">{company?.name || 'Company Dashboard'}</h1>
                            <div className="flex items-center gap-3 mt-2">
                                <Badge className={`rounded-sm text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 ${subscription?.isActive
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
                    <IndustrialStatCard
                        ...
                        icon={Briefcase}
                        delay={0.1}
                        progress={features.maxJobPostings > 0 ? ((usage.activeJobs || 0) / features.maxJobPostings) * 100 : 30}
                    />
                    <IndustrialStatCard
                        label="Team Members"
                        ...
                        icon={Users}
                        delay={0.15}
                        progress={features.maxRecruiters > 0 ? ((company?.recruitersCount || 0) / features.maxRecruiters) * 100 : 20}
                        color="#3B82F6"
                    />
                    <IndustrialStatCard
                        label="Total Applications"
                        value={usage.totalApplications || 0}
                        subtext={usage.applicationGrowth ? `${usage.applicationGrowth > 0 ? '+' : ''}${usage.applicationGrowth}% this month` : 'No growth data'}
                        icon={TrendingUp}
                        delay={0.2}
                ...
                >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                            { icon: Briefcase, label: 'View Jobs', path: '/admin/jobs', color: '#FFD700' },
                            { icon: FileText, label: 'Applications', path: '/admin/applications', color: '#FFD700' },
                            { icon: Video, label: 'Interviews', path: '/admin/interviews', color: '#FFD700' },
                            { icon: BarChart3, label: 'Analytics', path: '/admin/analytics', color: '#FFD700' }
                        ].map((action, idx) => (
                            <motion.button
                                key={idx}
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                    ...
                    >
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <span className="text-gray-500 text-sm font-mono">Current Plan</span>
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <span className={`text-sm font-mono ${subscription?.isActive ? 'text-[#00FF94]' : 'text-red-400'}`}>
                                    {subscription?.isActive ? 'Active' : 'Expired'}
                                </span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-gray-500 text-sm font-mono">Days Remaining</span>
                                <span className="text-white font-bold font-mono">{subscription?.daysRemaining || 0} days</span>
                            </div>
                        </div>
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p className="text-gray-500 text-sm">Manage your recruiters and their permissions</p>
                            </div>
                            <Dialog open={inviteDialogOpen} onOpenChange={setInviteDialogOpen}>
                                ...
                                <DialogContent className="bg-[#111111] border-white/10 rounded-sm">
                                    <DialogHeader>
                                        <DialogTitle className="text-white font-bold">Invite Recruiter</DialogTitle>
                                    ...
                                    <div className="space-y-4 py-4">
                                        <div>
                                            <Label htmlFor="email" className="text-gray-400 text-xs uppercase tracking-wider font-mono">Email Address</Label>
                                            <Input
                                                id="email"
                                                type="email"
                                        ...
                                        <div>
                                            <Label htmlFor="name" className="text-gray-400 text-xs uppercase tracking-wider font-mono">Name (Optional)</Label>
                                            <Input
                                                id="name"
                        ...
                        <div className="space-y-3">
                            {recruiters.length === 0 ? (
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-gray-600 text-sm">Invite recruiters to help manage applications</p>
                                </div>
                            ) : (
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            <Avatar className="w-10 h-10 rounded-sm">
                                                <AvatarImage src={recruiter.userId?.profile?.profilePhoto} />
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-500 font-mono">{recruiter.email}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <Badge className={`rounded-sm text-[10px] font-mono uppercase ${recruiter.status === 'active'
                                                ? 'bg-[#00FF94]/10 text-[#00FF94] border border-[#00FF94]/30'
                ...
                {pendingApprovals.length > 0 && (
                    ...
                    >
                        ...
                        <p className="text-gray-500 text-sm mb-6">Candidates passed by recruiters for your final decision</p>
                                ...
                                >
                                    <div className="flex items-start justify-between">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className="text-white font-semibold">{application.applicant?.fullname}</p>
                                                <p className="text-sm text-gray-500 font-mono">{application.applicant?.email}</p>
                                        ...
                                        <div className="text-right">
                                            <p className="text-xs text-gray-600 font-mono">Passed by</p>
                                            <p className="text-sm text-gray-400">
                                                {application.recruiterReview?.reviewedBy?.fullname || 'Recruiter'}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    {application.recruiterReview?.notes && (
                                        <div className="mt-3 p-3 bg-white/5 rounded-sm border border-white/5">
                                            <p className="text-xs text-gray-600 mb-1 font-mono uppercase tracking-wider">Recruiter Notes:</p>
                                            <p className="text-sm text-gray-400">{application.recruiterReview.notes}</p>
                                        </div>
                                    )}
                                    ...
                                    <div className="mt-4 flex items-center gap-3">
                                        ...
                                        <Button
                                            size="sm"
                                            variant="outline"
                                            onClick={() => navigate(`/ceo/reports/${application.interviewId}`)}
                                        ...
                                        <Button
                                            size="sm"
                                            onClick={() => handleApproval(application._id, 'approved')}
                                        ...
                                        <Button
                                            size="sm"
                                            variant="outline"
                                            onClick={() => handleApproval(application._id, 'rejected')}
                                            disabled={processingApproval === application._id}
                ...
                >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                            ...
                            { icon: Eye, label: 'Applications', route: '/admin/applications' },
                            ...
                            >
                                <action.icon className="w-6 h-6 text-[#FFD700] group-hover:scale-110 transition-transform" />
                                <span className="text-gray-400 text-xs font-mono uppercase tracking-wider">{action.label}</span>
                            </Button>
                        ))}
                    </div>
                </motion.div>
