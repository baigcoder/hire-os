...
import {
    Building, Users, Briefcase, TrendingUp, Award, Calendar,
    Mail, Phone, Globe, MapPin, Crown, Zap, CheckCircle,
    XCircle, Clock, DollarSign, Download, Upload, Edit,
    Plus, Send, MoreVertical, Eye, Trash2, RefreshCw,
    BarChart3, PieChart, Activity, Target, Sparkles,
    Shield, AlertCircle, ArrowUpRight, Settings
} from 'lucide-react';
...
const CompanyDashboard = () => {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    setCompanyData({
                        ...
                        logo: data.logo,
                        subscription: {
                            plan: data.subscription?.planDetails?.name?.toLowerCase() || 'basic',
                            billingCycle: data.subscription?.planDetails?.billingCycle || 'monthly',
    ...
    const handleInviteRecruiter = async () => {
        ...
        if (companyData.subscription.features.maxRecruiters !== -1 &&
            companyData.usage.activeRecruiters >= companyData.subscription.features.maxRecruiters) {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || "Failed to invite recruiter");
        }
    };

    if (loading) {
        return <DashboardLoader type="recruiter" message="LOADING COMPANY DASHBOARD" />;
    }

    if (!companyData) return null;

    // Calculate usage percentages
    const jobUsagePercent = companyData.subscription.features.maxJobPostings === -1 ? 0 : (companyData.usage.jobsPosted / companyData.subscription.features.maxJobPostings) * 100;
    const recruiterUsagePercent = companyData.subscription.features.maxRecruiters === -1 ? 0 : (companyData.usage.activeRecruiters / companyData.subscription.features.maxRecruiters) * 100;
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <Badge className={`${getPlanColor(companyData.subscription.plan)} capitalize`}>
                                ...
                                <div className="flex flex-wrap items-center gap-4 text-sm">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-blue-50">{companyData.companyEmail}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Phone className="h-4 w-4" />
                                        <span className="text-blue-50">{companyData.companyPhone}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <Card className="border-l-4 border-l-blue-500 hover:shadow-lg transition-shadow">
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                                {companyData.subscription.features.maxJobPostings === -1 ? 'Unlimited' : `of ${companyData.subscription.features.maxJobPostings} limit`}
                            </p>
                    ...
                    <Card className="border-l-4 border-l-purple-500 hover:shadow-lg transition-shadow">
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                                {companyData.subscription.features.maxRecruiters === -1 ? 'Unlimited' : `of ${companyData.subscription.features.maxRecruiters} limit`}
                            </p>
                                    ...
                                    <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border border-purple-200">
                                        <div className="flex items-center justify-between mb-3">
                                            <div>
                                                ...
                                                <p className="text-2xl font-bold capitalize">{companyData.subscription.plan}</p>
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between text-sm">
                                            ...
                                            <span className="font-semibold">{formatDate(companyData.subscription.endDate)}</span>
                                        </div>
                                    </div>
                            ...
                            <CardContent>
                                <div className="flex gap-3">
                                    <Input
                                        ...
                                        onKeyPress={(e) => e.key === 'Enter' && handleInviteRecruiter()}
                                    />
                                ...
                                <p className="text-xs text-gray-500 mt-2">
                                    {companyData.usage.activeRecruiters} of {companyData.subscription.features.maxRecruiters === -1 ? 'Unlimited' : companyData.subscription.features.maxRecruiters} recruiters used
                                </p>
                            </CardContent>
                        </Card>
                                    ...
                                    {companyData.recruiters.length === 0 ? (
                                        ...
                                        companyData.recruiters.map((recruiter) => (
                                            ...
                                            >
                                                <div className="flex items-center gap-4">
                                                    <Avatar>
                                                        <AvatarFallback className="bg-blue-100 text-blue-700">
                                                            {recruiter.name ? recruiter.name.charAt(0) : recruiter.email.charAt(0).toUpperCase()}
                                                        </AvatarFallback>
                                                    </Avatar>
                                                    <div>
                                                        <p className="font-semibold">{recruiter.name || 'Invited User'}</p>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <Badge className={recruiter.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}>
                                                        {recruiter.status}
                                                    </Badge>
                            ...
                            <CardContent className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <p className="text-xl font-bold capitalize">{companyData.subscription.plan}</p>
                                    </div>
                                    <div>
                                        <p className="text-sm text-gray-500 mb-1">Billing Cycle</p>
                                        <p className="text-xl font-bold capitalize">{companyData.subscription.billingCycle}</p>
                                    </div>
                                    <div>
                                        <p className="text-sm text-gray-500 mb-1">Start Date</p>
                                        <p className="font-semibold">{formatDate(companyData.subscription.startDate)}</p>
                                    </div>
                                    <div>
                                        <p className="text-sm text-gray-500 mb-1">Next Billing Date</p>
                                        <p className="font-semibold">{formatDate(companyData.subscription.endDate)}</p>
                                    </div>
                                </div>
                                ...
                                <div className="border-t pt-4">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-2xl font-bold">{formatPrice(companyData.subscription.amount)}</span>
                                    </div>
                                </div>
                            </CardContent>
                                ...
                                <div className="space-y-3">
                                    {companyData.billingHistory.length === 0 ? (
                                        ...
                                        companyData.billingHistory.map((bill) => (
                                            ...
                                            >
                                                <div>
                                                    <p className="font-semibold">{bill.plan} - {bill.duration}</p>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div className="text-right">
                                                        <p className="font-bold">{formatPrice(bill.amount)}</p>
