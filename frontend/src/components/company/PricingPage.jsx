...
import {
    Crown, Check, X, Zap, Building2, Users,
    Briefcase, Video, Brain, BarChart3, Shield,
...
const PricingPage = () => {
    ...
    const handleSelectPlan = (plan) => {
        ...
        navigate('/admin/signup', { state: { plan: plan.id, billing: billingCycle } });
    };
                ...
                <div className="container mx-auto px-4 relative z-10">
                    ...
                    >
                        <button
                            ...
                            className={`px-4 py-2 rounded-sm font-medium text-xs uppercase tracking-wider transition-all ${billingCycle === 'monthly'
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-sm font-medium text-xs uppercase tracking-wider transition-all ${billingCycle === 'yearly'
                                ? 'bg-[#FFD700] text-black'
                                : 'bg-[#111111] text-gray-500 hover:bg-white/10 border border-white/10'
                                }`}
                        >
                    ...
                    <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
                        {plans.map((plan, index) => (
                            ...
                            >
                                ...
                                <div className={`h-full transition-all duration-300 rounded-sm border overflow-hidden ${plan.popular
                                    ? 'bg-[#111111] border-[#FFD700]/50 shadow-[0_0_30px_rgba(255,215,0,0.1)]'
                                    : 'bg-[#111111] border-white/10 hover:border-[#FFD700]/30'
                                    }`}>

                                    {/* HUD Corner Accents */}
                                    <div className={`absolute top-0 right-0 w-4 h-4 border-t border-r ${plan.popular ? 'border-[#FFD700]/50' : 'border-white/10'}`} />
                                    <div className={`absolute bottom-0 left-0 w-4 h-4 border-b border-l ${plan.popular ? 'border-[#FFD700]/50' : 'border-white/10'}`} />

                                    {/* Header */}
                                    <div className="text-center p-6 border-b border-white/5">
                                        <div className={`w-12 h-12 mx-auto mb-3 rounded-sm flex items-center justify-center ${plan.popular
                                            ? 'bg-[#FFD700]'
                                            : 'bg-white/5 border border-white/10'
                                            }`}>
                                            {plan.id === 'basic' && <Building2 className={`w-6 h-6 ${plan.popular ? 'text-black' : 'text-[#FFD700]'}`} />}
                                            {plan.id === 'professional' && <Crown className={`w-6 h-6 ${plan.popular ? 'text-black' : 'text-[#FFD700]'}`} />}
                                            {plan.id === 'enterprise' && <Zap className={`w-6 h-6 ${plan.popular ? 'text-black' : 'text-[#FFD700]'}`} />}
                                        </div>
                                        <h3 className="text-lg font-bold text-white uppercase tracking-wider">{plan.name}</h3>
                                        <p className="text-gray-600 text-xs font-mono uppercase">{plan.description}</p>
                                        ...
                                        <div className="mt-4">
                                            ...
                                            {billingCycle === 'yearly' && (
                                                <p className="text-[10px] text-[#00FF94] mt-1 font-mono">
                                                    SAVE {calculateDiscount(plan.prices.monthly, plan.prices.yearly)}%
                                                </p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-6 space-y-3">
                                        {featuresList.map((feature) => {
                                            ...
                                            const hasFeature = value === true || typeof value === 'number' || value === 'Unlimited';

                                            return (
                                                <div key={feature.key} className={`flex items-center gap-2 ${hasFeature ? 'text-gray-400' : 'text-gray-700'}`}>
                                                    {hasFeature ? (
                                        ...
                                        <Button
                                            ...
                                            className={`w-full mt-6 py-5 font-bold text-xs uppercase tracking-wider rounded-sm transition-all ${plan.popular
