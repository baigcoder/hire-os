...
import {
    User, Mail, Phone, Building2, Briefcase, Calendar, Crown,
    Shield, Camera, Save, Edit3, Loader2, CheckCircle, X,
    CreditCard, Users, Clock, Star
} from 'lucide-react';

const USER_API = 'http://localhost:8000/api/v1/user';
const COMPANY_API = 'http://localhost:8000/api/v1/company';
...
const RecruiterProfile = () => {
        ...
        try {
            ...
            if (userResponse.data.success) {
                ...
                setProfile({
                    ...
                    role: userData.role || '',
                    profilePhoto: userData.profile?.profilePhoto || userData.profilePhoto || '',
    ...
    const getRoleDisplayName = (role) => {
        switch (role) {
            case 'company_admin': return 'Company Admin / CEO';
            case 'recruiter': return 'Recruiter';
            case 'student': return 'Job Seeker';
            default: return role;
        }
    };
    ...
    const getRoleIcon = (role) => {
        switch (role) {
            case 'company_admin': return Crown;
        ...
        return (
            <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-500 font-mono text-xs uppercase tracking-wider">Loading Profile...</p>
                </div>
            </div>
        );
    }
                ...
                >
                    ...
                    <div className="relative mb-8 rounded-md overflow-hidden border border-white/10">
                        ...
                        <div className="bg-[#111111] px-6 py-4 flex items-center justify-between">
                            <div className="flex items-center gap-4 -mt-10">
                                ...
                                <div className="mt-6">
                                    <h1 className="text-xl font-bold text-white uppercase tracking-tight">{profile.fullname}</h1>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        ...
                        <Card className="lg:col-span-1 bg-[#111111] border-white/10 rounded-md">
                            <CardContent className="pt-6">
                                <div className="flex flex-col items-center text-center">
                                    ...
                                    <h2 className="text-lg font-bold text-white mt-4 uppercase tracking-tight">{profile.fullname}</h2>
                                    <p className="text-gray-500 text-xs font-mono">{profile.email}</p>
                                    ...
                                    <div className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-sm bg-[#FFD700]/10 text-[#FFD700] border border-[#FFD700]/30">
                                        ...
                                        <span className="text-xs font-bold uppercase tracking-wider">{getRoleDisplayName(profile.role)}</span>
                                    </div>
                                    ...
                                    {companyInfo && (
                                        <div className="mt-4 p-3 bg-white/5 rounded-sm w-full border border-white/5">
                                            <div className="flex items-center gap-2 text-gray-400">
                                                ...
                                                <span className="text-xs font-mono">{companyInfo.name}</span>
                                            </div>
                                        </div>
                                    )}
                                </div>
                        ...
                        <Card className="lg:col-span-2 bg-[#111111] border-white/10 rounded-md">
                            <CardHeader>
                                <CardTitle className="text-white text-sm uppercase tracking-wider">Account Details</CardTitle>
                                <CardDescription className="text-gray-500 text-xs font-mono">Personal information</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                {/* Full Name */}
                                <div className="space-y-2">
                                    <Label className="text-gray-500 text-[10px] uppercase tracking-wider">Full Name</Label>
                                    {isEditing ? (
                                        <Input
                                        ...
                                        <div className="flex items-center gap-3 p-3 bg-white/5 rounded-sm border border-white/5">
                                            ...
                                            <span className="text-white text-sm font-mono">{profile.fullname}</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                {/* Email (read-only) */}
                                <div className="space-y-2">
                                    <Label className="text-gray-500 text-[10px] uppercase tracking-wider">Email Address</Label>
                                    <div className="flex items-center gap-3 p-3 bg-white/5 rounded-sm border border-white/5">
                                ...
                                {/* Phone */}
                                <div className="space-y-2">
                                    <Label className="text-gray-500 text-[10px] uppercase tracking-wider">Phone Number</Label>
                                    {isEditing ? (
                                        <Input
                                        ...
                                        <div className="flex items-center gap-3 p-3 bg-white/5 rounded-sm border border-white/5">
                                            ...
                                            <span className="text-white text-sm font-mono">{profile.phoneNumber || 'Not set'}</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                {/* Role */}
                                <div className="space-y-2">
                                    <Label className="text-gray-500 text-[10px] uppercase tracking-wider">Role</Label>
                                    <div className="flex items-center gap-3 p-3 bg-white/5 rounded-sm border border-white/5">
                                        <RoleIcon className="w-4 h-4 text-[#FFD700]" />
                                        <span className="text-white text-sm font-mono">{getRoleDisplayName(profile.role)}</span>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        {profile.role === 'company_admin' && companyInfo?.subscription && (
                            <Card className="lg:col-span-3 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border-yellow-500/30">
                                <CardHeader>
                                    ...
                                    <CardDescription>Your company's subscription plan</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        ...
                                        <div className="p-4 bg-zinc-900/50 rounded-xl">
                                            ...
                                            <p className={`text-xl font-bold ${companyInfo.subscription.status === 'active' ? 'text-green-500' : 'text-red-500'}`}>
                                        ...
                                        <div className="p-4 bg-zinc-900/50 rounded-xl">
                                            ...
                                            <p className="text-xl font-bold text-white">
                                                {companyInfo.subscription.endDate
                                                    ? new Date(companyInfo.subscription.endDate).toLocaleDateString()
                                                    : 'N/A'}
                                            </p>
                                        </div>
                                        ...
                                        <div className="p-4 bg-zinc-900/50 rounded-xl">
                                            ...
                                            <p className="text-xl font-bold text-white">
                                                {companyInfo.recruiterCount || 0} / {companyInfo.subscription.maxRecruiters || '∞'}
                                            </p>
                                        </div>
                                    </div>

                                    {/* Features */}
                                    {companyInfo.subscription.features && companyInfo.subscription.features.length > 0 && (
                                        <div className="mt-6">
                                            <h4 className="text-white font-semibold mb-3">Plan Features</h4>
                                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                                {companyInfo.subscription.features.map((feature, idx) => (
                        ...
                        {profile.role === 'recruiter' && companyInfo && (
                            <Card className="lg:col-span-3 bg-zinc-900/50 border-zinc-800">
                                <CardHeader>
                                    ...
                                    <CardDescription>You are a recruiter at this company</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <div className="p-4 bg-zinc-800/50 rounded-xl">
                                            ...
                                            <p className="text-white font-semibold">{companyInfo.industry || 'Not specified'}</p>
                                        </div>
                                        <div className="p-4 bg-zinc-800/50 rounded-xl">
                                            <p className="text-gray-400 text-sm mb-1">Location</p>
                                            <p className="text-white font-semibold">{companyInfo.location || 'Not specified'}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
