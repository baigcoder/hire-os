...
const RecruiterApplications = ({ jobId }) => {
  ...
  if (applications.length === 0) {
    return <div className="text-center py-8 text-gray-600">No applications received yet.</div>;
  }
        ...
        {applications.map(application => (
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                <h4 className="text-lg font-semibold">{application.user.name}</h4>
                ...
                {application.resumeAnalysis && (
                  <div className="mt-2 flex items-center">
                    <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center">
                      <span className="text-indigo-700 font-bold">{application.resumeAnalysis.analysisScore}%</span>
                    </div>
                    <span className="ml-2 text-sm text-gray-600">Match Score</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {application.resumeAnalysis && (
              <div className="mt-4 p-4 bg-indigo-50 rounded-lg">
                <h5 className="font-medium text-indigo-800 mb-2">Resume Analysis</h5>
                
                <div className="grid grid-cols-2 gap-4 mb-3">
                  <div>
                    <p className="text-xs text-gray-500">Overall Fit</p>
                    <p className={`font-semibold ${
                      application.resumeAnalysis.overallFit === 'Excellent' ? 'text-green-600' :
                      application.resumeAnalysis.overallFit === 'Good' ? 'text-blue-600' :
                      application.resumeAnalysis.overallFit === 'Fair' ? 'text-yellow-600' : 'text-red-600'
                    }`}>
                ...
                {application.resumeAnalysis.keySkillsMatch && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {application.resumeAnalysis.keySkillsMatch.map((skill, index) => (
                        <span 
                          key={index}
