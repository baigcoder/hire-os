...
import {
    FileText, User, Briefcase, CheckCircle, XCircle, AlertTriangle,
    Clock, Brain, Video, Shield, MessageSquare, ThumbsUp, ThumbsDown,
    Send, Download, ArrowLeft, Star, TrendingUp, Eye
} from 'lucide-react';
...
const InterviewReport = () => {
    ...
    const fetchReport = async () => {
        try {
            const response = await axios.get(`${INTERVIEW_API_END_POINT}/${interviewId}/report`, {
        ...
        <div className="min-h-screen bg-black text-white font-['Outfit',sans-serif]">
            ...
            <div className="max-w-5xl mx-auto px-4 py-8 relative z-10">
                ...
                <motion.div
                    ...
                    className="mb-8"
                >
                    <Button variant="ghost" onClick={() => navigate(-1)} className="mb-4 text-gray-400">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        ...
                        {/* Status Badge */}
                        <Badge className={`text-sm px-4 py-2 ${report.status === 'approved' ? 'bg-green-500/20 text-green-400' :
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <CardContent className="pt-6">
                                <div className="flex items-center justify-between mb-6">
                                    <div>
                                        ...
                                        <p className="text-5xl font-bold text-white">{report.overallScore || 0}%</p>
                                    </div>
                                    <div className="w-24 h-24 rounded-full border-4 border-purple-500 flex items-center justify-center">
                                        <Star className={`w-10 h-10 ${report.overallScore >= 70 ? 'text-yellow-400' : 'text-gray-500'}`} />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-black/30 rounded-xl p-4">
                                        ...
                                        <p className="text-2xl font-bold text-white">{report.mcqScore || 0}%</p>
                                        <Progress value={report.mcqScore || 0} className="mt-2 h-1" />
                                    </div>
                                    <div className="bg-black/30 rounded-xl p-4">
                                        <div className="flex items-center gap-2 mb-2">
                                            <Video className="text-blue-400" size={18} />
                                            <span className="text-gray-400 text-sm">Interview Score</span>
                                        </div>
                                        <p className="text-2xl font-bold text-white">{report.interviewScore || 0}%</p>
                                        <Progress value={report.interviewScore || 0} className="mt-2 h-1" />
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                            ...
                            <CardContent>
                                <div className="flex items-center justify-between mb-4">
                                    <div>
                                        ...
                                        <Badge className={`mt-1 ${getRiskBadge(report.fraudRisk?.level || 'low')}`}>
                                            {(report.fraudRisk?.level || 'Low').toUpperCase()}
                                        </Badge>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-gray-400 text-sm">Risk Score</p>
                                        <p className="text-2xl font-bold text-white">{report.fraudRisk?.score || 0}/100</p>
                                    </div>
                                </div>
                                ...
                                {report.fraudRisk?.alerts?.length > 0 && (
                                    <div className="mt-4 space-y-2">
                                        <p className="text-sm text-gray-400 mb-2">Detected Alerts:</p>
                                        {report.fraudRisk.alerts.map((alert, idx) => (
                                            <div key={idx} className={`flex items-start gap-2 p-3 rounded-lg ${alert.severity === 'critical' ? 'bg-red-500/10 border border-red-500/30' :
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-gray-500 text-xs">{alert.details}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                {(!report.fraudRisk?.alerts || report.fraudRisk.alerts.length === 0) && (
                                        ...
                                        <div className="mt-4">
                                            ...
                                            <ul className="space-y-1">
                                                {report.weaknesses.map((w, i) => (
                                                    <li key={i} className="text-gray-400 text-sm flex items-center gap-2">
                                                        <AlertTriangle size={14} className="text-yellow-400" /> {w}
                                                    </li>
                                                ))}
                                            </ul>
                                        </div>
                    ...
                    >
                        ...
                        <Card className="bg-zinc-900/50 border-zinc-800">
                            <CardContent className="pt-6">
                                <div className="flex items-center gap-4 mb-4">
                                    ...
                                    <div>
                                        <p className="text-white font-semibold">{report.candidateName}</p>
                                        <p className="text-gray-500 text-sm">{report.candidateEmail}</p>
                                    </div>
                                </div>
                        ...
                        {isCompanyAdmin && report.status === 'submitted' && (
                            <Card className="bg-gradient-to-br from-yellow-500/10 to-orange-500/10 border-yellow-500/30">
                                <CardHeader>
                                    ...
                                    <CardDescription>Review and approve/reject this candidate</CardDescription>
                                </CardHeader>
                        ...
                        {report.adminDecision && (
                            <Card className={`border ${report.adminDecision.decision === 'approved'
                                ...
                                <CardContent className="pt-6">
                                    <div className="text-center">
                                        ...
                                        <p className="text-white font-semibold">
                                            {report.adminDecision.decision === 'approved' ? 'Approved' : 'Rejected'}
                                        </p>
                        ...
                        <div className="space-y-3">
                            <Button variant="outline" className="w-full border-zinc-700 text-gray-300">
            ...
            <Dialog open={decisionDialogOpen} onOpenChange={setDecisionDialogOpen}>
                <DialogContent className="bg-zinc-900 border-zinc-800">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Are you sure you want to {pendingDecision === 'approved' ? 'approve' : 'reject'} {report.candidateName}?
                            {pendingDecision === 'approved' && ' An offer letter process will be initiated.'}
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setDecisionDialogOpen(false)} className="border-zinc-700">
                            Cancel
                        </Button>
                        <Button
                            onClick={() => handleDecision(pendingDecision)}
                            disabled={submitting}
                            className={pendingDecision === 'approved' ? 'bg-green-500' : 'bg-red-500'}
                        >
