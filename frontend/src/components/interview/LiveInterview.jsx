...
import {
    ...
    Monitor, MonitorOff, BrainCircuit, Sparkles, AlertTriangle, CheckCircle,
    Users, Settings, Send, Loader2, WifiOff, Wifi, Copy, FileText
} from 'lucide-react';
...
const LiveInterview = () => {
    ...
    // Socket hook for room management and chat
    const {
        socket,
        isConnected,
        connectionError,
        ...
        endInterview,
        reportFraudAlert
    } = useSocket(interviewId);

    // Video call hook for WebRTC
    const {
        localStream,
        remoteStream,
        localVideoRef,
        ...
        stopScreenShare,
        cleanup
    } = useVideoCall(socket, interviewId);
    ...
    useEffect(() => {
        ...
        const feedbacks = [
            ...
            { type: 'neutral', text: 'Try to slow down your speaking pace slightly.' },
            { type: 'warning', text: 'Technical term explained vaguely.' },
            { type: 'positive', text: 'Excellent structured answer using STAR method.' }
        ];

        const interval = setInterval(() => {
            const randomFeedback = feedbacks[Math.floor(Math.random() * feedbacks.length)];
            setAiFeedback(prev => [randomFeedback, ...prev].slice(0, 3));
            setConfidenceScore(prev => Math.min(100, Math.max(60, prev + (Math.random() * 10 - 5))));
        }, 10000);
        ...
        return (
            <div className="h-screen bg-[#0A0A0A] text-white flex items-center justify-center font-['Space_Grotesk',sans-serif]">
                ...
                >
                    ...
                    <h1 className="text-2xl font-bold mb-2 uppercase tracking-tight">Waiting Room</h1>
                    ...
                    {/* Share Link */}
                    <Button variant="outline" onClick={copyMeetingLink} className="border-white/10 mt-4 rounded-sm text-xs uppercase tracking-wider">
        ...
        <div className="h-screen bg-[#0A0A0A] text-white flex flex-col font-['Space_Grotesk',sans-serif] overflow-hidden">
            ...
            <div className="h-14 border-b border-white/10 bg-[#111111] flex items-center justify-between px-6 shrink-0 relative z-20">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="font-bold text-xs text-gray-300 uppercase tracking-wider">Interview Session</h2>
                        <p className="text-[10px] text-gray-500 font-mono">{formatTime(timer)} • {participants.length} Connected</p>
                    </div>
                </div>

                <div className="flex items-center gap-4">
                    {/* Connection Status */}
                    <Badge className={`rounded-sm text-[10px] uppercase tracking-wider font-mono ${isConnected ? 'bg-[#00FF94]/10 text-[#00FF94] border-[#00FF94]/30' : 'bg-red-500/10 text-red-400 border-red-500/30'}`}>
                    ...
                    <Button variant="destructive" className="bg-red-600 hover:bg-red-700 rounded-full px-6" onClick={handleEndCall}>
                        <PhoneOff className="mr-2 h-4 w-4" /> {isRecruiter ? 'End & Report' : 'End Call'}
                    </Button>
                </div>
            </div>
            ...
            <div className="flex-1 flex overflow-hidden relative">
                ...
                <div className="flex-1 bg-[#050505] p-4 flex flex-col gap-4 relative">
                    ...
                    <div className="flex-1 relative rounded-2xl overflow-hidden border border-white/10 bg-zinc-900 group">
                        ...
                        <div className="absolute bottom-6 left-6 z-20">
                            <h3 className="text-xl font-bold text-white mb-1">
                                {otherParticipant?.fullname || (isRecruiter ? 'Candidate' : 'Interviewer')}
                            </h3>
                            <p className="text-sm text-gray-400 flex items-center gap-2">
                                <Badge className="bg-blue-500/20 text-blue-400 border-none capitalize">
                                    {otherParticipant?.role || 'Participant'}
                                </Badge>
                                {otherParticipant?.isAudioOn === false && <MicOff size={14} className="text-red-400" />}
                            </p>
                        </div>
                                ...
                                {aiFeedback.map((fb, idx) => (
                                    <motion.div
                                        ...
                                        className={`p-3 rounded-lg backdrop-blur-md border border-white/10 text-xs font-medium shadow-lg
                                            ${fb.type === 'positive' ? 'bg-green-500/10 text-green-300' :
                                                fb.type === 'warning' ? 'bg-red-500/10 text-red-300' : 'bg-blue-500/10 text-cyan-300'}`}
                                    >
                                        <div className="flex gap-2">
                                            {fb.type === 'positive' ? <CheckCircle size={14} /> :
                                                fb.type === 'warning' ? <AlertTriangle size={14} /> : <Sparkles size={14} />}
                                            {fb.text}
                                        </div>
                                    </motion.div>
                                ))}
                    ...
                    <div className="flex justify-center gap-4 py-2">
                        ...
                        <Button variant="ghost" className="rounded-full h-12 w-12 p-0 text-gray-400 hover:text-white">
                            <Settings size={20} />
                        </Button>
                        <Button
                            variant="ghost"
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                ...
                                {messages.map((msg, idx) => (
                                    <div key={msg.id || idx} className={`flex flex-col ${msg.senderId === user?._id || msg.senderName === user?.fullname ? 'items-end' : 'items-start'}`}>
                                        <p className="text-[10px] text-gray-500 mb-1">{msg.senderName}</p>
                                        <div className={`max-w-[85%] p-3 rounded-2xl text-sm ${msg.senderId === user?._id || msg.senderName === user?.fullname
