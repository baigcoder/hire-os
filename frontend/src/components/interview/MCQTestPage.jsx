...
import {
    Brain, Clock, ChevronLeft, ChevronRight, CheckCircle, XCircle,
    AlertTriangle, Play, Timer, Flag, Send, Eye
} from 'lucide-react';
...
const MCQTestPage = () => {
    ...
    const fetchTestData = async () => {
        try {
            const response = await axios.get(`${INTERVIEW_API_END_POINT}/${interviewId}/mcq`, {
            ...
            if (response.data.success) {
                ...
                } else if (['completed', 'passed', 'failed'].includes(response.data.status)) {
    ...
    const fetchResult = async () => {
        try {
            const response = await axios.get(`${INTERVIEW_API_END_POINT}/${interviewId}/mcq/result`, {
                withCredentials: true
            });
            if (response.data.success) {
    ...
    const startTest = async () => {
        try {
            const response = await axios.post(`${INTERVIEW_API_END_POINT}/${interviewId}/mcq/start`, {}, {
                withCredentials: true
            });
            if (response.data.success) {
                setTestStarted(true);
    ...
    const reportFraudAlert = async (type, details) => {
        try {
            await axios.post(`${INTERVIEW_API_END_POINT}/${interviewId}/video/fraud-alert`, {
                type,
                details,
    ...
    const submitTest = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${INTERVIEW_API_END_POINT}/${interviewId}/mcq/submit`, {
            ...
            if (response.data.success) {
                ...
                if (response.data.result.passed) {
                    ...
                    toast.error('Unfortunately, you did not pass. Better luck next time!');
                }
            }
        } catch (error) {
        ...
        return (
            <div className="min-h-screen bg-black">
                ...
                <div className="container mx-auto px-4 py-16">
                    ...
                    >
                        <Card className={`text-center ${result.passed
                    ...
                    >
                        <Card className="bg-zinc-900/50 border-zinc-800">
                            <CardHeader className="text-center">
                                ...
                                <CardTitle className="text-2xl text-white">MCQ Assessment</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-zinc-800/50 rounded-xl p-4 text-center">
                                        <p className="text-3xl font-bold text-white">{testData?.totalQuestions}</p>
                                        <p className="text-sm text-gray-500">Questions</p>
                                    </div>
                                    <div className="bg-zinc-800/50 rounded-xl p-4 text-center">
                                        <p className="text-3xl font-bold text-white">{testData?.timeLimit} min</p>
                                ...
                                <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <p className="text-white font-medium">Important Instructions</p>
                                            <ul className="text-sm text-gray-400 mt-2 space-y-1">
                                                ...
                                                <li>• Minimum {testData?.passingScore}% required to pass</li>
            ...
            <div className="fixed top-0 left-0 right-0 z-50 bg-zinc-900 border-b border-zinc-800">
                <div className="container mx-auto px-4 py-3">
                    <div className="flex items-center justify-between">
                        ...
                        <div className={`flex items-center gap-2 px-4 py-2 rounded-lg ${timeLeft < 60 ? 'bg-red-500/20 text-red-400 animate-pulse' : 'bg-zinc-800 text-white'
                            }`}>
                            ...
                            <Card className="bg-zinc-900/50 border-zinc-800">
                                <CardHeader>
                                    <div className="flex items-center justify-between mb-4">
                                        <Badge className={`${currentQuestion?.difficulty === 'easy' ? 'bg-green-500/20 text-green-400' :
