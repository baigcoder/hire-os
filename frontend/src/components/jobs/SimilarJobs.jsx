...
import {
    Building2, MapPin, ArrowRight, Loader2, Sparkles
} from 'lucide-react';
...
const SimilarJobs = ({ jobId, limit = 4 }) => {
    ...
    const fetchSimilarJobs = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${API_BASE}/job/${jobId}/similar?limit=${limit}`);
            if (res.data.success) {
    ...
    return (
        <div className="bg-[#0a0a0a] border border-white/10 rounded-sm p-5">
            ...
            <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                    <h3 className="text-sm font-bold text-white tracking-tight">SIMILAR JOBS</h3>
                    <p className="text-[10px] text-gray-500 font-mono tracking-wider">BASED ON YOUR INTERESTS</p>
                </div>
            </div>
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-sm bg-white/5 flex items-center justify-center border border-white/10">
                                {job.company?.logo ? (
                                    <img src={job.company.logo} alt="" className="w-7 h-7 object-contain" />
                                ) : (
