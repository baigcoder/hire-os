...
import {
    Bell, BellOff, Check, CheckCheck, Trash2, Calendar, Briefcase,
    Video, Gift, AlertTriangle, MessageSquare, CreditCard, Users,
...
const NotificationItem = ({ notification, onRead, onDelete, onClick }) => {
    ...
    return (
        <motion.div
            ...
            className={`p-4 border-b border-zinc-800 hover:bg-zinc-800/50 transition-colors cursor-pointer ${!notification.read ? 'bg-zinc-900/50' : ''
                }`}
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                        ...
                        {notification.priority === 'urgent' && (
                            <Badge className="text-xs bg-red-500/20 text-red-400 px-2 py-0">Urgent</Badge>
                        )}
                        {notification.priority === 'high' && (
                            <Badge className="text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0">Important</Badge>
                        )}
                    </div>
                </div>
                <DropdownMenu>
...
export const NotificationBell = () => {
    ...
    const fetchNotifications = async () => {
        try {
            const response = await axios.get(`${NOTIFICATION_API_END_POINT}?limit=5`, {
    ...
    const handleMarkAsRead = async (id) => {
        try {
            await axios.put(`${NOTIFICATION_API_END_POINT}/${id}/read`, {}, { withCredentials: true });
...
const NotificationCenter = () => {
    ...
    const fetchNotifications = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${NOTIFICATION_API_END_POINT}?${params}`, {
                withCredentials: true
            });
                ...
                >
                    ...
                    <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); setPage(1); }}>
                        <TabsList className="bg-zinc-900 border border-zinc-800 mb-6">
                            ...
                            <TabsTrigger value="unread" className="data-[state=active]:bg-yellow-500 data-[state=active]:text-black">
