...
import {
    BarChart3, TrendingUp, Clock, Users, Target, Calendar,
    ArrowUpRight, ArrowDownRight, Download, RefreshCw, Filter
} from 'lucide-react';
...
const MetricCard = ({ title, value, change, icon: Icon, color = '#FFD700' }) => {
    ...
    return (
        ...
        >
            <div className="flex items-start justify-between">
                <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider font-mono">{title}</p>
                    <p className="text-3xl font-bold text-white font-mono mt-2">{value}</p>
                    {change !== undefined && (
                        <div className={`flex items-center gap-1 mt-2 ${isPositive ? 'text-[#00FF94]' : 'text-red-400'}`}>
        ...
        <div className="flex items-center gap-6">
            ...
            <div className="w-20">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono text-gray-400">{conversionRate}%</span>
                </div>
            </div>
        </div>
    </div>
...
const RecruiterCard = ({ recruiter }) => (
    ...
    >
        <div className="flex items-center gap-3 mb-4">
            ...
            <div>
                <p className="text-sm font-semibold text-white">{recruiter.user?.fullname}</p>
        ...
        <div className="grid grid-cols-4 gap-2">
            <div className="text-center p-2 bg-white/5 rounded-sm">
                <p className="text-lg font-bold text-white font-mono">{recruiter.stats?.jobsPosted || 0}</p>
                <p className="text-[10px] text-gray-500">Jobs</p>
            </div>
            <div className="text-center p-2 bg-white/5 rounded-sm">
                <p className="text-lg font-bold text-white font-mono">{recruiter.stats?.interviewsConducted || 0}</p>
                <p className="text-[10px] text-gray-500">Interviews</p>
            </div>
            <div className="text-center p-2 bg-white/5 rounded-sm">
                <p className="text-lg font-bold text-[#00FF94] font-mono">{recruiter.stats?.successfulHires || 0}</p>
                <p className="text-[10px] text-gray-500">Hires</p>
            </div>
            <div className="text-center p-2 bg-white/5 rounded-sm">
                <p className="text-lg font-bold text-[#FFD700] font-mono">{recruiter.stats?.hireRate || 0}%</p>
...
const AnalyticsDashboard = () => {
    ...
    const fetchAnalytics = async () => {
        try {
            setLoading(true);
            const [overviewRes, tthRes, sourceRes, funnelRes, recruiterRes] = await Promise.all([
                axios.get(`${RECRUITER_ANALYTICS_API_END_POINT}/overview`, { withCredentials: true }),
                axios.get(`${RECRUITER_ANALYTICS_API_END_POINT}/time-to-hire`, { withCredentials: true }),
                axios.get(`${RECRUITER_ANALYTICS_API_END_POINT}/source`, { withCredentials: true }),
                axios.get(`${RECRUITER_ANALYTICS_API_END_POINT}/funnel`, { withCredentials: true }),
                axios.get(`${RECRUITER_ANALYTICS_API_END_POINT}/recruiter-performance`, { withCredentials: true }).catch(() => ({ data: { success: false } }))
            ]);
            ...
            if (recruiterRes.data.success) setRecruiters(recruiterRes.data.recruiters || []);
        } catch (error) {
    ...
    const funnelColors = ['#FFD700', '#F59E0B', '#10B981', '#3B82F6', '#8B5CF6', '#00FF94'];
        ...
        <div className="min-h-screen bg-[#0A0A0A] font-['Space_Grotesk',sans-serif]">
            ...
            <div className="container mx-auto px-4 py-8 relative z-10">
                    ...
                    <div className="flex items-center gap-3">
                        <Select value={dateRange} onValueChange={setDateRange}>
                            ...
                            <SelectContent className="bg-[#111111] border-white/10">
                                <SelectItem value="7" className="text-gray-300">Last 7 days</SelectItem>
                                <SelectItem value="30" className="text-gray-300">Last 30 days</SelectItem>
                                <SelectItem value="90" className="text-gray-300">Last 90 days</SelectItem>
                            </SelectContent>
                        </Select>

                        <Button variant="outline" className="border-white/10 text-gray-400" onClick={fetchAnalytics}>
                ...
                <div className="grid lg:grid-cols-2 gap-6 mb-8">
                    ...
                    >
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-lg font-bold text-white">Application Funnel</h3>
                                <p className="text-gray-500 text-sm">Conversion through stages</p>
                            </div>
                        </div>
                        ...
                        <div className="mt-6 grid grid-cols-5 gap-2">
                            {funnel.slice(1).map((stage, idx) => (
                                <div key={idx} className="text-center">
                                    ...
                                    <p className="text-sm font-mono text-[#FFD700]">{stage.conversionRate}%</p>
                                </div>
                            ))}
                        </div>
                    </motion.div>
                    ...
                    >
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p className="text-gray-500 text-sm">Days from application to hire</p>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            <div className="p-4 bg-white/5 rounded-sm text-center">
                                <p className="text-3xl font-bold text-white font-mono">{timeToHire?.average || 0}</p>
                                <p className="text-xs text-gray-500 mt-1">Average Days</p>
                            </div>
                            <div className="p-4 bg-white/5 rounded-sm text-center">
                                <p className="text-3xl font-bold text-[#00FF94] font-mono">{timeToHire?.fastest || 0}</p>
                        ...
                        {timeToHire?.byJob?.length > 0 && (
                            <div>
                                <p className="text-xs text-gray-500 uppercase tracking-wider mb-3">By Job</p>
                                <div className="space-y-2">
                                    {timeToHire.byJob.slice(0, 4).map((job, idx) => (
                                        <div key={idx} className="flex items-center justify-between py-2 border-b border-white/5">
                                            <span className="text-sm text-gray-300 truncate max-w-[60%]">{job.job}</span>
                                            <div className="flex items-center gap-3">
                                                <span className="text-sm text-gray-500">{job.hires} hires</span>
                                                <span className="text-sm font-mono text-[#FFD700]">{job.averageDays}d</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                ...
                >
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h3 className="text-lg font-bold text-white">Application Sources</h3>
                            <p className="text-gray-500 text-sm">Where your candidates come from</p>
                        </div>
                    </div>
                ...
                {recruiters.length > 0 && (
                    ...
                    >
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-lg font-bold text-white">Recruiter Performance</h3>
                                <p className="text-gray-500 text-sm">Individual metrics for team members</p>
                            </div>
                        </div>
