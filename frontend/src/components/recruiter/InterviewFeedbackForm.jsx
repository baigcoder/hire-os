...
import {
    MessageSquare, Star, ThumbsUp, ThumbsDown, Save,
    Send, User, Briefcase, Clock, CheckCircle, X, Plus
} from 'lucide-react';
        ...
        <div className="space-y-2">
            ...
            <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                        <Star
                            className={`w-6 h-6 transition-colors ${star <= (value || 0)
...
const ScorecardSection = ({ scorecard, onChange }) => {
    const categories = [
        { key: 'technicalSkills', label: 'Technical Skills', description: 'Problem-solving, coding, domain knowledge' },
        { key: 'communication', label: 'Communication', description: 'Clarity, articulation, listening skills' },
        { key: 'problemSolving', label: 'Problem Solving', description: 'Analytical thinking, approach to challenges' },
        { key: 'cultureFit', label: 'Culture Fit', description: 'Values alignment, team compatibility' },
        { key: 'experience', label: 'Relevant Experience', description: 'Past work, industry knowledge' },
        { key: 'enthusiasm', label: 'Enthusiasm', description: 'Interest in role, motivation' }
    ];
    ...
    return (
        <div className="space-y-6">
            {categories.map((cat) => (
                <div key={cat.key} className="p-4 bg-[#0A0A0A] border border-white/10 rounded-sm">
                    <RatingInput
...
const InterviewFeedbackForm = ({
    ...
    const handleSubmit = async () => {
        ...
        const hasScores = Object.values(formData.scorecard).some(cat => cat.score > 0);
        if (!hasScores) {
    ...
    const overallScore = (() => {
        const scores = Object.values(formData.scorecard).map(cat => cat.score).filter(s => s > 0);
            ...
            <div className="container mx-auto px-4 max-w-3xl">
                ...
                >
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white">{candidate?.fullname || 'Candidate'}</h2>
                ...
                >
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-4xl font-bold text-[#FFD700] font-mono mt-1">{overallScore}</p>
                        ...
                        <div className="flex gap-1">
                            {[1, 2, 3, 4, 5].map((star) => (
                                <Star
                                    key={star}
                                    className={`w-8 h-8 ${star <= Math.round(overallScore)
                                            ? 'text-[#FFD700] fill-[#FFD700]'
                                            : 'text-gray-700'
                                        }`}
                                />
                ...
                >
                    ...
                    <ScorecardSection
                        ...
                        onChange={(scorecard) => setFormData(prev => ({ ...prev, scorecard }))}
                    />
                </motion.div>
                ...
                <motion.div
                    ...
                    className="p-6 bg-[#111111] border border-white/10 rounded-sm mb-6"
                >
                    <h3 className="text-sm font-bold text-white mb-4">Final Recommendation *</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {[
                            ...
                            { value: 'strong_no_hire', label: 'Strong No Hire', color: '#EF4444' }
                        ].map((rec) => (
                            <button
                                key={rec.value}
                                type="button"
                                onClick={() => setFormData(prev => ({ ...prev, recommendation: rec.value }))}
                                className={`p-4 border rounded-sm text-center transition-all ${formData.recommendation === rec.value
                            ...
                            >
                                ...
                                />
                                <p className={`text-sm ${formData.recommendation === rec.value
