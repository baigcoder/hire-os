...
import {
    Briefcase, Users, TrendingUp, Clock, Target, Zap,
    Calendar, Mail, BarChart3, FileText, Video, Plus,
    ArrowUpRight, ArrowDownRight, Activity, Bell, Settings,
    ChevronRight, Eye, CheckCircle, XCircle, UserCheck,
    Building2, Filter, Download, RefreshCw
} from 'lucide-react';
...
const StatCard = ({ label, value, change, changeLabel, icon: Icon, color, delay = 0 }) => {
        ...
        >
            ...
            <div className="flex items-start justify-between">
                <div>
                    ...
                    {change !== undefined && (
                        <div className={`flex items-center gap-1 mt-2 text-sm ${isPositive ? 'text-[#00FF94]' : 'text-red-400'}`}>
...
const PipelineStage = ({ stage, count, conversionRate, color, isLast }) => (
    <div className="flex items-center">
        <div className="flex flex-col items-center">
            ...
            <p className="text-xs text-gray-400 mt-2 text-center max-w-[80px]">{stage}</p>
            {conversionRate > 0 && (
                <p className="text-[10px] text-[#FFD700] font-mono mt-1">{conversionRate}%</p>
            )}
        </div>
        {!isLast && (
...
const ActivityItem = ({ activity }) => {
    const getActivityIcon = (type) => {
        switch (type) {
            case 'new_application': return { icon: FileText, color: '#FFD700' };
            case 'status_change': return { icon: RefreshCw, color: '#00FF94' };
            case 'interview_scheduled': return { icon: Calendar, color: '#3B82F6' };
        ...
        <div className="flex items-start gap-3 p-3 hover:bg-white/5 rounded-sm transition-colors">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-white truncate">{activity.applicant?.name}</span>
                </div>
                <p className="text-xs text-gray-500 mt-1">
                    {activity.type === 'new_application' ? 'Applied for' : 'Updated status on'}
            ...
            <Badge className={`text-[10px] rounded-sm ${activity.status === 'hired' ? 'bg-[#00FF94]/10 text-[#00FF94]' :
                activity.status === 'rejected' ? 'bg-red-500/10 text-red-400' :
                    activity.status === 'interview' ? 'bg-blue-500/10 text-blue-400' :
...
const RecruiterDashboardEnhanced = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            setLoading(true);
            const [overviewRes, funnelRes, activityRes, companyRes] = await Promise.all([
                ...
                axios.get(`${RECRUITER_ANALYTICS_API_END_POINT}/activity?limit=10`, { withCredentials: true }),
                axios.get(`${COMPANY_API_END_POINT}/dashboard`, { withCredentials: true })
            ]);
    ...
    if (loading) {
        return <DashboardLoader type="recruiter" message="LOADING RECRUITER DASHBOARD..." />;
    }
                ...
                >
                    <div className="flex items-center gap-4">
                        <div className="w-16 h-16 bg-[#FFD700] rounded-sm flex items-center justify-center">
                            {companyData?.logo ? (
                                <img src={companyData.logo} alt="Company" className="w-full h-full object-cover rounded-sm" />
                            ) : (
                ...
                >
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-gray-500 text-sm">Track candidates through hiring stages</p>
                        </div>
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {[
                                    ...
                                    { icon: FileText, label: 'Applications', path: '/admin/applications', color: '#3B82F6' },
                                    { icon: Mail, label: 'Email Templates', path: '/admin/email-templates', color: '#8B5CF6' },
                                    { icon: BarChart3, label: 'Analytics', path: '/admin/analytics', color: '#00FF94' }
                                ].map((action, idx) => (
                                    <motion.button
                                        key={idx}
                                    ...
                                    >
                                        <action.icon className="w-6 h-6 group-hover:scale-110 transition-transform" style={{ color: action.color }} />
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between p-3 bg-white/5 rounded-sm">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm text-gray-300">Applications to Review</span>
                                    </div>
                                ...
                                <div className="flex items-center justify-between p-3 bg-white/5 rounded-sm">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm text-gray-300">Upcoming Interviews</span>
                                    </div>
