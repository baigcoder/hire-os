...
import { Building, Briefcase, User, LogOut, Menu, X, ChevronDown, Bell, Search, CreditCard, Terminal, PenLine } from 'lucide-react'
...
const Navbar = () => {
    ...
    const navLinks = isStudent ? [
        ...
        { path: `/recruiter/${nameSlug}`, label: 'Dashboard', icon: Terminal },
    ...
    return (
        <>
            <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 font-['Space_Grotesk',sans-serif] ${isScrolled ? 'bg-[#0A0A0A]/90 backdrop-blur-lg border-b border-white/5 py-3' : 'bg-transparent py-5'}`}>
                <div className="container mx-auto px-4 flex justify-between items-center">
                    ...
                    <div className="hidden md:flex items-center gap-6">
                        ...
                        {!isLoggedIn && (
                            <>
                                <Link to="/browse" className={`text-sm font-medium transition-colors uppercase tracking-wide ${isActive('/browse') ? 'text-[#FFD700]' : 'text-gray-400 hover:text-white'}`}>
                                    Browse
                                </Link>
                                <Link to="/company/pricing" className={`text-sm font-medium transition-colors uppercase tracking-wide ${isActive('/company/pricing') ? 'text-[#FFD700]' : 'text-gray-400 hover:text-white'}`}>
                                    Pricing
                                </Link>
                            </>
                        )}
                    ...
                    <div className="flex items-center gap-3">
                        {isLoggedIn ? (
                                    ...
                                    {showUserMenu && (
                                        <div className="absolute right-0 mt-2 w-64 bg-[#111111] rounded-md shadow-2xl border border-white/10 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150">
                                            <div className="p-4 border-b border-white/5 bg-white/5">
                                                <p className="text-sm font-bold text-white truncate font-mono">{user?.fullname?.toUpperCase()}</p>
                            ...
                            <>
                                <Link to="/login">
                                    <Button variant="ghost" className="hidden sm:inline-flex text-gray-300 hover:text-[#FFD700] hover:bg-transparent px-3 font-medium uppercase tracking-wide text-sm">
                                        Login
                                    </Button>
                                </Link>
                                <Link to="/signup">
                ...
                {/* Mobile Menu */}
                {mobileMenuOpen && (
                    <div className="md:hidden absolute top-full left-0 w-full bg-[#0A0A0A] border-b border-white/10 animate-in slide-in-from-top-5 duration-200">
            ...
            {/* Review Modal */}
            <AddReviewModal isOpen={showReviewModal} onClose={() => setShowReviewModal(false)} />
        </>
    );
};
