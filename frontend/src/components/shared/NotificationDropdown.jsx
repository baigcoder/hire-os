    ...
    const fetchInterviewNotifications = async () => {
        try {
            ...
            if (res.data.success) {
                ...
                const unread = res.data.interviews.filter(interview => !interview.viewed).length;
        ...
        <Popover>
            ...
            <PopoverContent className="w-80 p-0 max-h-96 overflow-auto">
                ...
                {loading ? (
                    ...
                    <div className="p-4 text-center text-gray-500">No interview notifications</div>
                ) : (
                    <div>
                        {notifications.map(notification => (
                            <Link 
                                ...
                                className="block"
                            >
                                <div className={`p-4 border-b border-gray-100 hover:bg-gray-50 transition-colors ${!notification.viewed ? 'bg-blue-50' : ''}`}>
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <p className="font-medium">{notification.job?.title}</p>
                                            <p className="text-sm text-gray-600">{notification.job?.company?.name}</p>
                                        </div>
                                        {!notification.viewed && (
                                    ...
                                    <p className="text-sm mt-1">
                                        Interview scheduled for {formatDate(notification.interviewDetails?.date)}
                                    </p>
                                </div>
                            </Link>
                        ))}
