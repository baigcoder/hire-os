...
import {
    Bell, BellOff, MapPin, DollarSign, Clock, Building,
    Bookmark, ArrowUpRight, Sparkles, Filter, Settings, Check
} from 'lucide-react';
...
import { JOB_ALERTS_API_END_POINT, SAVED_JOBS_API_END_POINT } from '@/utils/constant';
...
const DailyJobAlerts = () => {
    ...
    const fetchDailyAlerts = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${JOB_ALERTS_API_END_POINT}/daily`, { withCredentials: true });
            if (res.data.success) {
    ...
    const fetchPreferences = async () => {
        try {
            const res = await axios.get(`${JOB_ALERTS_API_END_POINT}/preferences`, { withCredentials: true });
            if (res.data.success) {
    ...
    const savePreferences = async () => {
        try {
            setSavingPrefs(true);
            const res = await axios.put(`${JOB_ALERTS_API_END_POINT}/preferences`, preferences, { withCredentials: true });
            if (res.data.success) {
    ...
    const handleSaveJob = async (jobId) => {
        try {
            const res = await axios.post(`${SAVED_JOBS_API_END_POINT}/toggle/${jobId}`, {}, { withCredentials: true });
            if (res.data.success) {
        ...
        <div className="space-y-6">
                ...
                {showSettings && (
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {['Full-time', 'Part-time', 'Remote', 'Contract', 'Internship'].map(type => (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-lg text-sm transition-all ${preferences.jobTypes?.includes(type)
                            ...
                            <div>
                                <label className="block text-sm text-gray-400 mb-2">Salary Range (LPA)</label>
                            ...
                            <div className="flex items-center gap-3">
                                <button
                                    ...
                                    className={`w-12 h-6 rounded-full transition-all relative ${preferences.emailAlerts ? 'bg-[#FFD700]' : 'bg-white/10'
                                        }`}
                                >
                                    <div className={`w-5 h-5 rounded-full bg-white absolute top-0.5 transition-all ${preferences.emailAlerts ? 'left-6' : 'left-0.5'
                                        }`} />
                                </button>
            ...
            {summary && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        { label: 'Total Matches', value: summary.totalMatches, color: 'text-[#FFD700]' },
                        { label: 'New Today', value: summary.newJobs, color: 'text-emerald-400' },
                        { label: 'Avg Match', value: `${summary.avgMatchScore}%`, color: 'text-cyan-400' },
                        { label: 'Last Updated', value: 'Just now', color: 'text-gray-400' }
                    ].map((stat, idx) => (
                        <motion.div
                            key={idx}
            ...
            {loading ? (
                ...
                <div className="text-center py-16 bg-white/5 border border-white/10 rounded-2xl">
                    ...
                    <h3 className="text-xl font-medium text-white mb-2">No job alerts today</h3>
                        ...
                        >
                            <div className="flex items-start gap-4">
                                ...
                                <div className="w-14 h-14 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center flex-shrink-0 group-hover:border-[#FFD700]/30 transition-colors">
                                    {job.company?.logo ? (
                                        <img src={job.company.logo} alt="" className="w-10 h-10 object-contain rounded" />
                                    ) : (
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {job.matchedSkills?.length > 0 && (
                                        <div className="flex flex-wrap gap-1 mb-3">
                                            {job.matchedSkills.slice(0, 4).map((skill, i) => (
                                                <Badge key={i} className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-[10px]">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Button
                                            size="sm"
                                            variant="outline"
                                            className={`border-white/10 ${job.isSaved
