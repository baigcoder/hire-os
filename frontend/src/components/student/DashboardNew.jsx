...
import {
    Briefcase, CheckCircle, Clock, FileText, XCircle, TrendingUp,
    UserCheck, CalendarCheck, Search, MapPin, Building, DollarSign,
    Target, Zap, ArrowUpRight, Brain, FileSearch, Mic,
    Download, Mail, Phone, LayoutDashboard,
    Sparkles, GraduationCap, Video, ShieldCheck, MessageSquare, BrainCircuit,
    Bookmark, Calendar, BarChart3, Rocket, BookOpen, Users, Bell, ChevronDown
} from 'lucide-react';
...
import { ApplicationStatusChart, WeeklyActivityChart, SkillsMatchChart } from './DashboardCharts';
...
const StudentDashboard = () => {
    ...
    // Real-time dashboard socket connection
    const { isLive, lastUpdate, refreshStats } = useDashboardSocket({
    ...
    const getStatusBadge = (status) => {
        const badges = {
            pending: { color: 'bg-yellow-500/10 text-yellow-500 border-yellow-500/30', label: 'Under Review' },
            under_review: { color: 'bg-blue-500/10 text-blue-500 border-blue-500/30', label: 'Being Reviewed' },
            shortlisted: { color: 'bg-indigo-500/10 text-indigo-500 border-indigo-500/30', label: 'Shortlisted' },
            interview: { color: 'bg-purple-500/10 text-purple-500 border-purple-500/30', label: 'Interview Scheduled' },
            pending_ceo_approval: { color: 'bg-orange-500/10 text-orange-500 border-orange-500/30', label: 'Final Review' },
            hired: { color: 'bg-emerald-500/10 text-emerald-500 border-emerald-500/30', label: '🎉 Hired!' },
            accepted: { color: 'bg-emerald-500/10 text-emerald-500 border-emerald-500/30', label: 'Offer Received' },
            rejected: { color: 'bg-red-500/10 text-red-500 border-red-500/30', label: 'Not Selected' },
            withdrawn: { color: 'bg-gray-500/10 text-gray-400 border-gray-500/30', label: 'Withdrawn' }
        };
        const badge = badges[status?.toLowerCase()] || { color: 'bg-gray-500/10 text-gray-500 border-gray-500/30', label: status };
        return <Badge className={`${badge.color} border px-3 py-1 text-xs rounded-full uppercase tracking-wider font-bold`}>{badge.label}</Badge>;
    };
    ...
    const successRate = stats.applied > 0
        ? ((stats.accepted / stats.applied) * 100).toFixed(0)
        : 0;
                ...
                <div className="flex flex-col md:flex-row gap-8 mb-12">
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <div>
                                        ...
                                        <Badge variant="outline" className="border-[#FFD700]/30 text-[#FFD700] bg-[#FFD700]/5 text-xs uppercase tracking-wider">
                                ...
                                <div className="bg-white/5 rounded-sm p-3 border border-white/5">
                                    <div className="flex justify-between mb-2 text-xs">
                                        <span className="text-gray-500 uppercase tracking-wider">Profile Strength</span>
                                        <span className="text-[#FFD700] font-mono">{profileCompletion}%</span>
                                    </div>
                                    <Progress value={profileCompletion} className="h-1 bg-white/5" indicatorClassName="bg-[#FFD700]" />
                                </div>
                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-[#00FF94] font-bold uppercase tracking-wider">Free Trial</span>
                                            </div>
                                            <Badge className="bg-[#00FF94]/20 text-[#00FF94] border-[#00FF94]/30 text-[10px]">ACTIVE</Badge>
                                        </div>
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-5">
                                    ...
                                    <Button
                                        ...
                                        onClick={() => user?.profile?.resume ? window.open(user.profile.resume, '_blank') : toast.error("No resume found")}
                                    >
                    ...
                    <div className="flex-1">
                        <div className="flex items-center justify-between mb-3">
                            <span className="text-xs text-gray-500 uppercase tracking-wider">Real-time Stats</span>
                            <LiveBadge isLive={isLive} />
                        </div>
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            {[
                                { label: "APPLIED", value: stats.applied, icon: FileText, color: "text-[#00FF94]" },
                                { label: "INTERVIEWS", value: stats.interviews, icon: Video, color: "text-cyan-400" },
                                { label: "OFFERS", value: stats.accepted, icon: Target, color: "text-[#FFD700]" },
                                ...
                                >
                                    <div className="h-full bg-[#111111] border border-white/10 rounded-md p-5 relative overflow-hidden group hover:border-[#FFD700]/30 transition-all">
                                        <div className="absolute top-0 right-0 w-6 h-6 border-t border-r border-white/10 group-hover:border-[#FFD700]/30 transition-colors" />
                                        <div className="relative z-10">
                                            <div className={`p-2 rounded-sm bg-white/5 w-fit mb-3 ${stat.color}`}>
                                                <stat.icon size={18} />
                                            </div>
                            ...
                            >
                                <div className="bg-[#111111] border border-white/10 rounded-md p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative overflow-hidden">
                                    ...
                                    <div className="relative z-10 flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-sm font-bold text-white mb-1 uppercase tracking-wider">AI Interview Coach</h3>
                                            <p className="text-gray-500 text-xs font-mono">Gemini-powered mock interview preparation</p>
                                        </div>
                                    </div>
                                    <Button className="relative z-10 bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/30 font-bold rounded-sm px-5 border border-cyan-500/30 text-xs uppercase tracking-wider" onClick={() => setActiveTab('interview')}>
                            ...
                            >
                                <div className="bg-[#111111] border border-white/10 rounded-md p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative overflow-hidden h-full">
                                    ...
                                    <div className="relative z-10 flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-sm font-bold text-white mb-1 uppercase tracking-wider">Mock MCQ Tests</h3>
                                            <p className="text-gray-500 text-xs font-mono">AI-generated practice tests</p>
                                        </div>
                                    </div>
                                    <Button className="relative z-10 bg-[#FFD700]/20 text-[#FFD700] hover:bg-[#FFD700]/30 font-bold rounded-sm px-5 border border-[#FFD700]/30 text-xs uppercase tracking-wider" onClick={() => setActiveTab('tests')}>
                            ...
                            >
                                <div className="bg-[#111111] border border-white/10 rounded-md p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative overflow-hidden h-full">
                                    ...
                                    <div className="relative z-10 flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-sm font-bold text-white mb-1 uppercase tracking-wider">Skill Gap Analysis</h3>
                                            <p className="text-gray-500 text-xs font-mono">Identify skills to develop</p>
                                        </div>
                                    </div>
                                    <Button className="relative z-10 bg-[#00FF94]/20 text-[#00FF94] hover:bg-[#00FF94]/30 font-bold rounded-sm px-5 border border-[#00FF94]/30 text-xs uppercase tracking-wider" onClick={() => { setSearchParams({ tab: 'career', sub: 'skills' }); setActiveTab('career'); }}>
                ...
                >
                    <div className="bg-[#111111] border border-white/10 rounded-md p-2 inline-flex gap-2 flex-wrap justify-center">
                        {[
                            ...
                            { id: 'jobs', label: 'Jobs', icon: Briefcase, description: 'Find & save jobs' },
                            { id: 'interviews', label: 'Interviews', icon: Video, description: 'Mock interviews & calendar' },
                            { id: 'career', label: 'Career', icon: TrendingUp, description: 'Skills, salary & learning' },
                            { id: 'practice', label: 'Practice', icon: Brain, description: 'Mock tests' }
                        ].map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => {
                        ...
                        >
                            ...
                            <div className="flex gap-2 justify-center">
                                ...
                                <Button
                                    ...
                                    onClick={() => setSearchParams({ tab: 'jobs', sub: 'alerts' })}
                                >
                                    <Bell className="w-4 h-4 mr-2" /> Job Alerts
                                </Button>
                            </div>

                            {(!searchParams.get('sub') || searchParams.get('sub') === 'find') && <JobsForYou />}
                        ...
                        >
                            ...
                            <div className="flex gap-2 justify-center">
                                <Button
                                    ...
                                    onClick={() => setSearchParams({ tab: 'interviews', sub: 'mock' })}
                                >
                                ...
                                <Button
                                    ...
                                    onClick={() => setSearchParams({ tab: 'interviews', sub: 'calendar' })}
                                >
                                ...
                                <Button
                                    ...
                                    onClick={() => setSearchParams({ tab: 'interviews', sub: 'history' })}
                                >
                                    <Clock className="w-4 h-4 mr-2" /> History
                                </Button>
                            </div>

                            {(!searchParams.get('sub') || searchParams.get('sub') === 'mock') && <LiveInterview />}
                        ...
                        >
                            ...
                            <div className="flex gap-2 justify-center">
                                <Button
                                    ...
                                    onClick={() => setSearchParams({ tab: 'career', sub: 'skills' })}
                                >
                                ...
                                <Button
                                    ...
                                    onClick={() => setSearchParams({ tab: 'career', sub: 'salary' })}
                                >
                                ...
                                <Button
                                    ...
                                    onClick={() => setSearchParams({ tab: 'career', sub: 'learning' })}
                                >
                                    <BookOpen className="w-4 h-4 mr-2" /> Learning
                                </Button>
                            </div>

                            {(!searchParams.get('sub') || searchParams.get('sub') === 'skills') && <SkillGapAnalysis />}
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <div className="lg:col-span-2 space-y-6">
                                        ...
                                        >
                                            ...
                                            <div className="relative">
                                                ...
                                                <div className="flex justify-between relative z-10">
                                                    {[
                                                        { stage: 'Applied', icon: FileText, count: stats.applied, color: 'blue' },
                                                        { stage: 'Review', icon: Clock, count: stats.pending, color: 'yellow' },
                                                        { stage: 'Interview', icon: Video, count: stats.interviews, color: 'purple' },
                                                        { stage: 'Offer', icon: Target, count: stats.accepted, color: 'emerald' }
                                                    ].map((step, idx) => (
                                                    ...
                                                    ${step.count > 0
                                                                ...
                                                                style={{
                                                                    backgroundColor: step.count > 0 ? `rgba(var(--${step.color}-rgb), 0.2)` : undefined,
                                                                    borderColor: step.count > 0 ? `var(--${step.color}-color)` : undefined
                                                                }}
                                                            >
                                                                <step.icon size={20} />
                                                            </div>
                                                            <span className="text-xs text-gray-400 font-medium">{step.stage}</span>
                                                            <span className={`text-lg font-bold ${step.count > 0 ? 'text-white' : 'text-zinc-600'}`}>
                                                                {step.count}
                                                            </span>
                                                        </div>
                                                    ))}
                                            ...
                                            <div className="mt-6 pt-4 border-t border-zinc-800 flex items-center justify-between">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm font-bold text-yellow-500">{successRate}%</span>
                                                </div>
                                            </div>
                                        </motion.div>
                                    )}
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Button variant="ghost" className="text-yellow-500 hover:text-yellow-400 hover:bg-transparent" onClick={() => navigate('/applied-jobs')}>
                                    ...
                                    <div className="space-y-4">
                                        {recentApplications.length > 0 ? (
                                                    ...
                                                    <div className="flex items-center justify-between relative z-10">
                                                        <div className="flex items-center gap-4">
                                                            <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center text-white font-bold group-hover:bg-yellow-500/20 group-hover:text-yellow-500 transition-colors">
                                                                {app.job?.company?.name?.charAt(0) || <Building size={20} />}
                                                            </div>
                                                            <div>
                                                                <h3 className="font-bold text-white group-hover:text-yellow-500 transition-colors">{app.job?.title}</h3>
                                                                <p className="text-sm text-gray-500">{app.job?.company?.name} • {formatDate(app.createdAt)}</p>
                                                            </div>
                                                        </div>
                                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="text-center py-20 bg-[#0a0a0a] border border-white/10 rounded-3xl border-dashed">
                                                ...
                                                <p className="text-gray-500 mb-6">Start your journey by exploring open positions.</p>
                                                <Button className="bg-white text-black hover:bg-gray-200" onClick={() => navigate('/browse')}>
                                                    Browse Jobs
                                                </Button>
                                            </div>
                                        )}
                                        ...
                                        <div className="p-4 border-b border-white/10 flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-gray-500 font-mono uppercase tracking-wider">RAG + GPT MATCHING</p>
                                                </div>
                                            </div>
                                            <LiveBadge isLive={isLive} />
                                        </div>
                                        ...
                                        <div className="p-4 space-y-3">
                                            {aiLoading ? (
                                                ...
                                                aiJobs.slice(0, 4).map((job, idx) => {
                                                    ...
                                                    const tone = score >= 85
                                                        ? { badge: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20', border: 'hover:border-emerald-500/30' }
                                                        : score >= 70
                                                            ? { badge: 'bg-[#FFD700]/10 text-[#FFD700] border-[#FFD700]/30', border: 'hover:border-[#FFD700]/30' }
                                                            : score >= 55
                                                                ? { badge: 'bg-blue-500/10 text-blue-400 border-blue-500/20', border: 'hover:border-blue-500/30' }
                                                                : { badge: 'bg-white/5 text-gray-400 border-white/10', border: 'hover:border-white/20' };
                                                        ...
                                                        >
                                                            <div className="flex items-start gap-3">
                                                                <div className="w-11 h-11 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:border-[#FFD700]/30 transition-colors shrink-0">
                                                                    {job.company?.logo ? (
                                                                        <img src={job.company.logo} alt="" className="w-7 h-7 object-contain rounded" />
                                                                    ) : (
                                                                ...
                                                                <div className="flex-1 min-w-0">
                                                                    <div className="flex items-start justify-between gap-3">
                                                                        <div className="min-w-0">
                                                                            ...
                                                                            <p className="text-xs text-gray-500 font-mono truncate">{job.company?.name}</p>
                                                                        </div>
                                                                        <Badge className={`${tone.badge} text-[10px] font-mono uppercase tracking-wider`}>
                                                                            {score}% match
                                                                        </Badge>
                                                                    </div>
                                                                    ...
                                                                    <div className="flex items-center gap-3 text-[10px] mt-2 text-gray-400 font-mono uppercase tracking-wider">
                                                                        ...
                                                                        <span className="flex items-center gap-1">
                                                                            ...
                                                                            <span className="text-gray-300">{job.salary || 'Competitive'}</span>
                                                                        </span>
                                                                    </div>

                                                                    {(job.aiInsight || job.matchedSkills?.length > 0) && (
                                                                    ...
                                                                    {(job.whyGoodFit || (job.topStrengths && job.topStrengths.length > 0)) && (
                                                                        <div className="mt-3 border-t border-white/5 pt-2">
                                                                            ...
                                                                            {isExpanded && (
                                                                                <div className="mt-2 space-y-1">
                                                                                    ...
                                                                                    {job.topStrengths && job.topStrengths.length > 0 && (
                                                                                        <ul className="text-[11px] text-gray-400 list-disc list-inside">
                                                                                            {job.topStrengths.slice(0, 3).map((item, index) => (
                                                                                                <li key={`${job._id}-strength-${index}`}>
                                                                                                    {item}
                                                                                                </li>
                                                                                            ))}
                                                                                        </ul>
                                                ...
                                                <div className="text-center py-10 text-gray-500">
                                                    ...
                                                    <p className="text-sm">
                                                        {profileCompletion < 70 ? 'Complete your profile for personalized recommendations' : 'No job matches found. Check back soon!'}
                                                    </p>
