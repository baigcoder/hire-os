...
const InterviewCard = ({ application = {} }) => {
    ...
    useEffect(() => {
        if (!application.interviewDetails?.date || application.interviewDetails?.completed) {
            return;
        }
        ...
        const updateTimeRemaining = () => {
            ...
            if (diffDays > 0) {
                setTimeRemaining(`${diffDays} day${diffDays > 1 ? 's' : ''} ${diffHours} hr${diffHours > 1 ? 's' : ''}`);
            } else if (diffHours > 0) {
                setTimeRemaining(`${diffHours} hour${diffHours > 1 ? 's' : ''} ${diffMinutes} min${diffMinutes > 1 ? 's' : ''}`);
            } else {
    ...
    }, [application.interviewDetails?.date, application.interviewDetails?.completed]);
        ...
        <Card className="w-full">
            <CardHeader>
                <div className="flex justify-between items-center">
                    ...
                    >
                        {application.interviewDetails?.completed ? "Completed" : "Scheduled"}
                    </Badge>
                </div>
                    ...
                    <div className="bg-blue-50 p-4 rounded-md">
                        ...
                        {application.interviewDetails?.details && (
                            <div className="mt-3 pt-3 border-t border-blue-200">
                                <p className="text-sm">{application.interviewDetails.details}</p>
                            </div>
                        )}
                        
                        {!application.interviewDetails?.completed && (
                            <div className="mt-3 pt-3 border-t border-blue-200 text-sm text-blue-600">
                                {isInterviewTime().isActive ? 'Interview is happening now!' : isInterviewTime().isSoon ? `Interview starting soon! (${isInterviewTime().minutesRemaining} minutes)` : `Time remaining: ${timeRemaining}`}
                            </div>
                        )}
                    </div>
                </div>
                ...
                {application.status === 'interview' && 
                 !application.interviewDetails?.completed && (
                    <Button 
                        ...
                        variant={isInterviewTime().isActive ? "default" : isInterviewTime().isSoon ? "secondary" : "outline"}
                    >
                        {isInterviewTime().isActive ? "Join Interview Now" : isInterviewTime().isSoon ? `Interview Starting Soon (${isInterviewTime().minutesRemaining} min)` : "Interview Button Will Activate at Scheduled Time"}
                    </Button>
                )}
