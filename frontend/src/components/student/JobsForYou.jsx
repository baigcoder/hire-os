...
import {
    Briefcase, MapPin, Building2, Sparkles, Target, ChevronRight,
    TrendingUp, Check, X, Loader2, Award, Star, Filter
} from 'lucide-react';
...
const JobsForYou = () => {
    ...
    const getMatchBadge = (score) => {
        if (score >= 85) return { label: 'EXCELLENT MATCH', color: 'from-emerald-500 to-green-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', text: 'text-emerald-400' };
        if (score >= 70) return { label: 'STRONG MATCH', color: 'from-[#FFD700] to-amber-400', bg: 'bg-[#FFD700]/10', border: 'border-[#FFD700]/30', text: 'text-[#FFD700]' };
        if (score >= 55) return { label: 'GOOD MATCH', color: 'from-blue-500 to-cyan-400', bg: 'bg-blue-500/10', border: 'border-blue-500/30', text: 'text-blue-400' };
        return { label: 'EXPLORE', color: 'from-gray-500 to-gray-400', bg: 'bg-white/5', border: 'border-white/10', text: 'text-gray-400' };
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-white tracking-tight">JOBS FOR YOU</h2>
                        <p className="text-xs text-gray-500 font-mono tracking-wider">AI-POWERED RECOMMENDATIONS</p>
                    </div>
                </div>
            ...
            <div className="flex items-center justify-between p-4 bg-gradient-to-r from-[#0a0a0a] to-[#111111] border border-white/10 rounded-sm">
                <div className="flex items-center gap-8">
                    <div className="text-center">
                        <div className="text-2xl font-bold text-white">{jobs.filter(j => j.matchScore >= 85).length}</div>
                        <div className="text-xs text-emerald-400 font-mono">EXCELLENT</div>
                    </div>
                    <div className="w-px h-10 bg-white/10" />
                    <div className="text-center">
                        <div className="text-2xl font-bold text-white">{jobs.filter(j => j.matchScore >= 70 && j.matchScore < 85).length}</div>
                        <div className="text-xs text-[#FFD700] font-mono">STRONG</div>
                    </div>
                    <div className="w-px h-10 bg-white/10" />
                    <div className="text-center">
                        <div className="text-2xl font-bold text-white">{jobs.filter(j => j.matchScore >= 55 && j.matchScore < 70).length}</div>
                                ...
                                <div className="relative p-5">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="w-14 h-14 bg-white/5 rounded-sm flex items-center justify-center border border-white/10 group-hover:border-white/20 transition-colors">
                                            {job.company?.logo ? (
                                                <img src={job.company.logo} alt="" className="w-10 h-10 object-contain" />
                                            ) : (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-3 mb-1">
                                                ...
                                                <Badge className={`${badge.bg} ${badge.text} ${badge.border} border font-mono text-[10px] px-2`}>
                                                    {badge.label}
                                                </Badge>
                                            </div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {(job.matchedSkills || []).slice(0, 4).map((skill, i) => (
                                                    <span key={i} className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-500/10 text-emerald-400 text-xs rounded-sm border border-emerald-500/20">
                                                        <Check className="w-3 h-3" />
                                                        {skill}
                                                    </span>
                                                ))}
                                                {(job.missingSkills || []).slice(0, 2).map((skill, i) => (
                                                    <span key={`m-${i}`} className="inline-flex items-center gap-1 px-2 py-1 bg-red-500/10 text-red-400 text-xs rounded-sm border border-red-500/20">
                                                        <X className="w-3 h-3" />
                                                        {skill}
                                                    </span>
                                                ))}
                                        ...
                                        <div className="flex flex-col items-center">
                                            <div className={`relative w-16 h-16 rounded-full ${badge.bg} border ${badge.border} flex items-center justify-center`}>
                                                <span className={`text-xl font-bold ${badge.text}`}>{job.matchScore || 0}%</span>
                                            ...
                                            <span className="text-[10px] text-gray-500 mt-1 font-mono">MATCH</span>
                                        </div>
                                    </div>
                                    ...
                                    >
                                        <div className="text-xs text-gray-500 font-mono">
                                            {job.aiInsight || 'Skills and experience align well with requirements'}
                                        </div>
                                        <Button
                                            size="sm"
