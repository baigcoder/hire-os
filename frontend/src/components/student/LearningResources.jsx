...
import {
    BookOpen, ExternalLink, Clock, Star, Play, CheckCircle,
    Sparkles, Filter, GraduationCap, Code, Award, RefreshCw
} from 'lucide-react';
...
const LearningResources = () => {
    ...
    const fetchLearningResources = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${CAREER_INSIGHTS_API_END_POINT}/learning`, { withCredentials: true });
            if (res.data.success) {
    ...
    const getPlatformColor = (platform) => {
        switch (platform?.toLowerCase()) {
            case 'udemy': return 'text-purple-400 bg-purple-500/10 border-purple-500/20';
            case 'coursera': return 'text-blue-400 bg-blue-500/10 border-blue-500/20';
            case 'youtube': return 'text-red-400 bg-red-500/10 border-red-500/20';
            case 'freecodecamp': return 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20';
            case 'pluralsight': return 'text-orange-400 bg-orange-500/10 border-orange-500/20';
    ...
    const filteredCourses = (resources?.recommendedCourses || []).filter(course => {
            ...
            <div className="space-y-6">
                <div className="bg-[#111111] border border-white/10 rounded-2xl p-8">
                    <div className="flex flex-col items-center justify-center py-12">
                        ...
                        <p className="text-gray-400 mt-4 font-mono text-sm">CURATING LEARNING RESOURCES...</p>
                        <p className="text-gray-600 text-xs mt-2">Finding the best courses for you</p>
                    </div>
                </div>
            </div>
        );
                ...
                <div className="flex items-center gap-2">
                    {['all', 'free', 'paid'].map(f => (
                        <Button
                            ...
                            size="sm"
                            className={filter === f ? 'bg-[#FFD700] text-black' : 'text-gray-400'}
                            onClick={() => setFilter(f)}
                        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    ...
                                    {resources.learningPath.map((step, idx) => (
                                        <div key={idx} className="relative">
                                            <div className="absolute -left-10 w-8 h-8 rounded-full bg-[#FFD700]/20 border border-[#FFD700]/40 flex items-center justify-center">
                                                <span className="text-sm font-bold text-[#FFD700]">{idx + 1}</span>
                                            </div>
                ...
                <div className="space-y-6">
                    ...
                    {resources?.quickWins?.length > 0 && (
                        <div className="bg-[#111111] border border-white/10 rounded-2xl p-5">
                            <h4 className="font-bold text-white mb-4 flex items-center gap-2">
                                <Play className="w-4 h-4 text-emerald-400" /> Quick Wins (1 Week)
                            </h4>
                    ...
                    {resources?.certifications?.length > 0 && (
                        <div className="bg-[#111111] border border-white/10 rounded-2xl p-5">
                            <h4 className="font-bold text-white mb-4 flex items-center gap-2">
                                <Award className="w-4 h-4 text-[#FFD700]" /> Certifications to Pursue
                            </h4>
                            <div className="space-y-3">
