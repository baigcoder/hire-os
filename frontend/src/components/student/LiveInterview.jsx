...
import {
    ...
    MessageSquare, User, Bot, Loader2, Clock, Volume2, VolumeX,
    Sparkles, Brain, PhoneOff, Phone, Settings, ChevronDown
} from 'lucide-react';
...
const LiveInterview = () => {
    ...
    const [interviewer, setInterviewer] = useState({ name: 'AI', type: 'behavioral' });
    ...
    const [selectedLanguage, setSelectedLanguage] = useState({ id: 'en', name: 'English', flag: '🇺🇸' });
    ...
    const interviewTypes = [
        { id: 'behavioral', name: 'Behavioral', icon: MessageSquare, color: 'from-[#FFD700] to-[#CC9900]', description: 'STAR method & soft skills' },
        { id: 'technical', name: 'Technical', icon: Brain, color: 'from-[#FFD700] to-[#CC9900]', description: 'Coding & problem solving' },
        { id: 'hr', name: 'HR Round', icon: User, color: 'from-[#FFD700] to-[#CC9900]', description: 'Career goals & culture fit' }
    ];
    ...
    useEffect(() => {
        const fetchVoices = async () => {
            try {
                ...
                if (response.data.success && response.data.voices?.length > 0) {
                    ...
                    console.log(`✅ [LiveInterview] Loaded ${response.data.voices.length} voices`);
                } else {
                    throw new Error('No voices returned');
                }
            } catch (error) {
                console.log('⚠️ [LiveInterview] Failed to fetch voices, using defaults');
                // Set default voices as fallback
                setAvailableVoices([
                    { id: 'RACHEL', name: 'Rachel', gender: 'female', description: 'Professional female voice' },
                    { id: 'MATILDA', name: 'Matilda', gender: 'female', description: 'Professional woman, pleasing alto' },
                    { id: 'ALICE', name: 'Alice', gender: 'female', description: 'Clear, engaging, British accent' },
                    { id: 'LIAM', name: 'Liam', gender: 'male', description: 'Young adult with energy and warmth' },
                    { id: 'CLYDE', name: 'Clyde', gender: 'male', description: 'Deep, intense male voice' },
                    { id: 'ADAM', name: 'Adam', gender: 'male', description: 'Deep professional male voice' },
                ]);
            }
        };
        ...
        const fetchLanguages = async () => {
            try {
                ...
                const response = await axios.get(`${API_BASE}/live-interview/languages`);
                if (response.data.success) {
                    setAvailableLanguages(response.data.languages);
                    console.log(`✅ [LiveInterview] Loaded ${response.data.languages.length} languages`);
                }
            } catch (error) {
                console.log('⚠️ [LiveInterview] Failed to fetch languages, using defaults');
    ...
    const initializeMedia = async () => {
        try {
            console.log(`📹 [LiveInterview] Initializing media - Call type: ${callType}`);
    ...
    const initializeSpeechRecognition = () => {
        ...
        recognition.onerror = (event) => {
            ...
            switch (event.error) {
                case 'network':
                    console.log('[LiveInterview] Network error, disabling speech recognition for this session');
                    try {
                        recognition.stop();
                    } catch (e) {
                        /* ignore */
                    ...
                    toast.error('Speech recognition network error. You can continue by typing your answers or refresh the page.');
                    break;
                case 'no-speech':
        ...
        recognition.onend = () => {
            console.log('[LiveInterview] Speech recognition ended, isRecording:', isRecording, 'isAISpeaking:', isAISpeaking);
            ...
            if (isRecordingRef.current && !isAISpeakingRef.current) {
                setTimeout(() => {
                    try {
                        ...
                        console.error('[LiveInterview] Failed to restart speech recognition:', e);
                    }
                }, 100);
            }
        };
    ...
    const handleSilenceDetected = () => {
        ...
        const currentInterim = (interimTranscriptRef.current || interimTranscript).trim();

        const candidateText = currentTranscript.length >= 5 ? currentTranscript : currentInterim;

        if (candidateText.length >= 5 && !isProcessingRef.current && !isAISpeakingRef.current) {
    ...
    const playAudio = async (base64Audio, audioType = 'audio/mpeg') => {
        return new Promise((resolve, reject) => {
            try {
                ...
                audioRef.current.playbackRate = speedPreset === 'fast' ? 1.15 : speedPreset === 'slow' ? 0.9 : 1.0;
    ...
    const startInterview = async () => {
        ...
        try {
            console.log(`🚀 [LiveInterview] Starting interview - Type: ${interviewType}, Voice: ${selectedVoice?.name || 'auto'}, Call: ${callType}`);

            const response = await axios.post(`${API_BASE}/live-interview/start`, {
                type: interviewType,
            ...
            if (response.data.success) {
                const { sessionId, interviewer, greeting, firstQuestion, characterUsage } = response.data;
    ...
    const startListening = () => {
        ...
        if (!recognitionRef.current) {
            console.log('[LiveInterview] Initializing new speech recognition instance');
            ...
            try {
                ...
                setTimeout(() => {
                    try {
                        ...
                        setIsRecording(true);
                        console.log('[LiveInterview] Speech recognition started successfully');
                        ...
                        if (error.name === 'InvalidStateError') {
                            ...
                            console.error('[LiveInterview] Failed to start recognition:', error);
                            toast.error('Failed to start microphone. Please refresh the page.');
                        }
                    }
                }, 50);
            } catch (error) {
    ...
    const sendResponse = async (overrideText) => {
        const userText = (overrideText ?? (transcriptRef.current || transcript)).trim();
        console.log('[LiveInterview] sendResponse called:', { userText, length: userText.length, sessionId });
        ...
        if (!currentSessionId) {
            console.error('[LiveInterview] No sessionId! Interview session not started properly.');
            toast.error('Session expired. Please restart the interview.');
            setMode('setup');
            return;
        }

        console.log('[LiveInterview] Sending response to AI:', userText.substring(0, 50) + '...');
        stopListening();
        ...
        try {
            const response = await axios.post(`${API_BASE}/live-interview/respond`, {
            ...
            if (response.data.success) {
                ...
                setProgress({ current: prog.currentQuestion, total: prog.totalQuestions });
    ...
    const endInterview = async () => {
        ...
        try {
            const response = await axios.post(`${API_BASE}/live-interview/end`, {
                sessionId
            }, { withCredentials: true });
    ...
    const renderSetup = () => (
        ...
        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                >
                    <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">SELECT INTERVIEW TYPE</h3>
                        ...
                        {interviewTypes.map((type, idx) => (
                            ...
                            >
                                <div className="flex-1 text-left">
                                    <h4 className={`font-bold ${interviewType === type.id ? 'text-black' : 'text-white'}`}>
                                        {type.name}
                                    </h4>
                                    <p className={`text-sm ${interviewType === type.id ? 'text-black/70' : 'text-gray-500'}`}>
                                        {type.description}
                                    </p>
                                </div>
                            </motion.button>
                ...
                >
                    {/* Call Type */}
                    <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">CALL TYPE</h3>
                    ...
                    <div className="relative">
                        <label className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3 block">AI VOICE</label>
                        <button
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${selectedVoice?.gender === 'female'
                            ...
                            <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform ${showVoiceDropdown ? 'rotate-180' : ''}`} />
                        </button>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <div className="p-4 border-b border-white/10 flex items-center justify-between">
                                                <h3 className="text-white font-bold uppercase tracking-wider text-sm">🎤 Select AI Voice</h3>
                                                <button
                                                    onClick={() => setShowVoiceDropdown(false)}
                                            ...
                                            >
                                                ...
                                                <div className="flex-1">
                                                    <span className="text-white font-bold block">⚡ Auto Select</span>
                                                    <p className="text-xs text-gray-500">Based on interview type</p>
                                                </div>
                                                {!selectedVoice && <span className="text-[#FFD700] text-xl">✓</span>}
                                            </button>
                                                ...
                                                {availableVoices.map(voice => (
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            setShowVoiceDropdown(false);
                                                            console.log(`🎤 [LiveInterview] Selected voice: ${voice.name}`);
                                                        }}
                                                    ...
                                                    >
                                                        <div className={`w-10 h-10 rounded-sm flex items-center justify-center flex-shrink-0 ${voice.gender === 'female' ? 'bg-[#FF69B4]' : 'bg-[#00D4FF]'}`}>
                                                            <Volume2 className="w-5 h-5 text-white" />
                                                        </div>
                                                        <div className="flex-1">
                                                            <span className="text-white font-bold block">{voice.name}</span>
                                                            <p className="text-xs text-gray-500">{voice.description}</p>
                                                        </div>
                                                        <Badge className={`text-[10px] uppercase rounded-sm ${voice.gender === 'female' ? 'bg-[#FF69B4]/20 text-[#FF69B4] border-[#FF69B4]/30' : 'bg-[#00D4FF]/20 text-[#00D4FF] border-[#00D4FF]/30'}`}>
                                                            {voice.gender}
                                                        </Badge>
                                                        {selectedVoice?.id === voice.id && <span className="text-[#FFD700] text-xl">✓</span>}
                                                    </button>
                                                ))}
                                            </div>
                                        </motion.div>
            ...
            >
                ...
                <div className="rounded-sm border border-[#2a2a2a] bg-transparent p-4">
                    <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 block">🌐 LANGUAGE</label>
                    <button
                ...
                {/* Speed Preset */}
                <div className="rounded-sm border border-[#2a2a2a] bg-transparent p-4">
                    <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 block">⚡ SPEAKING SPEED</label>
                ...
                {/* Style Preset */}
                <div className="rounded-sm border border-[#2a2a2a] bg-transparent p-4">
                    <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 block">🎭 VOICE STYLE</label>
                    <div className="flex gap-2">
                        ...
                        >
                            ...
                            >
                                <div className="p-4 border-b border-[#2a2a2a] flex items-center justify-between">
                                    <h3 className="text-white font-bold uppercase tracking-wider text-sm">🌐 Select Interview Language</h3>
                                    <button onClick={() => setShowLanguageModal(false)} className="text-gray-400 hover:text-white p-1 text-lg">✕</button>
                                </div>
                                <div className="max-h-[50vh] overflow-y-auto">
                                    {availableLanguages.map(lang => (
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                console.log(`🌐 [LiveInterview] Selected language: ${lang.name}`);
                                            }}
                                        ...
                                        >
                                            ...
                                            <span className="text-white font-bold flex-1">{lang.name}</span>
                                            {selectedLanguage.id === lang.id && <span className="text-[#FFD700] text-xl">✓</span>}
                                        </button>
                                    ))}
                                </div>
                            </motion.div>
            ...
            >
                ...
                >
                    ...
                    <span className="uppercase tracking-wider">Start {callType === 'video' ? 'Video' : 'Voice'} Interview</span>
                </motion.button>
    ...
    const renderConnecting = () => (
        ...
        >
            ...
            <h2 className="text-xl font-bold text-white mt-6">Connecting to AI Interviewer...</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                        {/* AI Avatar */}
                        <div className={`w-20 h-20 rounded-full bg-[#FFD700] flex items-center justify-center ${isAISpeaking ? 'animate-pulse' : ''}`}>
                        ...
                        <div className="absolute bottom-3 left-3 px-3 py-1.5 bg-black/80 rounded-lg flex items-center gap-2 border border-[#2a2a2a]">
                            <span className={`w-2 h-2 rounded-full ${isAISpeaking ? 'bg-[#FFD700] animate-pulse' : 'bg-gray-500'}`} />
                            <span className="text-white text-sm font-medium">{interviewer.name}</span>
                        </div>
                        ...
                        {isAISpeaking && (
                            <div className="absolute top-3 right-3 px-3 py-1.5 bg-[#FFD700]/20 rounded-lg flex items-center gap-2 border border-[#FFD700]/40">
                                ...
                                <span className="text-[#FFD700] text-sm font-medium">Speaking</span>
                            </div>
                        )}
                    </motion.div>
                    ...
                    >
                        {callType === 'video' ? (
                            ...
                            <div className={`w-20 h-20 rounded-full bg-[#333] flex items-center justify-center ${isRecording ? 'ring-4 ring-[#FFD700]/50' : ''}`}>
                ...
                <div className="bg-[#0a0a0a] rounded-xl p-4 border border-[#2a2a2a] space-y-3">
                    <div>
                        <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">YOUR SPEECH</h3>
                    ...
                    <div className="rounded-full px-6 h-12 flex items-center gap-2 transition-all">
                        {isProcessing ? (
                            <div className="flex items-center gap-2 bg-[#FFD700]/20 px-4 py-2 rounded-lg border border-[#FFD700]/40">
                                ...
                                <span className="text-[#FFD700] font-medium">Processing...</span>
                            </div>
                            ...
                            <div className="flex items-center gap-2 bg-green-500/20 px-4 py-2 rounded-lg border border-green-500/40">
                                ...
                                <span className="text-green-400 text-sm">Auto-send in 3s of silence</span>
                            </div>
                        ) : null}
                    </div>
            ...
            <div className="space-y-4">
                ...
                <div className="bg-[#0a0a0a] rounded-xl p-4 border border-[#2a2a2a]">
                    <div className="flex justify-between text-sm mb-3">
                        <span className="text-gray-400 font-semibold uppercase tracking-wider">Progress</span>
                        <span className="text-[#FFD700] font-bold">{progress.current}/{progress.total}</span>
                    </div>
                    <Progress value={(progress.current / progress.total) * 100} className="h-2 bg-[#1a1a1a]" />
                </div>
                ...
                <div className="bg-[#0a0a0a] rounded-xl p-4 border border-[#2a2a2a] max-h-[350px] overflow-y-auto">
                    <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">CONVERSATION</h3>
                    <div className="space-y-3">
                        {messages.map((msg, i) => (
                            <div
                                key={i}
                ...
                <div className="text-center text-xs text-gray-500">
                    Voice: {charUsage.used.toLocaleString()}/{charUsage.limit.toLocaleString()} chars
                </div>
            </div>
        </div>
    );
    ...
    const renderSummary = () => (
        ...
        >
            <div className="text-center mb-8">
                <div className={`inline-flex items-center justify-center w-24 h-24 rounded-full mb-4 ${summary.overallScore >= 80 ? 'bg-[#00FF94]/20 border-2 border-[#00FF94]/40' :
                    summary.overallScore >= 60 ? 'bg-[#FFD700]/20 border-2 border-[#FFD700]/40' : 'bg-red-500/20 border-2 border-red-500/40'
                    }`}>
                    <span className={`text-4xl font-bold ${summary.overallScore >= 80 ? 'text-[#00FF94]' :
                        summary.overallScore >= 60 ? 'text-[#FFD700]' : 'text-red-400'
                        }`}>
                        {summary.overallScore}
                    </span>
                </div>
                <h2 className="text-2xl font-bold text-white mb-2">Interview Complete!</h2>
                <span className="inline-block px-4 py-1 bg-[#FFD700]/10 text-[#FFD700] rounded-full text-sm font-semibold border border-[#FFD700]/30">{summary.grade}</span>
            </div>
