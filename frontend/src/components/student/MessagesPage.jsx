...
import {
    Mail, MailOpen, Inbox, Send, Trash2, Reply, Star,
    ChevronLeft, ChevronRight, Search, Filter, RefreshCw,
    User, Building2, Shield, Crown, MessageSquare, MailPlus,
    Loader2, CheckCircle, Clock, AlertCircle, X
} from 'lucide-react';
...
const MessagesPage = () => {
    ...
    const fetchMessages = async () => {
        setLoading(true);
        try {
            const endpoint = activeTab === 'inbox' ? '/message/inbox' : '/message/sent';
    ...
    const markAllAsRead = async () => {
        try {
            await axios.put(`${API_BASE}/message/mark-all-read`, {}, {
                withCredentials: true
            });
    ...
    const sendReply = async () => {
        ...
        setSending(true);
        try {
            await axios.post(`${API_BASE}/message/reply/${selectedMessage._id}`, {
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] text-white p-6">
            ...
            <div className="flex items-center gap-4 mb-6 border-b border-white/10 pb-4">
                <button
                    onClick={() => { setActiveTab('inbox'); setPagination(prev => ({ ...prev, page: 1 })); }}
                    className={`flex items-center gap-2 px-4 py-2 rounded-sm text-sm font-mono transition-all ${activeTab === 'inbox'
                ...
                <button
                    onClick={() => { setActiveTab('sent'); setPagination(prev => ({ ...prev, page: 1 })); }}
                    className={`flex items-center gap-2 px-4 py-2 rounded-sm text-sm font-mono transition-all ${activeTab === 'sent'
                            ? 'bg-[#FFD700] text-black'
                            : 'bg-white/5 text-gray-400 hover:bg-white/10'
                        }`}
                >
                        ...
                        <>
                            {filteredMessages.map((message) => (
                                <motion.div
                                    ...
                                    className={`p-4 rounded-sm border cursor-pointer transition-all ${selectedMessage?._id === message._id
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        {/* Unread indicator */}
                                        <div className={`w-2 h-2 rounded-full mt-2 ${message.isRead ? 'bg-transparent' : 'bg-[#FFD700]'
                                            }`} />

                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                {getSenderIcon(activeTab === 'inbox' ? message.senderRole : message.receiverRole)}
                                            ...
                                            <p className={`text-sm truncate ${message.isRead ? 'text-gray-400' : 'text-white font-medium'
                                                }`}>
                                                {message.subject}
                                            </p>
                                            <div className="flex items-center gap-2 mt-2">
                                                ...
                                                {message.type !== 'general' && (
                                                    <Badge className={`text-[10px] ${getTypeBadge(message.type)}`}>
                                                        {message.type}
                                                    </Badge>
                                                )}
                                            </div>
                    ...
                    {selectedMessage ? (
                        ...
                        >
                            ...
                            <div className="p-6">
                                ...
                                {/* Related Info */}
                                {(selectedMessage.relatedTo?.jobId || selectedMessage.relatedTo?.applicationId) && (
                                    <div className="mt-6 p-4 bg-white/5 rounded-sm">
                                        <p className="text-xs text-gray-500 uppercase mb-2">Related To</p>
                    ...
                    >
                        ...
                        >
                            <div className="p-4 border-b border-white/10 flex items-center justify-between">
                                <h3 className="font-bold text-white">Reply to {selectedMessage.senderId?.fullname}</h3>
                                <button onClick={() => setShowReplyModal(false)}>
                                    <X className="w-5 h-5 text-gray-400 hover:text-white" />
                                </button>
                            </div>
                            <div className="p-4">
                                <p className="text-xs text-gray-500 mb-2">Re: {selectedMessage.subject}</p>
                                <textarea
                                    value={replyContent}
