...
import {
    Video, Mic, MicOff, VideoOff, Play, Square, RefreshCw,
    MessageSquare, User, Bot, Loader2, Clock, ChevronRight,
    ThumbsUp, ThumbsDown, Star, Award, AlertCircle, Volume2, VolumeX,
    Sparkles, Brain, Send, StopCircle
} from 'lucide-react';
...
const MockInterview = () => {
    ...
    const interviewTypes = [
        { id: 'behavioral', name: 'Behavioral', icon: MessageSquare, description: 'STAR method questions about past experiences' },
        { id: 'technical', name: 'Technical', icon: Brain, description: 'Coding concepts and problem-solving' },
        { id: 'hr', name: 'HR Round', icon: User, description: 'General questions about career and goals' }
    ];
    ...
    const generateQuestions = async () => {
        setLoading(true);
        try {
            const response = await axios.post(`${API_BASE}/mock-interview/generate-questions`, {
                type: interviewType,
            ...
            if (response.data.success) {
                ...
                toast.success(`Interview ready! Powered by ${response.data.generatedBy || 'AI'}`);
            } else {
    ...
    const getFallbackQuestions = (type) => {
        const fallbacks = {
            behavioral: [
                { question: "Tell me about a time you had to lead a team through a difficult project.", category: "Leadership", tips: "Use STAR method, quantify results" },
                { question: "Describe a situation where you had to resolve a conflict with a colleague.", category: "Conflict Resolution", tips: "Focus on collaboration and outcome" },
                { question: "Give an example of a time you failed and what you learned from it.", category: "Growth", tips: "Show self-awareness and learning" },
                { question: "Tell me about your greatest professional achievement.", category: "Achievement", tips: "Be specific about your contribution" },
                { question: "How do you handle working under pressure or tight deadlines?", category: "Stress Management", tips: "Give concrete examples" }
            ],
            technical: [
                { question: "Explain the difference between REST and GraphQL APIs.", category: "APIs", tips: "Compare use cases and trade-offs" },
                { question: "How would you optimize a slow database query?", category: "Database", tips: "Mention indexing, query analysis, caching" },
                { question: "What is the difference between SQL and NoSQL databases?", category: "Database", tips: "Discuss ACID, scalability, use cases" },
                { question: "Explain how you would design a URL shortening service.", category: "System Design", tips: "Cover hashing, storage, scalability" },
                { question: "What are the key principles of object-oriented programming?", category: "OOP", tips: "Explain with examples" }
            ],
            hr: [
                { question: "Where do you see yourself in 5 years?", category: "Career Goals", tips: "Align with company growth" },
                { question: "Why are you looking to leave your current position?", category: "Motivation", tips: "Be positive, focus on growth" },
                { question: "What are your salary expectations?", category: "Negotiation", tips: "Research market rates beforehand" },
                { question: "Why should we hire you over other candidates?", category: "Value Proposition", tips: "Highlight unique strengths" },
                { question: "Do you have any questions for us?", category: "Engagement", tips: "Ask thoughtful questions about role/culture" }
            ]
        };
    ...
    const getFeedbackForResponse = async (response) => {
        setIsAnalyzing(true);
        try {
            const res = await axios.post(`${API_BASE}/mock-interview/feedback`, {
                question: response.question.question,
                answer: response.answer,
                previousQA: responses.map(r => ({ question: r.question.question, answer: r.answer }))
            }, { withCredentials: true });
    ...
    const finishInterview = async () => {
        ...
        try {
            ...
            const scores = responses.filter(r => r.feedback?.score).map(r => r.feedback.score);
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Practice with AI-powered interview simulation</p>
                </div>
                {mode !== 'setup' && (
                    <Button
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {interviewTypes.map((type) => (
                                    ...
                                    >
                                        ...
                                        <div className={`text-xs mt-1 ${interviewType === type.id ? 'text-gray-300' : 'text-gray-500'}`}>
                                            {type.description}
                                        </div>
                                    </button>
                                ))}
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-md p-6">
                            ...
                            <div className="flex gap-4">
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-5 py-3 rounded-sm border text-sm font-medium ${videoEnabled
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-5 py-3 rounded-sm border text-sm font-medium ${audioEnabled
                                        ? 'bg-[#FFD700]/10 border-[#FFD700]/50 text-[#FFD700]'
                                        : 'bg-white/5 border-white/10 text-gray-400 hover:border-white/20'
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-5 py-3 rounded-sm border text-sm font-medium ${voiceEnabled
                                ...
                                <button
                                    onClick={() => setConversationMode(conversationMode === 'live' ? 'manual' : 'live')}
                                    className={`flex items-center gap-2 px-5 py-3 rounded-sm border text-sm font-medium ${conversationMode === 'live'
                                        ? 'bg-[#FFD700]/10 border-[#FFD700]/50 text-[#FFD700]'
                                        : 'bg-white/5 border-white/10 text-gray-400 hover:border-white/20'
                                        }`}
                                >
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="bg-[#111111] border border-white/10 rounded-md p-4">
                                    <div className="flex items-center justify-between mb-2">
                                        <label className="text-xs text-gray-500 uppercase tracking-wider font-mono">
                                            Your Response {audioEnabled ? '(Live Speech-to-Text)' : '(Type)'}
                                        </label>
                                        {isRecording && (
                                            <div className="flex items-center gap-2">
                                        ...
                                        {isAnalyzing && (
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-[#FFD700]">Analyzing...</span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="w-full min-h-[100px] px-3 py-2 bg-[#0A0A0A] border border-white/10 rounded-sm text-sm">
                                        <span className="text-white">{transcript || typedResponse}</span>
                                        {interimTranscript && (
                                            <span className="text-gray-400 italic">{interimTranscript}</span>
                                        )}
                                        {!transcript && !typedResponse && !interimTranscript && (
                                            <span className="text-gray-600">
                                                {audioEnabled ? "🎤 Start speaking... Your words will appear here in real-time" : "Type your response..."}
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    {responses.length > 0 && responses[responses.length - 1]?.feedback && (
                                        <div className="mt-3 p-3 bg-[#FFD700]/5 border border-[#FFD700]/25 rounded">
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className="text-sm font-medium text-[#FFD700]">
                                                    Score: {responses[responses.length - 1].feedback.score}/10
                                                </span>
                                                <Badge className="bg-[#FFD700]/15 text-[#FFD700] text-xs">
                                                    {responses[responses.length - 1].feedback.verdict || 'Analyzed'}
                                                </Badge>
                                            </div>
                                ...
                                <Button
                                    ...
                                    disabled={(!transcript && !typedResponse) || isRecording || isAnalyzing}
                ...
                {mode === 'feedback' && (
                    ...
                    >
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-md p-8 text-center relative overflow-hidden">
                            ...
                            <Badge className={`text-lg px-4 py-1 ${overallScore >= 80
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-md p-6">
                            ...
                            <div className="space-y-4 max-h-[400px] overflow-y-auto">
                                {responses.map((r, idx) => (
                                    <div key={idx} className="p-4 border border-white/5 rounded-sm bg-white/5">
                                        <div className="flex items-start justify-between gap-4">
                                            ...
                                            {r.feedback && (
                                                <Badge className={`${r.feedback.score >= 7 ? 'bg-green-500/20 text-green-400'
                                                    : r.feedback.score >= 5 ? 'bg-[#FFD700]/20 text-[#FFD700]'
