...
import {
    ...
    Play, ArrowRight, Trophy, RefreshCw, BookOpen,
    Target, Zap, Award, AlertCircle, ChevronRight,
    Timer, HelpCircle, Star
} from 'lucide-react';
...
const MockTestPrep = () => {
    ...
    const fetchCategories = async () => {
        try {
            const res = await axios.get(`${API_BASE}/practice/categories`, { withCredentials: true });
            if (res.data.success) {
            ...
            // Fallback categories
            setCategories([
                { id: 'technical', name: 'Programming & Technical', topics: ['JavaScript', 'React', 'Python', 'SQL'] },
                { id: 'hr', name: 'HR & Behavioral', topics: ['Behavioral', 'Situational', 'Leadership'] },
                { id: 'aptitude', name: 'Aptitude & Reasoning', topics: ['Logical', 'Quantitative', 'Verbal'] }
            ]);
        }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">AI-generated practice tests powered by Gemini</p>
                </div>
                {mode !== 'select' && (
                    <Button
                        onClick={resetTest}
                                    ...
                                    {['easy', 'medium', 'hard'].map((d) => (
                                        <button
                                            ...
                                            className={`flex-1 py-3 rounded-sm border text-sm font-medium capitalize transition-all ${difficulty === d
                                                ? d === 'easy' ? 'bg-green-500/10 border-green-500/50 text-green-400'
                                                    : d === 'medium' ? 'bg-[#FFD700]/10 border-[#FFD700]/50 text-[#FFD700]'
                    ...
                    >
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-md p-4">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <div className={`flex items-center gap-2 font-mono text-lg ${timeLeft < 60 ? 'text-red-400' : 'text-white'
                                    }`}>
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-md p-8 relative overflow-hidden">
                            ...
                            <div className="space-y-3">
                                {test.questions[currentQuestion]?.options.map((option, idx) => (
                                    <button
                                        key={idx}
                                        onClick={() => handleAnswer(test.questions[currentQuestion].id, idx)}
                ...
                {mode === 'result' && result && (
                    ...
                    >
                        ...
                        <div className="bg-[#111111] border border-white/10 rounded-md p-8 text-center relative overflow-hidden">
                            ...
                            <Trophy className={`w-16 h-16 mx-auto mb-4 ${result.passed ? 'text-[#FFD700]' : 'text-gray-500'
                                }`} />

                            <div className={`text-6xl font-bold font-mono mb-2 ${getGradeColor(result.grade)}`}>
                                {result.score}%
                            </div>

                            <Badge className={`text-lg px-4 py-1 ${result.passed
                                ? 'bg-green-500/20 text-green-400 border-green-500/30'
                                : 'bg-red-500/20 text-red-400 border-red-500/30'
                                }`}>
                                Grade: {result.grade} | {result.passed ? 'Passed!' : 'Keep Practicing'}
                            </Badge>

                            <div className="grid grid-cols-3 gap-4 mt-6">
                                <div className="p-4 bg-white/5 rounded-sm">
                                    <div className="text-2xl font-bold text-[#00FF94]">{result.correctAnswers}</div>
