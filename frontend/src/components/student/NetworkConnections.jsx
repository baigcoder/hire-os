...
import {
    ...
    Sparkles, Search, Filter, Check, X, Mail
} from 'lucide-react';
...
const NetworkConnections = () => {
    ...
    const fetchConnections = async () => {
        try {
            const res = await axios.get(`${CONNECTIONS_API_END_POINT}?status=accepted`, { withCredentials: true });
            if (res.data.success) {
    ...
    const fetchPendingRequests = async () => {
        try {
            const res = await axios.get(`${CONNECTIONS_API_END_POINT}/pending`, { withCredentials: true });
            if (res.data.success) {
    ...
    const fetchSuggestions = async () => {
        try {
            const res = await axios.get(`${CONNECTIONS_API_END_POINT}/suggestions`, { withCredentials: true });
            if (res.data.success) {
    ...
    const handleAccept = async (connectionId) => {
        try {
            const res = await axios.put(`${CONNECTIONS_API_END_POINT}/accept/${connectionId}`, {}, { withCredentials: true });
            if (res.data.success) {
    ...
    const handleReject = async (connectionId) => {
        try {
            const res = await axios.delete(`${CONNECTIONS_API_END_POINT}/${connectionId}`, { withCredentials: true });
            if (res.data.success) {
    ...
    const handleConnect = async (userId) => {
        try {
            const res = await axios.post(`${CONNECTIONS_API_END_POINT}/request/${userId}`, {}, { withCredentials: true });
            if (res.data.success) {
    ...
    const getRoleBadge = (role) => {
        switch (role) {
            ...
            case 'company_admin': return 'bg-orange-500/10 text-orange-400 border-orange-500/20';
                                    ...
                                    >
                                        <div className="flex items-center gap-3 mb-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-xs text-gray-500 truncate">{conn.user?.email}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <span className="text-xs text-gray-500">
                                                Connected {new Date(conn.connectedAt).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
                                            </span>
                                        </div>
                                        ...
                                        {conn.user?.profile?.skills?.length > 0 && (
                                            <div className="flex flex-wrap gap-1 mb-4">
                                                {conn.user.profile.skills.slice(0, 3).map((skill, i) => (
                                                    <Badge key={i} className="bg-white/5 text-gray-400 border-white/10 text-[10px]">
                                                        {skill}
                                                    </Badge>
                                                ))}
                                ...
                                {pendingRequests.map((request, idx) => (
                                    ...
                                    >
                                        <Avatar className="h-14 w-14 border-2 border-cyan-500/30">
                                            <AvatarImage src={request.requester?.profile?.profilePhoto} />
                                        ...
                                        <div className="flex-1">
                                            <h4 className="font-semibold text-white">{request.requester?.fullname}</h4>
                                            <p className="text-sm text-gray-500">{request.requester?.profile?.bio?.slice(0, 50)}...</p>
                                            {request.message && (
                                                <p className="text-xs text-cyan-400 mt-1">"{request.message}"</p>
                                            )}
                                        </div>
                ...
                {activeTab === 'suggestions' && (
                    ...
                    >
                        ...
                        {suggestions.length === 0 ? (
                            <div className="text-center py-16 bg-white/5 border border-white/10 rounded-2xl">
                                <Users className="w-16 h-16 mx-auto text-gray-600 mb-4" />
                                <h3 className="text-xl font-medium text-white mb-2">No suggestions available</h3>
                                <p className="text-gray-500">Check back later for new suggestions</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
