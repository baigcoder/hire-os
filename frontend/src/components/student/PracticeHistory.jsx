...
import {
    Clock, Brain, Video, Trophy, Target, TrendingUp,
    Calendar, ChevronRight, Filter, RefreshCw, Trash2,
...
const PracticeHistory = () => {
    ...
    const getGradeColor = (grade) => {
        if (grade === 'A+' || grade === 'A') return 'text-[#00FF94] bg-[#00FF94]/10 border-[#00FF94]/30';
        if (grade === 'B+' || grade === 'B') return 'text-[#FFD700] bg-[#FFD700]/10 border-[#FFD700]/30';
        if (grade === 'C+' || grade === 'C') return 'text-orange-400 bg-orange-400/10 border-orange-400/30';
                ...
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                            ...
                            <p className="text-gray-500 text-sm">Track your interview and test preparation progress</p>
                        </div>
                    </div>
                </div>
                ...
                >
                    <div className="bg-[#111111] border border-[#2a2a2a] rounded-sm p-4">
                        ...
                        <p className="text-2xl font-bold text-white font-mono">
                            {(stats.stats?.mock_test?.totalSessions || 0) + (stats.stats?.mock_interview?.totalSessions || 0)}
                        </p>
                    </div>
                    ...
                    <div className="bg-[#111111] border border-[#2a2a2a] rounded-sm p-4">
                        ...
                        <p className="text-2xl font-bold text-[#00FF94] font-mono">
                            {Math.round(((stats.stats?.mock_test?.avgScore || 0) + (stats.stats?.mock_interview?.avgScore || 0)) / 2)}%
                        </p>
                    </div>
                    ...
                    <div className="bg-[#111111] border border-[#2a2a2a] rounded-sm p-4">
                        ...
                        <p className="text-2xl font-bold text-cyan-400 font-mono">
                            {Math.max(stats.stats?.mock_test?.bestScore || 0, stats.stats?.mock_interview?.bestScore || 0)}%
                        </p>
                    </div>
            ...
            >
                {loading ? (
                    ...
                    <div className="p-8 text-center">
                        ...
                        <h3 className="text-lg font-bold text-white mb-2">No Practice Sessions Yet</h3>
                        <p className="text-gray-500 mb-4">Start practicing to build your history!</p>
                        ...
                        {history.map((session, idx) => (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    {/* Type Icon */}
                                    <div className={`w-12 h-12 rounded-sm flex items-center justify-center ${session.type === 'mock_interview'
                                        ? 'bg-cyan-400/10'
                                        : 'bg-[#FFD700]/10'
                                        }`}>
                                    ...
                                    <div className="text-right">
                                        <p className="text-2xl font-black text-[#00FF94] font-mono">{session.score}%</p>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <ChevronRight className={`w-5 h-5 text-gray-500 transition-transform ${selectedSession?._id === session._id ? 'rotate-90' : ''
                                            }`} />
                                    </div>
                                </div>
                                ...
                                <AnimatePresence>
                                    {selectedSession?._id === session._id && (
                                        ...
                                        >
                                            {session.type === 'mock_interview' && session.interviewData?.summary && (
                                                <div className="space-y-4">
                                                    <div className="grid grid-cols-3 gap-4">
                                                        <div className="bg-white/5 rounded-sm p-3 text-center">
                                                            <p className="text-xs text-gray-500 mb-1">Communication</p>
                                                            <p className="text-xl font-bold text-cyan-400">{session.interviewData.summary.communicationScore}%</p>
                                                        </div>
                                                        <div className="bg-white/5 rounded-sm p-3 text-center">
                                                            <p className="text-xs text-gray-500 mb-1">Confidence</p>
                                                            <p className="text-xl font-bold text-purple-400">{session.interviewData.summary.confidenceScore}%</p>
                                                        </div>
                                                        <div className="bg-white/5 rounded-sm p-3 text-center">
                                                            <p className="text-xs text-gray-500 mb-1">Questions</p>
                                                            <p className="text-xl font-bold text-[#FFD700]">{session.interviewData.questionsAnswered}</p>
                                                        </div>
                                                    </div>

                                                    {session.interviewData.summary.strengths?.length > 0 && (
                                                        <div>
                                                            <p className="text-xs text-gray-500 uppercase mb-2">Strengths</p>
                                                            <div className="flex flex-wrap gap-2">
                                                                {session.interviewData.summary.strengths.map((s, i) => (
                                                                    <Badge key={i} className="bg-[#00FF94]/10 text-[#00FF94] border-[#00FF94]/30">
                                                    ...
                                                    {session.interviewData.summary.improvements?.length > 0 && (
                                                        <div>
                                                            <p className="text-xs text-gray-500 uppercase mb-2">Areas to Improve</p>
                                                            <div className="flex flex-wrap gap-2">
                                                                {session.interviewData.summary.improvements.map((s, i) => (
                                                                    <Badge key={i} className="bg-orange-400/10 text-orange-400 border-orange-400/30">
                                            ...
                                            {session.type === 'mock_test' && session.testData && (
                                                <div className="grid grid-cols-3 gap-4">
                                                    <div className="bg-white/5 rounded-sm p-3 text-center">
                                                        ...
                                                        <p className="text-xl font-bold text-[#00FF94]">{session.testData.correctAnswers}</p>
                                                    </div>
                                                    <div className="bg-white/5 rounded-sm p-3 text-center">
                                                        <p className="text-xs text-gray-500 mb-1">Wrong</p>
                                                        <p className="text-xl font-bold text-red-400">{session.testData.wrongAnswers}</p>
                                                    </div>
                                                    <div className="bg-white/5 rounded-sm p-3 text-center">
                                                        <p className="text-xs text-gray-500 mb-1">Total</p>
                                                        <p className="text-xl font-bold text-[#FFD700]">{session.testData.totalQuestions}</p>
                                                    </div>
                                                </div>
                                            )}
                                        </motion.div>
