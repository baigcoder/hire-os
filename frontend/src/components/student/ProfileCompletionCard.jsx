...
const ProfileCompletionCard = () => {
  ...
  const completionData = useMemo(() => {
    ...
    const checks = [
      ...
      },
      {
        ...
        label: "PHOTO",
        icon: Image,
        complete: !!user.profile?.profilePhoto,
      },
      {
        id: "bio",
        label: "BIO",
        icon: FileText,
        complete: user.profile?.bio?.length > 20,
      },
      {
        id: "skills",
        label: "SKILLS 5+",
        icon: Award,
        complete: (user.profile?.skills?.length || 0) >= 5,
      },
      {
        id: "resume",
        label: "RESUME",
        icon: FileText,
        complete: !!user.profile?.resume,
      },
      {
        id: "experience",
        label: "EXPERIENCE",
        icon: Briefcase,
        complete: (user.profile?.experience?.length || 0) > 0,
      },
      {
        id: "education",
        label: "EDUCATION",
        icon: GraduationCap,
        complete: (user.profile?.education?.length || 0) > 0,
      },
    ];
  ...
  const getProgressStyle = (percentage) => {
    if (percentage >= 80)
      return { color: "emerald", gradient: "from-emerald-500 to-green-400" };
    if (percentage >= 60)
      return { color: "[#FFD700]", gradient: "from-[#FFD700] to-amber-400" };
    if (percentage >= 40)
  ...
  const getMessage = (percentage) => {
    if (percentage >= 100)
      return {
        text: "PROFILE COMPLETE — MAXIMUM VISIBILITY ACHIEVED",
        type: "success",
      };
    if (percentage >= 80)
      return { text: "ALMOST THERE — COMPLETE REMAINING ITEMS", type: "info" };
    if (percentage >= 60)
      return { text: "GOOD PROGRESS — ADD MORE DETAILS", type: "warning" };
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-500 font-mono tracking-wider">
              COMPLETE PROFILES GET 40% MORE VIEWS
            </p>
          </div>
        </div>
        <div className="text-right">
          <span
            className={`text-3xl font-bold font-mono text-${style.color}-400`}
          >
      ...
      {/* Message */}
      <div
        className={`flex items-center gap-2 p-3 rounded-sm mb-5 ${
      ...
      <div className="grid grid-cols-3 gap-2 mb-5">
        {completionData.checks?.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`flex items-center gap-2 p-2.5 rounded-sm text-[10px] font-mono transition-all ${
                item.complete
