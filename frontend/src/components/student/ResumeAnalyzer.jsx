...
import {
    FileText, Upload, Target, Zap, CheckCircle, XCircle,
    Loader2, TrendingUp, Award, AlertTriangle, BookOpen,
    ChevronDown, ChevronUp, Sparkles, RefreshCw, FileSearch,
    GraduationCap, Briefcase, Code, ArrowRight
} from 'lucide-react';
...
const ResumeAnalyzer = () => {
    ...
    const handlePDFUpload = async (e) => {
        ...
        try {
            ...
            if (error.response?.data?.code === 'EMPTY_RESUME') {
                toast.error('Resume appears empty. Please upload a valid resume with content.');
            } else {
    ...
    const getSuggestions = () => {
        ...
        if (Array.isArray(analysis.onePageSummary?.improvementAreas) && analysis.onePageSummary.improvementAreas.length > 0) {
        ...
        if (Array.isArray(analysis.skillAnalysis?.missing) && analysis.skillAnalysis.missing.length > 0) {
            return analysis.skillAnalysis.missing.map((item) => {
            ...
            <AnimatePresence mode="wait">
                {loading ? (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                            {loadingSteps.map((step, idx) => (
                                <motion.div
                                    key={idx}
                                    className={`w-3 h-3 rounded-full transition-all duration-300 ${idx <= loadingStep
                                        ? 'bg-[#FFD700]'
                                        : 'bg-white/20'
                                        }`}
                                    animate={{
                            ...
                            <div className="bg-[#111111] border border-white/10 rounded-md p-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <Badge className={`${getFitBadge(existingAnalysis.overallFit)} border`}>
                                                {existingAnalysis.overallFit}
                                            </Badge>
                                            <p className="text-gray-500 text-xs mt-1">
                                                Last analyzed: {new Date(existingAnalysis.analyzedAt).toLocaleDateString()}
                                            </p>
                                        </div>
                                    </div>
                                    <Button
                                    ...
                                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                        {pdfFile ? (
                                            <>
                                                ...
                                                <p className="text-sm text-[#FFD700] font-medium">{pdfFile.name}</p>
                    ...
                    >
                            ...
                            <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                                ...
                                <div className="relative">
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                        <span className="text-4xl font-bold text-white font-mono">{analysis.score}</span>
                                ...
                                <div className="flex-1 space-y-4">
                                    <div className="flex items-center gap-3">
                                        <Badge className={`${getFitBadge(analysis.overallFit)} border px-3 py-1 text-sm`}>
                                            {analysis.overallFit} Match
                                        </Badge>
                                        {analysis.source && (
                                            <Badge className="bg-cyan-500/10 text-cyan-400 border-cyan-500/30 text-xs">
                                                {analysis.source?.includes('gpt') ? '🚀 GPT-4o mini' :
                                                    analysis.source === 'gemini-ai' ? '✨ Gemini AI' :
                                                        analysis.analysisMethod === 'unified-3-engine' ? '🔥 3-Engine AI' : 'Quick Scan'}
                                            </Badge>
                                        )}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="p-3 bg-white/5 rounded-sm">
                                            <div className="text-xs text-gray-500 uppercase mb-1">ATS Score</div>
                                            <div className="text-xl font-bold text-white font-mono">{analysis.atsScore || '--'}%</div>
                                        </div>
                                        <div className="p-3 bg-white/5 rounded-sm">
                                            <div className="text-xs text-gray-500 uppercase mb-1">Skills Found</div>
                                        ...
                                        <div className="p-3 bg-white/5 rounded-sm">
                                            ...
                                            <div className="text-xl font-bold text-white font-mono">
                                                {analysis.experienceYears ? `${analysis.experienceYears} yrs` :
                                                    analysis.experience?.totalYears ? `${analysis.experience.totalYears} yrs` :
                                                        analysis.candidate?.estimatedExperience ? `${analysis.candidate.estimatedExperience} yrs` : '--'}
                                            </div>
                                        </div>
                                        <div className="p-3 bg-white/5 rounded-sm">
                                            <div className="text-xs text-gray-500 uppercase mb-1">Education</div>
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                            ...
                            <div className="bg-[#111111] border border-white/10 rounded-md p-5">
                                <h3 className="text-sm font-bold text-[#00FF94] uppercase tracking-wider mb-4 flex items-center gap-2">
                                    ...
                                    Skills Matched ({(analysis.keySkillsMatch || analysis.skills?.matched || analysis.skills?.technical || []).length})
                                </h3>
                                <div className="flex flex-wrap gap-2">
                                    {(analysis.keySkillsMatch || analysis.skills?.matched || analysis.skills?.technical || []).slice(0, 15).map((skill, idx) => (
                                        <Badge
                                            key={idx}
                                    ...
                                    {(analysis.keySkillsMatch || analysis.skills?.matched || analysis.skills?.technical || []).length === 0 && (
                            ...
                            <div className="bg-[#111111] border border-white/10 rounded-md p-5">
                                <h3 className="text-sm font-bold text-amber-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                                    ...
                                    Skills to Add ({(analysis.missingSkills || analysis.skills?.missing || []).length})
                                </h3>
                                <div className="flex flex-wrap gap-2">
                                    {(analysis.missingSkills || analysis.skills?.missing || []).slice(0, 8).map((skill, idx) => (
                                        <Badge
                                            key={idx}
                                    ...
                                    {(analysis.missingSkills || analysis.skills?.missing || []).length === 0 && (
                                        <span className="text-gray-500 text-sm">Great coverage!</span>
                                    )}
                                </div>
                            </div>
                        </div>
                                ...
                                <ul className="space-y-2">
                                    {getSuggestions().map((suggestion, idx) => (
                                        <li key={idx} className="flex items-start gap-2 text-gray-300 text-sm">
                                            ...
                                            {typeof suggestion === 'string' ? suggestion : suggestion?.suggestion || suggestion?.title || JSON.stringify(suggestion)}
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        ...
                        {analysis.atsOptimizationTips && analysis.atsOptimizationTips.length > 0 && (
                        ...
                        {analysis.learningResources && analysis.learningResources.length > 0 && (
                            <div className="bg-[#111111] border border-purple-500/20 rounded-md p-5">
                                ...
                                <div className="space-y-3">
                                    {analysis.learningResources.slice(0, 5).map((resource, idx) => (
                                        <div key={idx} className="bg-white/5 rounded-sm p-3">
                                            <div className="flex items-center justify-between mb-1">
                                                <span className="text-white font-medium text-sm">{resource.skill}</span>
                                                <Badge className={`text-xs ${resource.priority === 'Critical' ? 'bg-red-500/20 text-red-400 border-red-500/30' :
                                                        resource.priority === 'High' ? 'bg-amber-500/20 text-amber-400 border-amber-500/30' :
                                    ...
                                    {analysis.interviewQuestions.slice(0, 4).map((q, idx) => (
                                        <div key={idx} className="bg-white/5 rounded-sm p-3">
                                            ...
                                            <p className="text-gray-500 text-xs mb-1">
                                                <span className="text-pink-400">Why this matters:</span> {q.reason}
                                            </p>
                                            {q.howToPrepare && (
                                                <p className="text-gray-400 text-xs">
                                                    <span className="text-[#FFD700]">How to prepare:</span> {q.howToPrepare}
                                                </p>
                                            )}
                                        </div>
                                    ))}
                                ...
                                <ul className="space-y-2">
                                    {analysis.keyStrengths.slice(0, 5).map((strength, idx) => (
                                        <li key={idx} className="flex items-start gap-2 text-gray-300 text-sm">
                                            ...
                                            {typeof strength === 'string' ? strength : strength?.strength || strength}
                                        </li>
                                    ))}
                                </ul>
                            </div>
                            ...
                            <div className="bg-[#111111] border border-white/10 rounded-md p-5">
                                ...
                                <div className="grid md:grid-cols-2 gap-4">
                                    <div className="p-3 bg-white/5 rounded-sm">
                                        ...
                                        <div className="text-lg font-bold text-white">{analysis.competitorComparison.marketPosition}</div>
                                    </div>
                                    {analysis.competitorComparison.standoutFactor && (
                                        <div className="p-3 bg-white/5 rounded-sm">
                                            <div className="text-xs text-gray-500 uppercase mb-1">Standout Factor</div>
                                            <div className="text-sm text-gray-300">{analysis.competitorComparison.standoutFactor}</div>
                                        </div>
                                    )}
                                </div>
                                {analysis.competitorComparison.competitiveAdvantage && (
                                    <div className="mt-3 p-3 bg-[#FFD700]/10 rounded-sm">
                                        <div className="text-xs text-[#FFD700] uppercase mb-1">Competitive Advantage</div>
                                        <div className="text-sm text-gray-300">{analysis.competitorComparison.competitiveAdvantage}</div>
                                    </div>
                                )}
                            </div>
                        )}
