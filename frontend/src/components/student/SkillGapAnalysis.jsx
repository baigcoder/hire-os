...
import {
    Target, TrendingUp, Sparkles, CheckCircle, AlertCircle,
    Lightbulb, ArrowRight, RefreshCw, Brain, Zap
} from 'lucide-react';
...
const SkillGapAnalysis = () => {
    ...
    const fetchSkillGap = async (isRefresh = false) => {
        try {
            ...
            const res = await axios.get(`${CAREER_INSIGHTS_API_END_POINT}/skill-gap`, { withCredentials: true });
            if (res.data.success) {
            ...
            <div className="space-y-6">
                <div className="bg-[#111111] border border-white/10 rounded-2xl p-8">
                    <div className="flex flex-col items-center justify-center py-12">
                        ...
                        <p className="text-gray-400 mt-4 font-mono text-sm">ANALYZING YOUR SKILLS...</p>
                        <p className="text-gray-600 text-xs mt-2">Comparing with market demand</p>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="flex items-center gap-6 mb-6">
                            <div className="relative w-32 h-32">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center">
                                    <span className="text-3xl font-bold text-white">{analysis?.overallReadiness || 0}%</span>
                                    <span className="text-xs text-gray-500">READY</span>
                                </div>
                            </div>
                            <div className="flex-1">
                                <p className="text-gray-300 text-sm leading-relaxed">
                                    {analysis?.careerAdvice || 'Complete your profile to get personalized career advice.'}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <h4 className="text-sm font-medium text-gray-400 mb-3 flex items-center gap-2">
                                    <CheckCircle className="w-4 h-4 text-emerald-400" /> Your Strong Skills
                                </h4>
                                <div className="space-y-2">
                                    {(analysis?.strongSkills || []).length > 0 ? (
                                        analysis.strongSkills.map((skill, idx) => (
                                            ...
                                            >
                                                ...
                                                <span className="text-sm text-emerald-300">{skill}</span>
                                            </div>
                                        ))
                                    ) : (
                            ...
                            <div>
                                <h4 className="text-sm font-medium text-gray-400 mb-3 flex items-center gap-2">
                                    <AlertCircle className="w-4 h-4 text-red-400" /> Skills to Learn
                                </h4>
                                <div className="space-y-2">
                                    {(analysis?.skillGaps || []).length > 0 ? (
                                        ...
                                        <p className="text-gray-500 text-sm">Great! No major gaps identified</p>
                                    )}
                                </div>
                            </div>
                        </div>
                    ...
                    {analysis?.prioritySkills?.length > 0 && (
                        ...
                        >
                            <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                                <Lightbulb className="text-[#FFD700]" /> Priority Skills to Develop
                            </h3>
                                ...
                                {analysis.prioritySkills.map((item, idx) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`px-3 py-1 rounded-full text-xs font-bold ${getPriorityColor(item.priority)}`}>
                                                {item.priority?.toUpperCase()}
                                            </div>
                                            <span className="font-medium text-white">{item.skill}</span>
                                        </div>
                                        <p className="text-sm text-gray-500 max-w-xs text-right">{item.reason}</p>
                                    </div>
                                ))}
                            </div>
                        </motion.div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-[#111111] border border-white/10 rounded-2xl p-5">
                        <h4 className="font-bold text-white mb-4">Your Skills ({userSkills.length})</h4>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="w-16 h-1.5 bg-white/10 rounded-full overflow-hidden">
                                            <div
                                                ...
                                                style={{ width: `${Math.min(100, (item.demand / trendingSkills[0]?.demand) * 100)}%` }}
                                            />
                                        </div>
                                        <span className="text-xs text-gray-500 w-8">{item.demand}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
