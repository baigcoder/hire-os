...
export const useSocket = (interviewId, options = {}) => {
    ...
    useEffect(() => {
        ...
        socket.on('connect', () => {
            ...
            if (interviewId) {
                socket.emit('join-interview', {
                    interviewId,
                    role: user?.role === 'recruiter' || user?.role === 'company_admin'
                        ? 'recruiter'
                        : 'candidate'
                });
            }
        ...
        socket.on('participant-media-changed', ({ socketId, isAudioOn, isVideoOn, isScreenSharing }) => {
    ...
    // Media state updates
    const updateMediaState = useCallback((state) => {
        if (!socketRef.current || !interviewId) return;
