...
export const useSubscription = () => {
    ...
    const handleExpiredSubscription = useCallback((data) => {
    ...
    const isLimitReached = useCallback((limitType) => {
        ...
        switch (limitType) {
            case 'jobs':
                return features.maxJobPostings !== -1 && usage.activeJobs >= features.maxJobPostings;
            case 'recruiters':
                return features.maxRecruiters !== -1 && usage.activeRecruiters >= features.maxRecruiters;
            default:
                return false;
        }
    }, [subscription]);
    ...
    const getUsagePercent = useCallback((limitType) => {
        ...
        switch (limitType) {
            ...
            case 'recruiters':
                ...
                return Math.round((usage.activeRecruiters / features.maxRecruiters) * 100);
            default:
                return 0;
        }
    }, [subscription]);
